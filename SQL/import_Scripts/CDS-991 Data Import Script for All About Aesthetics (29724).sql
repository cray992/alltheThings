...
SELECT DISTINCT
          ...
          d.npi ,  -- NPI - int
		  d.[External]
FROM dbo._import_1_18_Doctor d 
...
FROM dbo.[_import_1_18_InsuranceCompany] i
	INNER JOIN dbo.[_import_1_18_InsuranceCompanyPlan] icp ON
		i.InsuranceCompanyID = icp.InsuranceCompanyID
	INNER JOIN dbo.[_import_1_18_InsurancePolicy] ip ON
...
FROM dbo.InsuranceCompany ic 
	INNER JOIN dbo.[_import_1_18_InsuranceCompany] i ON
...
FROM dbo.[_import_1_18_PracticeToInsuranceCompany] ptic
...
SELECT 
          ...
          @VendorImportID 
FROM dbo.[_import_1_18_InsuranceCompanyPlan] i
	INNER JOIN dbo.InsuranceCompany ic ON 
		i.insurancecompanyid = ic.VendorID AND
		ic.VendorImportID = @VendorImportID
WHERE i.InsuranceCompanyPlanID IN (SELECT InsuranceCompanyPlanID FROM dbo._import_1_18_InsurancePolicy WHERE PracticeID = @SourcePracticeID)
PRINT CAST (@@ROWCOUNT AS VARCHAR(10)) + ' records inserted '
...
FROM dbo.[_import_1_18_Doctor] i WHERE i.PracticeID = @SourcePracticeID AND i.[External] = 1 AND
...
--FROM dbo.[_import_1_18_CollectionCategory] i 
...
INNER JOIN dbo._import_1_18_ServiceLocation osl ON
...
FROM dbo._import_1_18_Patient i 
INNER JOIN dbo._import_1_18_CollectionCategory cc ON 
...
FROM dbo.[_import_1_18_Patient] p
...
FROM dbo.[_import_1_18_PatientAlert] pa
...
SELECT DISTINCT
          ...
          pjn.NoteTypeCode ,
          pjn.lastnote 
FROM dbo.[_import_1_18_PatientJournalNote] pjn
	INNER JOIN dbo.Patient p ON
...
FROM dbo.[_import_1_18_PatientCase] pc
	INNER JOIN dbo.Patient p ON 
...
FROM dbo.[_import_1_18_InsurancePolicy] ip
