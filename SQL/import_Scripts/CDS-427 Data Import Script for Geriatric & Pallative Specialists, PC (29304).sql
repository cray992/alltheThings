...
--SELECT DISTINCT insuredplanname2 FROM dbo.[_import_1_1_PatDemoIns] pd WHERE NOT EXISTS (SELECT * FROM dbo.[_import_1_1_InsList] il
--WHERE il.payorname = pd.insuredplanname2)
...
SELECT  DISTINCT
          il.payorname, -- InsuranceCompanyName - varchar(128)
     --   '' , -- Notes - text
          il.address1 , -- AddressLine1 - varchar(256)
          il.address2 , -- AddressLine2 - varchar(256)
          il.city	 , -- City - varchar(128)
          il.stateorregion , -- State - varchar(2) 
          '' , -- Country - varchar(32)
          CASE WHEN LEN(il.postalcode) = 4 THEN '0' + il.postalcode ELSE LEFT(il.postalcode, 9) END  , -- ZipCode - varchar(9)
      ...
      /*  '' , -- ContactFirstName - varchar(64)
          ...
          LEFT(il.phonenumber,10) , -- Phone - varchar(10)
          CASE WHEN LEN(il.phonenumber) > 10 THEN LEFT(SUBSTRING(il.phonenumber,11,LEN(il.phonenumber)),10) ELSE NULL END , -- PhoneExt - varchar(10)
       /* '' , -- Fax - varchar(10)
          '' , -- FaxExt - varchar(10) */
     ...
     /*   '' , -- LocalUseFieldTypeCode - char(5)
      ...
      --  NULL , -- RecordTimeStamp - timestamp
          ...
          il.payorname, -- VendorID - varchar(50)
...
FROM dbo.[_import_2_1_InsList] as il
where il.payorname <>''
...
FROM dbo.[_import_2_1_PatDemoIns] i
...
SELECT DISTINCT 
      ...
      --  0 , -- PatientReferralSourceID - int
           CASE WHEN pd.preferredphysician = '' THEN NULL ELSE pd.preferredphysician END  , -- PrimaryProviderID - int
...
FROM dbo.[_import_2_1_PatDemoIns] AS pd
...
INNER JOIN dbo.InsuranceCompanyPlan AS inscoplan ON
    inscoplan.VendorID = pd.insuredplanname AND
