...
INNER JOIN dbo.[_import_2_1_MASTER] AS m ON
   pat.medicalrecordnumber = m.medicalrecordnumber 
   AND pat.vendorImportID =1 
   -- pat.vendorImportID = @vendorImportID 
WHERE m.balanceduepatient <> '' 
...
WHERE patcase.Name = 'Balance Forward' -- AND m.balanceduepatient <>''
...
SELECT DISTINCT
          ...
          -50 , -- ModifiedUserID - int
      --  NULL , -- RecordTimeStamp - timestamp
          m.balanceduepatient , -- ServiceChargeAmount - money   
...
INNER JOIN dbo._import_2_1_MASTER as m ON 
   m.medicalrecordnumber = enc.vendorID 
