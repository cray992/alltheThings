...
SET @PracticeID = 22
SET @VendorImportID = 11 -- Vendor import record created through import tool
...
SELECT    @PracticeID , -- PracticeID - int
          ...
          1  -- External - bit
FROM dbo._import_11_22_referral ref
...
SELECT DISTINCT
	...
	,CASE WHEN pat.[referralid] <> '' THEN 
		(SELECT DoctorID FROM dbo.Doctor WHERE VendorID = referralid AND PracticeID = @PracticeID) ELSE NULL END 
	...
	,1
FROM dbo.[_import_11_22_Patient] pat
...
LEFT JOIN dbo.[_import_11_22_Coverage] cov ON
	impCase.patientid = cov.patientid
WHERE impCase.patientid NOT IN (SELECT patientid FROM dbo.[_import_11_22_Coverage])
PRINT CAST(@@ROWCOUNT AS VARCHAR(10)) + ' records inserted'
	*/
...
FROM dbo._import_11_22_PatientNotes pjn
INNER JOIN dbo.Patient realP ON
...
SELECT DISTINCT
	...
	,@VendorImportID
	,'Y'
FROM dbo.[_import_11_22_Policy] impIP
LEFT JOIN dbo._import_11_22_Coverage cov ON 
...
LEFT JOIN dbo.PatientCase pc ON
	CAST(pc.Notes AS VARCHAR(20)) = cov.patientid + cov.policyid AND 
	pc.NAME = 'Case ' + ic.InsuranceCompanyName
...
PRINT 'Inserting records into ContractFeeSchedule (Standard)...'
	...
	SELECT
		...
		,0
		,0
	FROM dbo.[_import_11_22_FeeSchedule] impFS
...
FROM dbo.[_import_11_22_Patient] pa
