...
FROM dbo._import_1_6_Case AS ins
...
FROM dbo.[_import_1_6_PatientDemographics] 
...
SELECT DISTINCT
	...
	@VendorImportID -- VendorImportID - int
FROM dbo.[_import_1_6_Case] impCase
...
SELECT  distinct  
	...
	impPolicy.alttype , -- PolicyNumber - varchar(32)
	...
	1 , -- Active - bit
	@PracticeID , -- PracticeID - int
	impPolicy.acctplan + impPolicy.althohflag , -- VendorID - varchar(50)
	@VendorImportID -- VendorImportID - int
FROM dbo.[_import_1_6_Case] AS impCase
INNER JOIN
	(
		SELECT impPolicy.acctplan, impPolicy.althohflag, impPolicy.alttype FROM _import_1_6_InsuredID impPolicy
		INNER JOIN
			(
				SELECT acctplan, althohflag, MAX(AutoTempID) AS LatestPolicyRowID FROM dbo.[_import_1_6_InsuredID] GROUP BY acctplan, althohflag
			) t1 ON impPolicy.AutoTempID = t1.LatestPolicyRowID
	) impPolicy ON
	impCase.[table] = impPolicy.acctplan AND
	impCase.acctplan = impPolicy.althohflag
...
INNER JOIN dbo.InsuranceCompanyPlan AS icp ON 
	...
	icp.PlanName = impPolicy.althohflag
INNER JOIN dbo.[_import_1_6_GuarantorList] AS impGuarantor ON
...
FROM dbo.[_import_1_6_FeeSchedule] impSFS
