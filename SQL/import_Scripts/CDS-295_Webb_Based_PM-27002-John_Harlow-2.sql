...
IF  EXISTS (SELECT * FROM sys.objects WHERE object_id = OBJECT_ID(N'[dbo].[_import_23_23_Insurances]') AND type in (N'U'))
BEGIN
...
PRINT 'Inserting into Insurance Company...'
      ...
      FROM dbo._import_23_23_Insurances
...
PRINT 'Inserting into Insurance Company Plan...'
      ...
      SELECT DISTINCT
                    ...
                    @VendorImportID  -- VendorImportID - int
      FROM dbo._import_23_23_Insurances AS impIns
...
IF  EXISTS (SELECT * FROM sys.objects WHERE object_id = OBJECT_ID(N'[dbo].[_import_23_23_ReferringPhys]') AND type in (N'U'))
BEGIN
...
PRINT 'Inserting into Referring Doctor...'
      ...
      SELECT DISTINCT
                    ...
                    1  -- External - bit
      FROM dbo._import_23_23_ReferringPhys AS impRph
      WHERE impRph.[firstname] <> '' AND impRph.[lastname] <> ''
PRINT CAST(@@ROWCOUNT AS VARCHAR) + ' records inserted'
END 
*/
-- Patients (only if table exists)
IF  EXISTS (SELECT * FROM sys.objects WHERE object_id = OBJECT_ID(N'[dbo].[_import_23_23_PatientData]') AND type in (N'U'))
BEGIN
...
PRINT 'Inserting into Patient...'
      ...
      FROM dbo._import_23_23_PatientData AS impPat
      LEFT OUTER JOIN dbo._import_23_23_Providers AS impPro ON
...
IF  EXISTS (SELECT * FROM sys.objects WHERE object_id = OBJECT_ID(N'[dbo].[_import_23_23_CaseInformation]') AND type in (N'U'))
BEGIN
...
FROM dbo.[_import_23_23_CaseInformation] AS impCas
...
INSERT INTO dbo.InsurancePolicy
      ...
      INNER JOIN 
            ...
            FROM dbo._import_23_23_PatientData) AS holder ON
...
IF  EXISTS (SELECT * FROM sys.objects WHERE object_id = OBJECT_ID(N'[dbo].[_import_23_23_StandardFeeSchedule]') AND type in (N'U'))
BEGIN
...
PRINT 'Inserting into Standard Fee...'
      ...
      FROM dbo._import_23_23_StandardFeeSchedule AS Impsfs
...
IF  EXISTS (SELECT * FROM sys.objects WHERE object_id = OBJECT_ID(N'[dbo].[_import_23_23_Appointment]') AND type in (N'U'))
BEGIN
...
PRINT 'Inserting into Appointment...'
      ...
      FROM dbo.[_import_23_23_Appointment] AS impApp
...
PRINT 'Inserting into Appointment To Resource...'
      ...
      INNER JOIN dbo._import_23_23_Providers AS impPro ON
...
PRINT 'Inserting into Patient Case for Balance Forward...'
      ...
      INNER JOIN dbo._import_23_23_PatientData AS impPat ON
...
PRINT 'Inserting into Encounter for Balance Forward...'
      ...
      FROM dbo._import_23_23_PatientData impPat
...
INNER JOIN dbo.[_import_23_23_CaseInformation] impCas ON
	impCas.chartnumber = pat.VendorID
INNER JOIN dbo.[_import_23_23_PatientData] impPat ON
...
impCas.chartnumber = (SELECT TOP 1 impCas.chartnumber FROM dbo.[_import_23_23_CaseInformation] WHERE
