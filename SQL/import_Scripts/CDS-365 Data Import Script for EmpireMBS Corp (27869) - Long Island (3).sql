...
DECLARE @Practice3VendorImportID INT

SET @PracticeID3 = 3
SET @Practice3VendorImportID = 3

PRINT ''
PRINT ''
PRINT 'PracticeID = ' + CAST(@PracticeID3 AS VARCHAR(10))
PRINT 'VendorImportID = ' + CAST(@Practice3VendorImportID AS VARCHAR(10))

--DELETE FROM dbo.InsurancePolicy WHERE PracticeID = @PracticeID3 AND VendorImportID = @Practice3VendorImportID
--PRINT '    ' + CAST(@@ROWCOUNT AS VARCHAR(10)) + ' Insurance Policy records deleted'
--DELETE FROM dbo.InsuranceCompanyPlan WHERE VendorImportID = @Practice3VendorImportID
--PRINT '    ' + CAST(@@ROWCOUNT AS VARCHAR(10)) + ' Insurance Company Plan records deleted'
--DELETE FROM dbo.InsuranceCompany WHERE VendorImportID = @Practice3VendorImportID
--PRINT '    ' + CAST(@@ROWCOUNT AS VARCHAR(10)) + ' Insurance Company records deleted'
--DELETE FROM dbo.PatientCaseDate WHERE PatientCaseID IN (SELECT PatientCaseID FROM dbo.PatientCase WHERE PracticeID = @PracticeID3 AND VendorImportID = @Practice3VendorImportID)
--PRINT '    ' + CAST(@@ROWCOUNT AS VARCHAR(10)) + ' Patient Case Date records deleted'
--DELETE FROM dbo.PatientCase WHERE PracticeID = @PracticeID3 AND VendorImportID = @Practice3VendorImportID
--PRINT '    ' + CAST(@@ROWCOUNT AS VARCHAR(10)) + ' Patient Case records deleted'
--DELETE FROM dbo.PatientAlert WHERE PatientID IN (SELECT PatientID FROM dbo.Patient WHERE PracticeID = @PracticeID3 AND VendorImportID = @Practice3VendorImportID)
--PRINT '    ' + CAST(@@ROWCOUNT AS VARCHAR(10)) + ' Patient Alert records deleted'
--DELETE FROM dbo.Patient WHERE PracticeID = @PracticeID3 AND VendorImportID = @Practice3VendorImportID
--PRINT '    ' + CAST(@@ROWCOUNT AS VARCHAR(10)) + ' Patient records deleted'
--DELETE FROM dbo.Doctor WHERE PracticeID = @PracticeID3 AND VendorImportID = @Practice3VendorImportID
...
SELECT DISTINCT
		 ...
		 (CASE WHEN i.insurancetelephonerelations = '' THEN '' ELSE 'Phone Relations: ' + i.insurancetelephonerelations END) , -- Notes - text
          ...
          @PracticeID3 , -- CreatedPracticeID - int
          ...
          @Practice3VendorImportID , -- VendorImportID - int
          1 , -- NDCFormat - int
          1 , -- UseFacilityID - bit
          'U' , -- AnesthesiaType - varchar(1)
          18  -- InstitutionalBillingFormID - int
FROM dbo.[_import_3_3_InsuranceMaster] i
...
WHERE VendorImportID = @Practice3VendorImportID 
PRINT CAST(@@ROWCOUNT AS VARCHAR) + ' records inserted'
...
FROM dbo.[_import_3_3_ReferringMaster]
...
FROM dbo.[_import_3_3_LongIslandOBGYNAssociates] i
	LEFT JOIN dbo.Doctor rd ON
		i.patientreferringcode = rd.VendorID AND
		rd.VendorImportID = @Practice3VendorImportID
...
FROM dbo.[_import_3_3_LongIslandOBGYNAssociates] i
	INNER JOIN dbo.Patient p ON
		...
		p.VendorImportID = @Practice3VendorImportID
...
FROM dbo.[_import_3_3_PolicyMaster] i
	INNER JOIN dbo.PatientCase pc ON
		i.policyaccountnumber = pc.VendorID AND
		pc.VendorImportID = @Practice3VendorImportID
	INNER JOIN dbo.InsuranceCompanyPlan icp ON
		i.policyinsurancecode = icp.VendorID AND
		icp.CreatedPracticeID = @PracticeID3
...
UPDATE dbo.PatientCase 
      ...
      WHERE pc.VendorImportID = @Practice3VendorImportID AND
