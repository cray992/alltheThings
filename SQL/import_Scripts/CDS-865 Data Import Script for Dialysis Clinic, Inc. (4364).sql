...
SELECT DISTINCT
          ...
          d.npi ,  -- NPI - int
		  d.[External]
FROM dbo._import_1_9_Doctor d 
WHERE d.PracticeID = @SourcePracticeID 

UPDATE dbo.Doctor SET NPI = '1689694598' WHERE DoctorID = 2330 AND PracticeID = @TargetPracticeID
...
FROM dbo.[_import_1_9_InsuranceCompany] i
	INNER JOIN dbo.[_import_1_9_InsuranceCompanyPlan] icp ON
		i.InsuranceCompanyID = icp.InsuranceCompanyID
	INNER JOIN dbo.[_import_1_9_InsurancePolicy] ip ON
...
FROM dbo.InsuranceCompany ic 
	INNER JOIN dbo.[_import_1_9_InsuranceCompany] i ON
...
FROM dbo.[_import_1_9_PracticeToInsuranceCompany] ptic
...
SELECT 
          ...
          @VendorImportID 
FROM dbo.[_import_1_9_InsuranceCompanyPlan] i
	INNER JOIN dbo.InsuranceCompany ic ON 
		i.insurancecompanyid = ic.VendorID AND
		ic.VendorImportID = @VendorImportID
WHERE i.InsuranceCompanyPlanID IN (SELECT InsuranceCompanyPlanID FROM dbo._import_1_9_InsurancePolicy WHERE PracticeID = @SourcePracticeID)
PRINT CAST (@@ROWCOUNT AS VARCHAR(10)) + ' records inserted '
...
FROM dbo.[_import_1_9_Doctor] i WHERE i.PracticeID = @SourcePracticeID AND i.[External] = 1 AND
...
INNER JOIN dbo._import_1_9_ServiceLocation osl ON
...
FROM dbo.[_import_1_9_Patient] p
...
FROM dbo.[_import_1_9_PatientAlert] pa
...
SELECT DISTINCT
          ...
          pjn.NoteTypeCode ,
          pjn.lastnote 
FROM dbo.[_import_1_9_PatientJournalNote] pjn
	INNER JOIN dbo.Patient p ON
...
FROM dbo.[_import_1_9_PatientCase] pc
	INNER JOIN dbo.Patient p ON 
...
FROM dbo.[_import_1_9_InsurancePolicy] ip
...
SELECT 		
          ...
          @VendorImportID  -- VendorImportID - int
FROM dbo.[_import_1_9_InsurancePolicyAuthorization] i
