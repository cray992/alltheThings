...
--			 i.ssn  -- SSN - varchar(9)
--FROM dbo.Patient p 
--INNER JOIN dbo.[_import_26_1_PatientDemographics] i ON p.PatientID = i.id
...
FROM dbo.[_import_23_1_491171PatientDemoUpdate1] i
...
SELECT DISTINCT    
		  ...
		  i.ResponsiblePartyState , 
		  i.ResponsiblePartyZipCode 
FROM dbo.[_import_24_1_491171PatientDemoUpdate2] i
...
UPDATE dbo.Patient 
	SET MedicalRecordNumber = CASE WHEN i.mrn <> p.MedicalRecordNumber THEN i.mrn ELSE p.MedicalRecordNumber END ,
		AddressLine1 = CASE WHEN i.address1 <> p.AddressLine1 THEN i.address1 ELSE p.AddressLine1 END ,
...
FROM dbo.[_import_23_1_491171PatientDemoUpdate1] AS PD
	...
	LEFT JOIN dbo.Patient p ON PD.personid = p.VendorID AND p.VendorImportID IN (@VendorImportID1,@VendorImportID2,@VendorImportID3,@VendorImportID4,@VendorImportID5)
...
SELECT DISTINCT
	...
	CASE
		...
		ELSE '' END , -- EmergencyPhone - varchar(10)
	LEFT(dbo.fn_RemoveNonNumericCharacters(PD.Emergencyphoneext),10) -- EmergencyPhoneExt - varchar(10)
FROM dbo.[_import_24_1_491171PatientDemoUpdate2] AS PD
	LEFT JOIN dbo.Doctor AS ReferringDoc ON 
			ReferringDoc.DoctorID = (SELECT MIN(DoctorID) FROM dbo.Doctor AS RD WHERE RD.FirstName = PD.ReferringPhysicianFirstName AND 
											RD.LastName = PD.ReferringPhysicianLastName AND RD.PracticeID = @PracticeID AND RD.ActiveDoctor = 1)
	LEFT JOIN dbo.Doctor AS PrimaryProvider ON 
...
FROM dbo.[_import_23_1_491171PatientDemoUpdate1] pd
...
SELECT DISTINCT 
	...
	0 , -- EmergencyRelated - bit
	0 -- HomeboundRelatedFlag
FROM dbo.[_import_24_1_491171PatientDemoUpdate2] pd
	INNER JOIN dbo.Patient AS PAT ON pat.VendorImportID = @NewVendorImportID AND pat.VendorID = pd.personid
	LEFT JOIN dbo.PatientCase pc ON PAT.PatientID = pc.PatientID AND pc.PracticeID = @PracticeID
WHERE pc.PatientCaseID IS NULL
PRINT CAST(@@ROWCOUNT AS VARCHAR) + ' records inserted'	
...
(	SELECT DISTINCT 
	...
	FROM dbo.[_import_23_1_491171PatientDemoUpdate1]
...
(	SELECT DISTINCT 
		...
		('M','Male') THEN 'M' WHEN Holder1gender IN ('F','Female') THEN 'F' ELSE 'U' END AS HolderGender,
		holder1PolicyNumber AS HolderPolicyNumber, Employer1 AS Employer, Policy1Note AS PolicyNote
	FROM dbo.[_import_24_1_491171PatientDemoUpdate2]
	WHERE insurancecode1 <> ''
...
WHERE ip.Precedence = 1 AND ip.VendorImportID IN (@VendorImportID1,@VendorImportID2,@VendorImportID3,@VendorImportID4,@VendorImportID5)
PRINT CAST(@@ROWCOUNT AS VARCHAR) + ' records inserted'
...
LEFT JOIN dbo.InsuranceCompanyPlan ICP ON 
	TP.InsuranceCode = ICP.VendorID AND 
	ICP.VendorImportID IN (@VendorImportID1,@VendorImportID2,@VendorImportID3)
WHERE ip.Precedence = 2 AND ip.VendorImportID IN (@VendorImportID1,@VendorImportID2,@VendorImportID3,@VendorImportID4,@VendorImportID5) 
PRINT CAST(@@ROWCOUNT AS VARCHAR) + ' records inserted'
...
LEFT JOIN dbo.InsuranceCompanyPlan ICP ON 
	TP.InsuranceCode = ICP.VendorID AND 
	ICP.VendorImportID IN (@VendorImportID1,@VendorImportID2,@VendorImportID3)
WHERE ip.Precedence = 3 AND ip.VendorImportID IN (@VendorImportID1,@VendorImportID2,@VendorImportID3,@VendorImportID4,@VendorImportID5)
PRINT CAST(@@ROWCOUNT AS VARCHAR) + ' records inserted'
...
FROM #tempPolicies1 AS TP
	JOIN dbo.Patient AS PAT ON VendorImportID IN (@VendorImportID1,@VendorImportID2,@VendorImportID3,@VendorImportID4,@VendorImportID5,@NewVendorImportID) AND VendorID = PatientVendorID
	JOIN dbo.PatientCase AS PC ON PAT.PatientID = pc.PatientID AND PC.VendorImportID IN (@VendorImportID1,@VendorImportID2,@VendorImportID3,@VendorImportID4,@VendorImportID5,@NewVendorImportID)
...
FROM dbo.Doctor d 
	INNER JOIN dbo.[_import_25_1_491171Providers] i ON
		d.FirstName = i.firstname AND 
		d.LastName = i.lastname AND
        d.[External] = 1 AND 
		d.PracticeID = @PracticeID AND 
		d.ActiveDoctor = 1 
WHERE d.ActiveDoctor = 1 AND d.[External] = 1 AND d.PracticeID = @PracticeID AND i.deleteindicator = 'N'
...
FROM dbo.[_import_25_1_491171Providers] AS RP 
...
LEFT JOIN dbo.InsurancePolicy ip ON 
    ...
    ip.VendorImportID IN (@VendorImportID1,@VendorImportID2,@VendorImportID3,@VendorImportID4,@VendorImportID5,@NewVendorImportID)
WHERE pc.CreatedUserID = 0 AND pc.ModifiedUserID = 0 AND pc.VendorImportID IN (@VendorImportID1,@VendorImportID2,@VendorImportID3,@VendorImportID4,@VendorImportID5,@NewVendorImportID) 
