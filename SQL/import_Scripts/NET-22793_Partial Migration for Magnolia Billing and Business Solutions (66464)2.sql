...
--ALTER TABLE dbo._import_1_3_CaseInformation ADD planname VARCHAR(50)
--ALTER TABLE dbo.Patient  
--NOCHECK CONSTRAINT FK_Patient_Doctor;  
--GO  
...
SELECT DISTINCT ic.insurancecompanyid,
       ...
       0                               -- IsEnrollable - bit
--select * 
FROM dbo._import_1_3_InsuranceCompany ic
...
--SELECT icp.planname,* 
FROM dbo._import_1_3_CaseInformation ip
    INNER JOIN _import_1_3_InsuranceCompanyPlan icp
...
SELECT DISTINCT 
       ...
       @VendorImportID        -- VendorImportID - int
--select * 
FROM dbo._import_1_3_InsuranceCompanyPlan icp
...
FROM dbo._import_1_3_PatientDemographics i
...
SELECT  DISTINCT
          ...
          mp.alertmessage , -- AlertMessage - text
      ...
      --  NULL , -- RecordTimeStamp - timestamp
		  ...
		  --SELECT * 
FROM dbo._import_1_3_NotesAlerts as mp 
INNER JOIN dbo.Patient as pat on
         pat.PatientID = mp.id
     AND pat.PracticeID = @TargetPracticeID
WHERE mp.alertmessage <> ''     
PRINT CAST(@@ROWCOUNT AS VARCHAR) + ' records updated'
...
--select ci.* 
FROM dbo._import_1_3_CaseInformation ci
...
SELECT  DISTINCT 
	...
	--SELECT * 
FROM dbo._import_1_3_inspolicytable ip
...
--NOCHECK CONSTRAINT dbo.Patient p
--UPDATE p SET 
--p.PrimaryProviderID= ''
----SELECT * 
--FROM dbo.Patient p
--WHERE p.PracticeID = 3 AND 
--p.PrimaryProviderID = 28