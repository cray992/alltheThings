...
FROM dbo._import_2_1_ReferringDoctors AS RP 
...
FROM dbo._import_2_1_patientdemos AS PD
...
WHERE PD.firstname <> '' AND PD.lastname <> '' AND pd.existingpatientid = '#N/A'
PRINT CAST(@@ROWCOUNT AS VARCHAR) + ' records inserted'	
...
FROM dbo._import_2_1_patientdemos pd
...
(	SELECT DISTINCT 
	...
	FROM dbo._import_2_1_patientdemos
...
PRINT 'Inserting Into Appointment - Existing Patients...'
	...
	SELECT  DISTINCT  PAT.PatientID , -- PatientID - int
	          ...
	          CASE pa.office
				...
				WHEN 'FMH' THEN (SELECT ServiceLocationID FROM dbo.ServiceLocation WHERE Name = 'Frisbie Memorial Hospital' AND PracticeID = @PracticeID)
				WHEN 'WDH' THEN (SELECT ServiceLocationID FROM dbo.ServiceLocation WHERE Name = 'Wentworth-Douglass Hospital ' AND PracticeID = @PracticeID)
			  ...
			  CASE 		
					...
					WHEN PA.appointmentstatus = 'Confirmed' THEN 'X'
					WHEN PA.appointmentstatus = 'Arrived' THEN 'I'
					WHEN PA.appointmentstatus = 'In Session' THEN 'I'
					WHEN PA.appointmentstatus = 'Complete' THEN 'O'
					WHEN PA.appointmentstatus = 'No Show' THEN 'N'
	...
	FROM dbo._import_2_1_Appointments PA
...
WHERE pa.existingpatient <> '#N/A' AND a.AppointmentID IS NULL
PRINT CAST(@@ROWCOUNT AS VARCHAR) + ' records inserted'	

PRINT ''
PRINT 'Inserting Into Appointment to Appointment Reason - Existing Patients...'
...
SELECT DISTINCT  
          ...
          CASE PA.apptprofile 
			WHEN '1 Occipital Nerve Block' THEN (SELECT AppointmentReasonId FROM dbo.AppointmentReason WHERE Name = '1 Occipital Block' AND PracticeID = @PracticeID) 
			WHEN '1 Trochanteric Bursa Injection' THEN (SELECT AppointmentReasonId FROM dbo.AppointmentReason WHERE Name = '1 Troch. Bursa Injections' AND PracticeID = @PracticeID) 
			WHEN '2 Occipital Nerve Blocks' THEN (SELECT AppointmentReasonId FROM dbo.AppointmentReason WHERE Name = '2 Occipital Blocks' AND PracticeID = @PracticeID )
			WHEN '2 Trochanteric Bursa Injections' THEN (SELECT AppointmentReasonId FROM dbo.AppointmentReason WHERE Name = '2 Troch. Bursa Injections' AND PracticeID = @PracticeID )
			WHEN 'Botox' THEN (SELECT AppointmentReasonId FROM dbo.AppointmentReason WHERE Name = 'BOTOX' AND PracticeID = @PracticeID )
			WHEN 'EEG' THEN (SELECT AppointmentReasonId FROM dbo.AppointmentReason WHERE Name = 'EEG Procedure' AND PracticeID = @PracticeID )
			WHEN 'EMG - One Limb' THEN (SELECT AppointmentReasonId FROM dbo.AppointmentReason WHERE Name = 'EMG Procedure 1-limb' AND PracticeID = @PracticeID )
			WHEN 'EMG - Two Limbs' THEN (SELECT AppointmentReasonId FROM dbo.AppointmentReason WHERE Name = 'EMG Procedure 2-limbs' AND PracticeID = @PracticeID )
			WHEN 'Follow-up Visit' THEN (SELECT AppointmentReasonId FROM dbo.AppointmentReason WHERE Name = 'Follow-up visit' AND PracticeID = @PracticeID )
			WHEN 'New Patient Visit' THEN (SELECT AppointmentReasonId FROM dbo.AppointmentReason WHERE Name = 'New Patient Visit' AND PracticeID = @PracticeID )
			WHEN 'Trigger Point Injections' THEN (SELECT AppointmentReasonId FROM dbo.AppointmentReason WHERE Name = 'Trigger Point Injection' AND PracticeID = @PracticeID )
		 END
...
WHERE PA.existingpatient <> '#N/A' AND pa.apptprofile <> '' AND app.CreatedDate > DATEADD(mi,-2,GETDATE())
PRINT CAST(@@ROWCOUNT AS VARCHAR) + ' records inserted'	
...
PRINT 'Inserting Into Appointmen to Resource - Existing Patients...'	
	...
	SELECT DISTINCT  APP.AppointmentID , -- AppointmentID - int
	          1 , -- AppointmentResourceTypeID - int
	          CASE  pa.provider 
				WHEN 'Shelley Landry, MPA-C' THEN (SELECT doctorid FROM dbo.Doctor WHERE FirstName = 'Shelley' AND LastName = 'Landry' AND [External] = 0 AND PracticeID = @PracticeID)
			    WHEN 'Kishori Somyreddy, MD' THEN (SELECT doctorid FROM dbo.Doctor WHERE FirstName = 'Kishori' AND LastName = 'Somyreddy' AND [External] = 0 AND PracticeID = @PracticeID)
	...
	INNER JOIN dbo.Appointment APP ON 
		...
		APP.StartDate = CAST(PA.StartDate AS DateTime) AND
		APP.EndDate = CAST(PA.EndDate AS Datetime)    
WHERE PA.existingpatient <> '#N/A' AND app.CreatedDate > DATEADD(mi,-2,GETDATE()) 
PRINT CAST(@@ROWCOUNT AS VARCHAR) + ' records inserted'	
...
PRINT ''
PRINT 'Inserting Into Appointment to Appointment Reason - New Patients...'
INSERT INTO dbo.AppointmentToAppointmentReason
        ( AppointmentID ,
          AppointmentReasonID ,
          PrimaryAppointment ,
