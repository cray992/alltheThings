...
FROM dbo.[_import_2_1_PatDemo] as id
where id.primaryCareFirstName <> '' AND id.primarycarelastname <> ''
AND NOT EXISTS (SELECT * FROM dbo.Doctor d WHERE id.primarycarefirstname = d.FirstName AND id.primarycarelastname = d.LastName)
...
FROM dbo.[_import_2_1_PatDemo] as pdp
LEFT JOIN dbo.Doctor AS doc ON
   pdp.primaryCareFirstName = doc.firstname AND
   pdp.primarycarelastname = doc.lastname 
where pdp.pid <> '' AND NOT EXISTS (SELECT * FROM dbo.Patient p WHERE p.FirstName = pdp.firstname AND p.LastName = pdp.lastname AND p.DOB
= DATEADD(hh,12,CAST(pdp.dateofbirth AS DATETIME)))
...
WHERE pd.policyid <> '' AND pd.insrank NOT IN ('','tertiary')
...
INNER JOIN dbo.Appointment AS dboa ON
	...
	dboa.CreatedDate > DATEADD(mi,-5,GETDATE())
