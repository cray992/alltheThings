...
-- Update existing patients with data from import file
UPDATE dbo.Patient
SET VendorID = i.chartnumber , 
	MobilePhone = LEFT(dbo.fn_RemoveNonNumericCharacters(i.cellphone),10) , 
	MobilePhoneExt = CASE
						WHEN LEN(dbo.fn_RemoveNonNumericCharacters(i.cellphone)) > 10 THEN LEFT(SUBSTRING(i.cellphone,11,LEN(i.cellphone)),10)
					 ELSE NULL END ,
	HomePhone = LEFT(dbo.fn_RemoveNonNumericCharacters(i.homephone),10) , 
	HomePhoneExt = 	CASE
	...
	EmergencyPhone = LEFT(dbo.fn_RemoveNonNumericCharacters(i.emergencyphone),10) ,

	ResponsibleDifferentThanPatient = CASE WHEN i.firstname <> i.responsiblepartyfirstname OR i.lastname <> i.responsiblepartylastname 
										   THEN 1 ELSE 0 
									  END ,
	ResponsibleFirstName = CASE WHEN i.firstname <> i.responsiblepartyfirstname OR i.lastname <> i.responsiblepartylastname THEN i.responsiblepartyfirstname END , 
	ResponsibleLastName = CASE WHEN i.firstname <> i.responsiblepartyfirstname OR i.lastname <> i.responsiblepartylastname THEN i.responsiblepartylastname END , 
	ResponsibleAddressLine1 = CASE WHEN i.firstname <> i.responsiblepartyfirstname OR i.lastname <> i.responsiblepartylastname THEN i.responsiblepartyaddress1 END , 
	ResponsibleAddressLine2 = CASE WHEN i.firstname <> i.responsiblepartyfirstname OR i.lastname <> i.responsiblepartylastname THEN i.responsiblepartyaddress2 END , 
	ResponsibleCity = CASE WHEN i.firstname <> i.responsiblepartyfirstname OR i.lastname <> i.responsiblepartylastname THEN i.responsiblepartycity END ,
	ResponsibleState = CASE WHEN i.firstname <> i.responsiblepartyfirstname OR i.lastname <> i.responsiblepartylastname THEN i.responsiblepartystate END , 
	ResponsibleZipCode = CASE WHEN i.firstname <> i.responsiblepartyfirstname OR i.lastname <> i.responsiblepartylastname 
								THEN CASE WHEN LEN(dbo.fn_RemoveNonNumericCharacters(i.zipcode)) IN (4,8) THEN '0' + dbo.fn_RemoveNonNumericCharacters(i.zipcode)
...
FROM dbo._import_1_1_PatientDemographics AS PD
	...
	LEFT JOIN dbo.Patient p ON PD.firstname = p.FirstName AND pd.lastname = p.LastName AND DATEADD(hh,12,CAST(pd.dateofbirth AS DATETIME)) = p.DOB AND p.PracticeID = @PracticeID
...
FROM dbo._import_1_1_PatientDemographics pd
	JOIN dbo.Patient AS p ON pd.firstname = p.FirstName AND pd.lastname = p.LastName AND DATEADD(hh,12,CAST(pd.dateofbirth AS DATETIME)) = p.DOB AND p.PracticeID = @PracticeID
PRINT CAST(@@ROWCOUNT AS VARCHAR) + ' records inserted'	
...
FROM #tempPolicies AS TP
	JOIN dbo.Patient AS PAT ON PAT.PracticeID = @PracticeID AND VendorID = PatientVendorID
