USE superbill_23696_dev
GO

--BEGIN TRANSACTION
UPDATE dp 
SET dp.DOB = rp.DOB
FROM superbill_23696_restore.dbo.patient rp 
INNER JOIN superbill_23696_dev.dbo.Patient dp
...
WHERE p.PracticeID = @targetPracticeID AND 
	  p.Active = 1
...
SELECT DISTINCT	
    @targetPracticeID,         -- PracticeID - int
    ip.doctorlastname,        -- Name - varchar(128)
    DATEDIFF(MINUTE,CAST(ip.startdate AS DATETIME),CAST(ip.enddate AS DATETIME)),         -- DefaultDurationMinutes - int
    null,      -- DefaultColorCode - int
    ip.doctorlastname,        -- Description - varchar(256)
    GETDATE(), -- ModifiedDate - datetime
	@VendorImportID
	--SELECT * 
...
@VendorImportID
--select * 
FROM _import_1_1_PatientAppointments iapt
...
WHERE a.CreatedDate > DATEADD(mi,-3,GETDATE()) AND iapt.AutoTempID <> 1
