--USE superbill_52827_dev
USE superbill_52827_prod
GO
...
--			 i.ssn  -- SSN - varchar(9)
--FROM dbo.Patient p 
--INNER JOIN dbo.[_import_2_1_PatientDemographics] i ON p.PatientID = REPLACE(REPLACE(i.id,'.00',''),',','')
...
FROM dbo.[_import_1_1_DrHayesPatientDemographic] 
...
FROM dbo.[_import_1_1_DrHayesPatientDemographic] i 
WHERE NOT EXISTS 
...
SELECT DISTINCT
          ...
          CASE i.defaultrenderingproviderfullname 
			WHEN 'Angela  Brown, NP' THEN (SELECT DoctorID FROM dbo.Doctor WHERE FirstName = 'Angela' AND LastName = 'Brown' AND [External] = 0 AND ActiveDoctor = 1 AND PracticeID = @PracticeID)
			WHEN 'FREDDIE L HAYES, MD' THEN (SELECT DoctorID FROM dbo.Doctor WHERE FirstName = 'Freddie' AND LastName = 'Hayes' AND [External] = 0 AND ActiveDoctor = 1 AND PracticeID = @PracticeID)
		  ELSE NULL END  , -- PrimaryProviderID - int
          CASE i.servicelocationname 
			WHEN 'Freddie L. Hayes, M.D.' THEN (SELECT ServiceLocationID FROM dbo.ServiceLocation WHERE Name = 'Freddie L Hayes MD' AND PracticeID = @PracticeID) 
          ...
          CASE i.primarycarephysicianfullname
			WHEN 'Angela  Brown, NP' THEN (SELECT MAX(DoctorID) FROM dbo.Doctor WHERE FirstName = 'Angela' AND LastName = 'Brown' AND PracticeID = @PracticeID)  
			WHEN 'FREDDIE L HAYES, MD' THEN (SELECT MAX(DoctorID) FROM dbo.Doctor WHERE FirstName = 'Freddie' AND LastName = 'Hayes' AND PracticeID = @PracticeID)  
