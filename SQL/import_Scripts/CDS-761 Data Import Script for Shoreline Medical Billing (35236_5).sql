...
--			 i.ssn  -- SSN - varchar(9)
--FROM dbo.Patient p 
--INNER JOIN dbo.[_import_15_5_PatientDemographics] i ON p.PatientID = REPLACE(REPLACE(i.ID,',',''),'.00','') 
...
FROM dbo.[_import_14_5_PatientDemographics] i 
INNER JOIN dbo.Patient p ON 
	i.firstname = p.FirstName AND 
	i.lastname = p.LastName AND 
	DATEADD(dd,1462,DATEADD(hh,12,CAST(i.dateofbirth AS DATETIME))) = p.DOB
...
CREATE TABLE #tempins (insuranceid INT)
INSERT INTO #tempins
        ( insuranceid )
SELECT DISTINCT  insurancecode1 
FROM dbo.[_import_14_5_PatientDemographics] i
LEFT JOIN dbo.Patient p ON 
	i.firstname = p.FirstName AND 
	i.lastname = p.LastName AND 
	DATEADD(dd,1462,DATEADD(hh,12,CAST(i.dateofbirth AS DATETIME))) = p.DOB
WHERE insurancecode1 <> '' AND p.PatientID IS NULL

INSERT INTO #tempins
        ( insuranceid )
SELECT DISTINCT  insurancecode2
FROM dbo.[_import_14_5_PatientDemographics] i 
LEFT JOIN dbo.Patient p ON 
	i.firstname = p.FirstName AND 
	i.lastname = p.LastName AND 
	DATEADD(dd,1462,DATEADD(hh,12,CAST(i.dateofbirth AS DATETIME))) = p.DOB
WHERE NOT EXISTS (SELECT * FROM #tempins ti WHERE i.insurancecode2 = ti.insuranceid) AND
	  i.insurancecode2 <> '' AND p.PatientID IS NULL
...
SELECT DISTINCT
	...
	0, -- Copay - money
	0, -- Deductible - money
	icp.insuranceid,  -- VendorID - varchar(50) --FIX
	@VendorImportID  -- VendorImportID - int
FROM dbo._import_14_5_InsuranceCOMPANYPLANList ICP 
...
INNER JOIN #tempins ti ON 
	ICP.insuranceid = ti.insuranceid

PRINT CAST(@@ROWCOUNT AS VARCHAR) + ' records inserted'	
/*
==========================================================================================================================================
CREATE INSURANCE COMPANIES FROM [_import_14_5_InsuranceCOMPANYPLANList]
==========================================================================================================================================
*/	
...
FROM dbo._import_14_5_InsuranceCOMPANYPLANList AS IICL
INNER JOIN #tempins ti ON 
	IICL.insuranceid = ti.insuranceid
...
SELECT DISTINCT
	...
	CASE
		WHEN ISDATE(PD.dateofbirth) = 1 THEN DATEADD(dd,1462,PD.dateofbirth)
...
FROM dbo._import_14_5_PatientDemographics AS PD
	...
	LEFT JOIN dbo.Patient p ON 
			pd.firstname = p.FirstName AND
            pd.lastname = p.LastName AND
            DATEADD(dd,1462,DATEADD(hh,12,CAST(pd.dateofbirth AS DATETIME))) = p.DOB
...
FROM dbo._import_14_5_PatientDemographics pd
...
(	SELECT DISTINCT 
	...
	FROM dbo._import_14_5_PatientDemographics
...
PRINT 'Inserting Into Patient Journal Note...'
	...
	SELECT    CASE WHEN PJN.CreatedDate IS NULL THEN GETDATE() ELSE PJN.CreatedDate END , -- CreatedDate - datetime
	          ...
	          1 , -- NoteTypeCode - int
	          0  -- LastNote - bit
	FROM dbo.[_import_14_5_PatientJournalNote] PJN
