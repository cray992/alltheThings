--06012006 kdev01_0218 HCPlus - New Coastal EMS import script (test) - includes Facilities imported as referring physicians; removes patient records with multiple guarantors
-- PracticeID = 31
-- VendorImportID = ? -- 
...
INSERT INTO VendorImport (VendorName, Notes, VendorFormat) VALUES ('Coastal EMS', 'HCPlus Import - Patients, Ref.Ph., Facil. & Ins.', 'MediSoft')
...
FROM Doctor D INNER JOIN Coastal_mwrph a
...
SELECT chartnumber, casenumber from coastal_mwcas
WHERE chartnumber in (
SELECT chartnumber
FROM coastal_mwcas
GROUP BY chartnumber				-- 3604
HAVING count(casenumber) > 10		-- 165
)
...
-- case# 8327  to 8550 for 00000282
