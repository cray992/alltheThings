USE superbill_55389_dev
--USE superbill_55389_prod
GO
...
SELECT DISTINCT
          ...
          EIN ,
          BillTypeID 
FROM dbo._import_1_1_ServiceLocation
WHERE Name <> 'ZZ'
PRINT CAST(@@ROWCOUNT AS VARCHAR) + ' records inserted'
...
PRINT ''
PRINT 'Inserting Into Payer Scenario...'
...
SELECT DISTINCT
		  ps.PayerScenarioID ,
	      ps.Name , -- Name - varchar(128)
          ps.[Description] , -- Description - varchar(256)
          ps.PayerScenarioTypeID , -- PayerScenarioTypeID - int
          ps.StatementActive  -- StatementActive - bit
FROM dbo._import_1_1_PayerScenario ps
LEFT JOIN dbo.PayerScenario sps ON 
	ps.PayerScenarioID = sps.PayerScenarioID
WHERE sps.PayerScenarioID IS NULL
PRINT CAST(@@ROWCOUNT AS VARCHAR) + ' records inserted'
