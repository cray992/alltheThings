-- 030606 k0_0122 DCC/National Billing import script
-- Modified 02232006 to remove 'None', 'NONE', etc from groupnumber field
-- Modified 02242006 to insert NULL when policy start and end dates and patient DOB are '' to avoid inserting '1900/01/01'
-- National Billing - superbill_0122
--select top 10 * from DCC_MWPAT
-- PracticeID = 9
-- VendorImportID = ?
...
ALTER TABLE InsurancePolicy
	ALTER COLUMN VendorID VARCHAR(50) NULL
GO
*/
--select * from doctor where practiceid = 9
BEGIN TRANSACTION

DECLARE @PracticeID int
DECLARE @VendorImportID int

INSERT INTO VendorImport (VendorName, Notes, VendorFormat) VALUES ('Doctors Care Center', 'Doctors Care Center Import - Patients, Doctors and Insurance data', 'MediSoft')

SET @VendorImportID = @@IDENTITY
SET @PracticeID = 9 -- DOCTORS CARE CENTER
...
SELECT @PracticeID, '', 
	...
	0
FROM DCC_MWPHY
WHERE (LTRIM(RTRIM(Code)) <> '1' AND LTRIM(RTRIM(Code)) <> '2') -- Exclude Dr. Werthaim because he already exists with DoctorID = 14
ORDER BY Code
...
SELECT @PracticeID, '', 
	...
	@VendorImportID, 1
FROM DCC_MWRPH
--WHERE LTRIM(RTRIM(Code)) <> 'AVI' -- Do not exclude Dr. Werthaim (appears once in the Doctor table with External=1 and ID=14
ORDER BY Code

--Eliminate Duplicate Patient Records
CREATE TABLE #Dups(SocialSecurityNumber VARCHAR(50))
INSERT INTO #Dups(SocialSecurityNumber)
SELECT SocialSecurityNumber 
FROM dcc_mwpat
WHERE SocialSecurityNumber NOT IN ('','?','999999999','NA')
GROUP BY SocialSecurityNumber
HAVING COUNT(chartnumber)>1

CREATE TABLE #MaxCase(ChartNumber VARCHAR(50), CaseNumber INT)
INSERT INTO #MaxCase(ChartNumber, CaseNumber)
SELECT ChartNumber, MAX(CAST(CaseNumber AS INT)) CaseNumber
FROM DCC_MWCAS
GROUP BY ChartNumber

CREATE TABLE #DupsMaxCase(SocialSecurityNumber VARCHAR(50), CaseNumber INT)
INSERT INTO #DupsMaxCase(SocialSecurityNumber, CaseNumber)
SELECT D.SocialSecurityNumber, MAX(MC.CaseNumber) CaseNumber
FROM #Dups D INNER JOIN DCC_MWPAT P
ON D.SocialSecurityNumber=P.SocialSecurityNumber
INNER JOIN #MaxCase MC
ON P.ChartNumber=MC.ChartNumber
GROUP BY D.SocialSecurityNumber

--Use this table to exclude inserts of duplicates
CREATE TABLE #ChartsToIgnore(ChartNumber VARCHAR(50))
INSERT INTO #ChartsToIgnore(ChartNumber)
SELECT MC.ChartNumber
FROM #Dups D INNER JOIN DCC_MWPAT P
ON D.SocialSecurityNumber=P.SocialSecurityNumber
INNER JOIN #MaxCase MC
ON P.ChartNumber=MC.ChartNumber
LEFT JOIN #DupsMaxCase DMC
ON P.SocialSecurityNumber=DMC.SocialSecurityNumber AND MC.CaseNumber=DMC.CaseNumber
WHERE DMC.CaseNumber IS NULL
...
FROM DCC_MWPAT pat
INNER JOIN DCC_MWCAS cas ON cas.Guarantor = pat.ChartNumber
...
LEFT JOIN #ChartsToIgnore CTI ON pat.ChartNumber=CTI.ChartNumber
WHERE UPPER(LTRIM(RTRIM(pat.PatientType))) <> 'Guarantor' AND CTI.ChartNumber IS NULL
...
FROM Patient P INNER JOIN DCC_MWCAS DC
...
FROM DCC_MWCAS DC INNER JOIN PatientCase PC
...
DROP TABLE #Dups
DROP TABLE #MaxCase
DROP TABLE #DupsMaxCase
DROP TABLE #ChartsToIgnore
