USE superbill_25516_dev
--USE superbill_25516_prod
GO
...
SET ReviewCode = 'R'
FROM dbo.[_import_1_3_InsurancePolicy] i
...
PRINT ''
PRINT 'Updating Existing Renderring Provider Records with VendorID...'
UPDATE dbo.Doctor 
	SET VendorID = i.providerid
FROM dbo.Doctor d
INNER JOIN dbo.[_import_1_3_Doctor] i ON
	d.FirstName = i.firstname AND
	d.LastName = i.lastname AND
	d.NPI = i.npi 
WHERE d.PracticeID = @PracticeID AND d.[External] = 0 AND i.providertype = 'Rendering'
PRINT CAST (@@ROWCOUNT AS VARCHAR(10)) + ' records updated '

--PRINT ''
--PRINT 'Updating Existing Referring Provider Records with VendorID...'
--UPDATE dbo.Doctor 
--	SET VendorID = i.providerid
--FROM dbo.Doctor d
--INNER JOIN dbo.[_import_1_3_Doctor] i ON
...
--	d.LastName = i.lastname AND
--	d.NPI = i.npi 
--WHERE d.PracticeID = @PracticeID AND d.[External] = 1 AND i.providertype = 'Referring'
--PRINT CAST (@@ROWCOUNT AS VARCHAR(10)) + ' records updated '
...
FROM dbo.[_import_1_3_Doctor] i WHERE providertype = 'Referring' 
								 AND NOT EXISTS
...
SELECT DISTINCT
          ...
          6 , -- DefaultServiceLocationID - int
         -- CASE WHEN i.employerid <> '' THEN i.employerid END , -- EmployerID - int
		  ...
		  pcp.DoctorID
FROM dbo.[_import_1_3_Patient] i
...
LEFT JOIN dbo.Doctor drp ON
	...
	drp.[External] = 0 AND
	drp.VendorID <> ''
LEFT JOIN dbo.[_import_1_3_PatientReferralSource] iprs ON
	i.patientreferralsourceid = iprs.PatientReferralSourceID 
LEFT JOIN dbo.[_import_1_3_PatientReferralSource] prs ON	
	iprs.[description] = prs.[description] AND
...
FROM dbo.[_import_1_3_PatientAlert] i
...
FROM dbo.[_import_1_3_PatientJournalNote] AS pjn 
...
FROM dbo.[_import_1_3_PatientCase] as patcase
...
SELECT DISTINCT
          ...
          CASE WHEN iip.CardOnFile = '-1' THEN 1 ELSE iip.cardonfile END , -- CardOnFile - bit
          ...
          CASE WHEN iip.PatientInsuranceStatusID = '-1' THEN 1 ELSE iip.patientinsurancenumber END , -- PatientInsuranceStatusID - int
          GETDATE() , -- CreatedDate - datetime
          -50 , -- CreatedUserID - int
          GETDATE() , -- ModifiedDate - datetime
          -50 , -- ModifiedUserID - int
...
FROM dbo.[_import_1_3_InsurancePolicy] AS iip
...
SELECT DISTINCT
          ...
          @VendorImportID  -- VendorImportID - int
FROM dbo.[_import_1_3_InsurancePolicyAuthorization] i
