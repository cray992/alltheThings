...
DECLARE @Practice1VendorImportID INT

SET @PracticeID1 = 1
SET @Practice1VendorImportID = 1


PRINT 'PracticeID = ' + CAST(@PracticeID1 AS VARCHAR(10))
PRINT 'VendorImportID = ' + CAST(@Practice1VendorImportID AS VARCHAR(10))


--DELETE FROM dbo.InsurancePolicy WHERE PracticeID = @PracticeID1 AND VendorImportID = @Practice1VendorImportID
--PRINT '    ' + CAST(@@ROWCOUNT AS VARCHAR(10)) + ' Insurance Policy records deleted'
--DELETE FROM dbo.InsuranceCompanyPlan WHERE VendorImportID = @Practice1VendorImportID
--PRINT '    ' + CAST(@@ROWCOUNT AS VARCHAR(10)) + ' Insurance Company Plan records deleted'
--DELETE FROM dbo.InsuranceCompany WHERE VendorImportID = @Practice1VendorImportID
--PRINT '    ' + CAST(@@ROWCOUNT AS VARCHAR(10)) + ' Insurance Company records deleted'
--DELETE FROM dbo.PatientCaseDate WHERE PatientCaseID IN (SELECT PatientCaseID FROM dbo.PatientCase WHERE PracticeID = @PracticeID1 AND VendorImportID = @Practice1VendorImportID)
--PRINT '    ' + CAST(@@ROWCOUNT AS VARCHAR(10)) + ' Patient Case Date records deleted'
--DELETE FROM dbo.PatientCase WHERE PracticeID = @PracticeID1 AND VendorImportID = @Practice1VendorImportID
--PRINT '    ' + CAST(@@ROWCOUNT AS VARCHAR(10)) + ' Patient Case records deleted'
--DELETE FROM dbo.PatientAlert WHERE PatientID IN (SELECT PatientID FROM dbo.Patient WHERE PracticeID = @PracticeID1 AND VendorImportID = @Practice1VendorImportID)
--PRINT '    ' + CAST(@@ROWCOUNT AS VARCHAR(10)) + ' Patient Alert records deleted'
--DELETE FROM dbo.Patient WHERE PracticeID = @PracticeID1 AND VendorImportID = @Practice1VendorImportID
--PRINT '    ' + CAST(@@ROWCOUNT AS VARCHAR(10)) + ' Patient records deleted'
--DELETE FROM dbo.Doctor WHERE PracticeID = @PracticeID1 AND VendorImportID = @Practice1VendorImportID
--PRINT '    ' + CAST(@@ROWCOUNT AS VARCHAR(10)) + ' Doctor records deleted'



PRINT ''
PRINT 'Updating Existing Insurance Company Records with VendorID...'
UPDATE dbo.InsuranceCompany 
	SET VendorID = i.insurancecode , 
		ModifiedUserID = -50 ,
		ModifiedDate = GETDATE()
FROM dbo.InsuranceCompany ic
INNER JOIN dbo.[_import_1_1_InsuranceMaster] i ON
	ic.InsuranceCompanyName = insurancename
WHERE ic.CreatedPracticeID = @PracticeID1
...
SELECT DISTINCT
		 ...
		 (CASE WHEN i.insurancetelephonerelations = '' THEN '' ELSE 'Phone Relations: ' + i.insurancetelephonerelations END) , -- Notes - text
          ...
          i.insurancephone , -- Phone - varchar(10)
          i.insurancetelephonefax , -- Fax - varchar(10)
          13 , -- BillingFormID - int
          'CI' , -- InsuranceProgramCode - char(2)
          'C' , -- HCFADiagnosisReferenceFormatCode - char(1)
          'D' , -- HCFASameAsInsuredFormatCode - char(1)
          @PracticeID1 , -- CreatedPracticeID - int
          ...
          @Practice1VendorImportID , -- VendorImportID - int
          1 , -- NDCFormat - int
          1 , -- UseFacilityID - bit
          'U' , -- AnesthesiaType - varchar(1)
          18  -- InstitutionalBillingFormID - int
FROM dbo.[_import_1_1_InsuranceMaster] i
WHERE NOT EXISTS (SELECT VendorID FROM dbo.InsuranceCompany ic WHERE i.insurancecode = ic.VendorID)
PRINT CAST(@@ROWCOUNT AS VARCHAR) + ' records inserted'
...
FROM dbo.InsuranceCompany ic
	INNER JOIN dbo.[_import_1_1_InsuranceMaster] i ON
		ic.VendorID = i.insurancecode AND
		ic.CreatedPracticeID = @PracticeID1
WHERE VendorImportID = @Practice1VendorImportID OR ic.VendorID = i.insurancecode
PRINT CAST(@@ROWCOUNT AS VARCHAR) + ' records inserted'
...
FROM dbo.[_import_1_1_ReferringMaster]
...
FROM dbo.[_import_1_1_JohnBiordi1] i
	LEFT JOIN dbo.Doctor rd ON
		i.patientreferringcode = rd.VendorID AND
		rd.VendorImportID = @Practice1VendorImportID
...
FROM dbo.[_import_1_1_JohnBiordi1] i
	INNER JOIN dbo.Patient p ON
		...
		p.VendorImportID = @Practice1VendorImportID
...
FROM dbo.Patient 
WHERE VendorImportID = @Practice1VendorImportID
...
SELECT DISTINCT
          ...
          CASE WHEN ISDATE(i.policystartdate) = 1 THEN i.policystartdate ELSE NULL END , -- PolicyStartDate - datetime
          CASE WHEN ISDATE(i.policyenddate) = 1 THEN i.policyenddate ELSE NULL END , -- PolicyEndDate - datetime
...
FROM dbo.[_import_1_1_PolicyMaster] i
	INNER JOIN dbo.PatientCase pc ON
		i.policyaccountnumber = pc.VendorID AND
		pc.VendorImportID = @Practice1VendorImportID
	INNER JOIN dbo.InsuranceCompanyPlan icp ON
		i.policyinsurancecode = icp.VendorID AND
		icp.CreatedPracticeID = @PracticeID1
...
UPDATE dbo.PatientCase 
      ...
      WHERE pc.VendorImportID = @Practice1VendorImportID AND
