...
SELECT DISTINCT
          icpl.insurancename, -- InsuranceCompanyName - varchar(128)
      /*  '' , -- Notes - text
     ...
     --   '' , -- LocalUseFieldTypeCode - char(5)
      ...
      /*  0 , -- KareoInsuranceCompanyID - int
          ...
          icpl.insuranceid, -- VendorID - varchar(50)
...
FROM  [dbo].[_import_2_5_InsuranceCOMPANYPLANList] as icpl
WHERE icpl.insurancename<>''  
...
SELECT DISTINCT 
        ...
        --  doc.DoctorID , -- ReferringPhysicianID - int
          ...
          '' , -- Country - varchar(32)  
		  CASE WHEN LEN(pd.zipcode) = 4 THEN '0' + pd.zipcode ELSE 
		  LEFT(dbo.fn_RemoveNonNumericCharacters(REPLACE(pd.zipcode,'-','')),9)  END, -- ZipCode - varchar(9)
      ...
      --  '' , -- WorkPhoneExt - varchar(10)
          ...
          CASE WHEN LEN(pd.socialsecuritynumber) >= 6 THEN RIGHT('000' + pd.socialsecuritynumber, 9) ELSE '' END , -- SSN - char(9)
      ...
      --  0 , -- CollectionCategoryID - int
          CASE pd.activestatusyesorno WHEN 'Y' THEN 1 ELSE 0 END  -- Active - bit
...
FROM [dbo].[_import_2_5_PatientDemographics] as pd  
where pd.chartnumber <>''
...
SELECT 
      ...
      --  NULL , -- timestamp - timestamp
          ...
          'K' , -- SoftwareApplicationID - char(1)
           0 , -- Hidden - bit
          pd.patientnote  -- NoteMessage - varchar(max)  
...
SELECT DISTINCT
          pat.patientID , -- PatientID - int
          pd.patientalertmessage , -- AlertMessage - text
...
SELECT  DISTINCT
          pat.PatientID , -- PatientID - int
          CASE WHEN pat.PatientID IS NOT NULL THEN i.casename ELSE 'Default Case' END  , -- Name - varchar(128)
          1 , -- Active - bit
...
LEFT JOIN dbo.[_import_2_5_InsurancePolicies] i ON
	pat.VendorID = i.chartnumber AND 
	pat.VendorImportID = @VendorImportID
WHERE pat.PracticeID = @PracticeID AND pat.VendorImportID = @VendorImportID
...
SELECT DISTINCT 
          ...
          pd.patientcasedatelastseen , -- StartDate - datetime  
...
INNER JOIN dbo.patientcase as patcase ON
        patcase.vendorID = pd.chartnumber AND
...
SELECT DISTINCT
     ...
     --     '' , -- PolicyStartDate - datetime 
          CASE WHEN ISDATE(ip.policy1enddate) = 1 THEN ip.policy1enddate ELSE NULL END , -- PolicyEndDate - datetime
      --  NULL , -- CardOnFile - bit
     ...
     --     ip.casename , -- Notes - text     **********************casename **********************
      --  '' , -- Phone - varchar(10)
      --  '' , -- PhoneExt - varchar(10)
      --  '' , -- Fax - varchar(10)
      --  '' , -- FaxExt - varchar(10)
          ip.Policy1Copay , -- Copay - money
      --  '' , -- Deductible - money
      --  '' , -- PatientInsuranceNumber - varchar(32)
          CASE ip.planactive WHEN 'Y' THEN 1 ELSE 0 END, -- Active - bit
          @PracticeID , -- PracticeID - int
...
FROM [dbo].[_import_2_5_InsurancePolicies] as ip 
INNER JOIN dbo.patientcase as patcase ON
        patcase.VendorID = ip.chartnumber  AND
        patcase.VendorImportID = @VendorImportID 
INNER JOIN dbo.insurancecompanyplan as inscoplan ON
        inscoplan.VendorID = ip.insuranceid  AND
		inscoplan.VendorImportID = @VendorImportID
WHERE ip.policynumber1 <>'' AND ip.policynumber1<>'0'    
