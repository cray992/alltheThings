...
-- ReferringPhysician		Note: did not consider that some of them may already exist -- Is IT NECESSARY?
-- MNR_refffile


INSERT INTO ReferringPhysician (PracticeID, Prefix, FirstName, MiddleName, LastName, Suffix, UPIN, 
WorkPhone, AddressLine1, AddressLine2, City, State, ZipCode, MM_ref_dr_no, VendorImportID)
SELECT @PracticeID, '', 
...
LEN(CASE WHEN MaritalStatus IS NOT NULL AND LEN(LTRIM(RTRIM(MaritalStatus))) < 2 THEN LTRIM(RTRIM(MaritalStatus)) ELSE 'U' END) marita, 
LEN(CASE WHEN HomePhone IS NOT NULL THEN LTRIM(RTRIM(HomePhone)) ELSE NULL END) homephon, 
LEN(CASE WHEN WorkPhone IS NOT NULL THEN LTRIM(RTRIM(WorkPhone)) ELSE NULL END) workphone, 
LEN(CASE WHEN DOB IS NOT NULL AND LEN(DOB) = 8 THEN CAST(LTRIM(RTRIM(DOB)) AS DATETIME) ELSE NULL END) dob
FROM MNR_patients mp
WHERE mp.SSN NOT IN (SELECT p.SSN from Patient p WHERE p.PracticeID = 63)
and LEN(CASE WHEN MaritalStatus IS NOT NULL AND LEN(LTRIM(RTRIM(MaritalStatus))) < 2 THEN LTRIM(RTRIM( MaritalStatus)) ELSE NULL END) > 1
*/
...
CASE WHEN AddressLine2 IS NULL THEN '' ELSE LTRIM(RTRIM(AddressLine2)) END, 
CASE WHEN City IS NOT NULL THEN LTRIM(RTRIM(City)) ELSE NULL END, 
CASE WHEN State IS NOT NULL THEN LTRIM(RTRIM(State)) ELSE NULL END, 
CASE WHEN ZipCode IS NOT NULL THEN LTRIM(RTRIM(ZipCode)) ELSE NULL END, 
CASE WHEN Gender IS NOT NULL THEN LTRIM(RTRIM(Gender)) ELSE NULL END, 
CASE WHEN MaritalStatus IS NOT NULL THEN LTRIM(RTRIM(MaritalStatus)) ELSE NULL END, 
CASE WHEN HomePhone IS NOT NULL THEN LTRIM(RTRIM(HomePhone)) ELSE NULL END, 
CASE WHEN WorkPhone IS NOT NULL THEN LTRIM(RTRIM(WorkPhone)) ELSE NULL END, 
CASE WHEN DOB IS NOT NULL AND LEN(DOB) = 8 THEN CAST(LTRIM(RTRIM(DOB)) AS DATETIME) ELSE NULL END, 
CASE WHEN SSN IS NOT NULL THEN (LTRIM(RTRIM(SSN))) ELSE NULL END, 
...
--CREATE TABLE #IC(ICID INT IDENTITY(1,1), InsuranceCompanyCode varchar(50), InsuranceCompanyName varchar(100)
--INSERT INTO #IC(InsuranceCompanyCode, InsuranceCompanyName)
...
where icp.ZipCode = ip.ZipCode
and icp.PlanName = ip.PlanName
and icp.AddressLine1 = ip.AddressLine1 order by ip.MM_CompanyID

--select count(0) cnt,len(zipcode) from MNR_insplan GROUP BY len(zipcode) order by cnt desc
--select * from mnr_insplan where len(zipcode) = 10
...
-- select InsuranceCompanyPlanID from #IP where MM_CompanyID, ZipCode, PlanName and AddressLine1 match
-- select distinct InsuranceCompanyPlanID from #IP ip join MNR_insplan mp on mp.InsurancePlanNo = ip.InsurancePlanNo 
...
-- for new patients create new 'CASE 1', for existing: add new cases
-- count PatientCaseID for each existing PatientID having PracticeID = 63 in PatientCase
...
AND VendorImportID IS NOT NULL
AND PracticeID = 63
...
BEGIN
	...
	BEGIN
		...
		BEGIN
			SET @Ins1CompanyPlanID = (SELECT DISTINCT InsuranceCompanyPlanID FROM #IP ip join MNR_insplan mp on mp.InsurancePlanNo = ip.InsurancePlanNo
			 			 WHERE ip.InsurancePlanNo = @InsVendorID1)
		END

	SELECT @Ins1Policy=InsurancePolicyNumber,		-- declare these variables above
	...
	IF @InsuredParty1No IS NOT NULL	-- set HolderDirrerentThanPatient to 1, PatientRelationshipToInsured to 'U', add insured party information			
	...
	@InsuredParty1LastName,										-- TODO: CHECK FOR NULL VALUES
	...
	IF @InsVendorID2<>''
	BEGIN
		-- TODO: REPEAT AS ABOVE, USE PRECEDENCE = 2, change numbers in variables to 2
		...
		BEGIN
			SET @Ins2CompanyPlanID = (SELECT DISTINCT InsuranceCompanyPlanID FROM #IP ip JOIN MNR_insplan mp ON mp.InsurancePlanNo = ip.InsurancePlanNo
						 WHERE ip.InsurancePlanNo = @InsVendorID2)
		END
	...
	@InsuredParty2LastName,											-- TODO: CHECK FOR NULL VALUES
	...
	IF @InsVendorID3<>''
	BEGIN
		-- TODO: REPEAT AS ABOVE, USE PRECEDENCE = 3, change numbers in variables to 3
		...
		BEGIN
			SET @Ins3CompanyPlanID = (SELECT DISTINCT InsuranceCompanyPlanID FROM #IP ip JOIN MNR_insplan mp ON mp.InsurancePlanNo = ip.InsurancePlanNo
						 WHERE ip.InsurancePlanNo = @InsVendorID3)
		END
...
SELECT * FROM PATIENT WHERE VENDORID='343' -- PATIENTID=332024--'5161' -- 329544
SELECT * FROM PATIENTCASE WHERE PATIENTID= 332024 --329544 --PATIENTCASEID = 112603
SELECT * FROM INSURANCEPOLICY WHERE PATIENTCASEID = 116083
--113603
SELECT * FROM REFERRINGPHYSICIAN WHERE REFERRINGPHYSICIANID=21678
