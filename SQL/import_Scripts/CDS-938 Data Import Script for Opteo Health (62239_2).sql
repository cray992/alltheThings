...
--INNER JOIN dbo._import_1_2_PatientDemographics p ON
--	p.id = i.patientid
...
FROM dbo.[_import_1_2_Doctor] i WHERE i.PracticeID = @SourcePracticeID AND i.[External] = 1 AND
...
FROM dbo.[_import_1_2_CollectionCategory] i 
...
SELECT DISTINCT
          ...
          GETDATE() , -- ModifiedDate - datetime
          0  -- ModifiedUserID - int
FROM dbo.[_import_1_2_Employers] i
	INNER JOIN dbo.[_import_1_2_Patient] p ON
...
FROM dbo.[_import_1_2_Employers] e 
...
FROM dbo.[_import_1_2_PatientAlert] pa
...
SELECT DISTINCT
          ...
          pjn.NoteTypeCode ,
          pjn.lastnote 
FROM dbo.[_import_1_2_PatientJournalNote] pjn
	INNER JOIN dbo.Patient p ON
...
FROM dbo._import_1_2_PayerScenario i
	LEFT JOIN dbo.PayerScenario ps ON 
		i.Name = ps.Name 
	INNER JOIN dbo._import_1_2_PatientCase pc ON 
		i.PayerScenarioID = pc.PayerScenarioID AND
		pc.PracticeID = @SourcePracticeID
WHERE ps.PayerScenarioID IS NULL
PRINT CAST(@@ROWCOUNT AS VARCHAR) + ' records inserted'
...
FROM dbo.[_import_1_2_PatientCase] pc
	...
	LEFT JOIN dbo._import_1_2_PayerScenario ps ON 
