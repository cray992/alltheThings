...
AND FileReceiveDate BETWEEN ''''11/1/2017'''' and ''''3/31/2018''''
AND C.PaymentID IS NULL
--AND C.SourceAddress LIKE ''''%proxymed%''''

INSERT INTO Sharedserver.DataCollection.dbo.ERA90Days3
...
---Run for KMB ñ Updated all payerids
PRINT @CurrentDB
...
--FROM dbo.era90days3 ch
--Inner join superbill_shared.dbo.customer c on ch.customerid=c.customerid
--INNER JOIN [DataCollection].[dbo].['90day_era_list$'] b ON b.[Kareo ID] = ch.CustomerID
--INNER JOIN SHAREDSERVER.DataCollection.dbo.ERA90DAyCustomers dc ON c.CustomerID=dc.[Kareo ID] and dc.[practice id]=ch.practiceid
