...
DELETE FROM dbo.ContractToInsurancePlan WHERE ContractID IN (SELECT ContractID FROM 
...
SELECT    @PracticeID , -- PracticeID - int
          ...
          REPLACE(SUBSTRING(ref.lastName, 0, (LEN(ref.lastName) -3)), ',', '') , -- LastName - varchar(64)
          '' , -- Suffix - varchar(16)
          1 , -- ActiveDoctor - bit
          GETDATE() , -- CreatedDate - datetime
          0 , -- CreatedUserID - int
          GETDATE() , -- ModifiedDate - datetime
          0 , -- ModifiedUserID - int
          REPLACE(RIGHT(ref.lastName, 3), ' ', '') , -- Degree - varchar(8)
          ref.referralID , -- VendorID - varchar(50)
          @VendorImportID , -- VendorImportID - int
          1  -- External - bit
FROM dbo._import_2_18_referral ref
...
SELECT DISTINCT
	...
	,CASE WHEN pat.referralid <> '' THEN (SELECT DoctorID FROM dbo.Doctor WHERE
		VendorID = referralid AND PracticeID = @PracticeID) ELSE NULL END 
	...
	,pat.PatientID -- VendorID 
	,@VendorImportID
	,1
	,1
	,CASE WHEN emailAddress <> '' THEN 1 ELSE 0 END
	,1
FROM dbo.[_import_2_18_Patient] pat
...
SELECT
	...
	,@VendorImportID
FROM dbo.[_import_2_18_Coverage] cov
LEFT JOIN dbo.Patient realP ON
	cov.patientid = realP.VendorID
	AND realP.VendorImportID = @VendorImportID
LEFT JOIN dbo.[_import_2_18_Policy] pol ON
	cov.policyid = pol.policyid
LEFT JOIN dbo.InsuranceCompany ic ON
	pol.carrierid = ic.VendorID 
...
FROM dbo._import_2_18_PatientNotes pjn
LEFT JOIN dbo.Patient realP ON
	pjn.patientnotesid = realP.VendorID AND
...
SELECT DISTINCT
	...
	,pol.copayment
	,CASE WHEN pol.holderid <> LEFT(pc.VendorID, 6) THEN 'O'
		ELSE 'S' END
	,CASE WHEN pol.holderid <> LEFT(pc.VendorID, 6) THEN holder.FirstName ELSE '' end
	,CASE WHEN pol.holderid <> LEFT(pc.VendorID, 6) THEN holder.LastName ELSE '' END
	,CASE WHEN pol.holderid <> LEFT(pc.VendorID, 6) THEN holder.MiddleName ELSE '' END
	,CASE WHEN pol.holderid <> LEFT(pc.VendorID, 6) THEN holder.AddressLine1 ELSE '' END
	,CASE WHEN pol.holderid <> LEFT(pc.VendorID, 6) THEN holder.AddressLine2 ELSE '' END
	,CASE WHEN pol.holderid <> LEFT(pc.VendorID, 6) THEN holder.City ELSE '' end
	,CASE WHEN pol.holderid <> LEFT(pc.VendorID, 6) THEN holder.[State] ELSE '' END 
	,''	
	,CASE WHEN pol.holderid <> LEFT(pc.VendorID, 6) THEN holder.DOB ELSE '' END
	,CASE WHEN pol.holderid <> LEFT(pc.VendorID, 6) THEN holder.HomePhone ELSE '' end
	,@PracticeID
	,pol.PolicyID 
	,@VendorImportID
	,'Y'
FROM dbo.[_import_2_18_Coverage] cov
LEFT JOIN dbo._import_2_18_Policy pol ON 
	pol.policyid = cov.policyid
LEFT JOIN dbo.PatientCase pc ON 
	pc.VendorID = cov.patientid + cov.policyid
...
PRINT 'Inserting records into ContractFeeSchedule (Standard)...'
	...
	SELECT
		...
		,0
		,0
	FROM dbo.[_import_2_18_FeeSchedule] impFS
	INNER JOIN dbo.[Contract] c ON 
		CAST(c.Notes AS VARCHAR) = CAST(@VendorImportID AS VARCHAR) AND
		c.PracticeID = @PracticeID
	INNER JOIN dbo.ProcedureCodeDictionary pcd ON impFS.[feescheduleid] = pcd.ProcedureCode
	WHERE
...
PRINT 'Inserting records into ContractFeeSchedule ...'
	...
	INNER JOIN dbo.[Contract] c ON 
		CAST(c.Notes AS VARCHAR) = CAST(@VendorImportID AS VARCHAR) AND
		c.PracticeID = @PracticeID AND
		c.ContractName = 'Blue Shield'
	INNER JOIN dbo.ProcedureCodeDictionary pcd ON impFS.[feescheduleid] = pcd.ProcedureCode
	WHERE
		CAST([AmountA] AS MONEY) > 0 AND
		AmountA <> ''
...
SELECT 
	      ...
	      GETDATE() , -- ModifiedDate - datetime
          0 , -- ModifiedUserID - int
          c.ContractID , -- ContractID - int
          icp.InsuranceCompanyPlanID  -- PlanID - int
FROM dbo.[_import_2_18_FeeSchedule] impFS
INNER JOIN dbo.[Contract] c ON
	CAST(c.Description AS VARCHAR) = 'Vendor Import ' + CAST(@VendorImportID AS VARCHAR) AND
	c.PracticeID = @PracticeID AND
	c.ContractName = 'Blue Shield'
INNER JOIN dbo.InsuranceCompanyPlan icp ON
	icp.PlanName LIKE '%Blue Shield%' AND
	icp.VendorImportID = @VendorImportID
...
FROM dbo.[_import_2_18_Patient] pa
LEFT JOIN dbo.Patient realP ON	
	pa.patientid = realP.VendorID AND
	realP.VendorImportID = @VendorImportID
WHERE pa.alertnote <> ''
PRINT CAST(@@rowcount AS VARCHAR(10)) + ' records inserted'
