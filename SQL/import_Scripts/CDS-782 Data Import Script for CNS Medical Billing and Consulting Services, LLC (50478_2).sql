--USE superbill_50478_dev
USE superbill_50478_prod
GO
...
UPDATE dbo.[_import_1_1_Doctor] SET FirstName = 'MOHAMMED' WHERE doctorid = 2
...
PRINT ''
PRINT 'Updating Existing Insurance Company with VendorID...'
UPDATE dbo.InsuranceCompany 
	SET VendorID = i.insurancecompanyid , 
		VendorImportID = @VendorImportID
FROM dbo.InsuranceCompany ic 
INNER JOIN dbo.[_import_1_1_InsuranceCompany] i ON 
	ic.InsuranceCompanyName = i.InsuranceCompanyName AND 
	ic.AddressLine1 = i.AddressLine1 AND 
	ic.ZipCode = i.ZipCode
PRINT CAST(@@ROWCOUNT AS VARCHAR) + ' records updated'
...
FROM dbo.[_import_1_1_PracticeToInsuranceCompany] ptic
	INNER JOIN dbo.InsuranceCompany ic ON
		...
		ic.VendorImportID = @VendorImportID
	LEFT JOIN dbo.PracticeToInsuranceCompany optic ON 
		ic.InsuranceCompanyID = optic.InsuranceCompanyID AND 
		optic.PracticeID = @TargetPracticeID
WHERE ptic.practiceid = @SourcePracticeID AND optic.PK_ID IS NULL
PRINT CAST (@@ROWCOUNT AS VARCHAR(10)) + ' records inserted '
