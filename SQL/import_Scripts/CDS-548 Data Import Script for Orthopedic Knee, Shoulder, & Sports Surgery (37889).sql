USE superbill_37889_dev
--USE superbill_37889_prod
go
...
FROM dbo.[_import_1_1_Carriers] i
LEFT JOIN dbo.[_import_1_1_CarrierCategories] cc ON
	i.carriercategoryfid = cc.carriercategoryuid
PRINT CAST(@@ROWCOUNT AS VARCHAR) + ' records inserted'
...
WHERE i.facilityuid <> 56 AND i.display = 1
PRINT CAST(@@ROWCOUNT AS VARCHAR) + ' records inserted'
...
SELECT DISTINCT
          ...
          GETDATE()  -- ModifiedDate - datetime
FROM dbo.[_import_1_1_AppointmentTypes] i WHERE NOT EXISTS (SELECT * FROM dbo.AppointmentReason ar WHERE ar.name = i.name AND ar.PracticeID = @PracticeID) AND i.name <> ''
PRINT CAST(@@ROWCOUNT AS VARCHAR) + ' records inserted'
