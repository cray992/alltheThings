...
--			 i.ssn  -- SSN - varchar(9)
--FROM dbo.Patient p 
--INNER JOIN dbo._import_6_2_PatientDemographics i ON p.PatientID = i.id
...
PRINT ''
PRINT 'Updating Patients in Practice 1 with ZipCode...'
UPDATE dbo.Patient
	SET	ZipCode = REPLACE(i.zipcode,'-','0') , 
...
INNER JOIN dbo._import_3_1_PatientDemographics i ON 
    ...
    p.PracticeID = 1 AND 
    p.VendorImportID = 3
WHERE p.ModifiedUserID = 0 AND p.ZipCode = '' AND LEN(i.zipcode) = 9
PRINT CAST(@@ROWCOUNT AS VARCHAR) + ' records updated'
...
FROM dbo._import_5_2_InsuranceCOMPANYPLANList ICP 
...
FROM dbo._import_5_2_InsuranceCOMPANYPLANList AS IICL
...
FROM dbo.Patient p 
INNER JOIN dbo._import_5_2_PatientDemographics i ON
...
SELECT DISTINCT
	...
	CASE 
		WHEN LEN(REPLACE(PD.zipcode,'-','0')) IN (5,9) THEN REPLACE(PD.zipcode,'-','0')
	...
	CASE
		WHEN ISDATE(PD.dateofbirth) = 1 THEN CAST(PD.dateofbirth AS DATETIME)
...
FROM dbo._import_5_2_PatientDemographics AS PD
...
FROM dbo._import_5_2_PatientDemographics pd
...
(	SELECT DISTINCT 
	...
	FROM dbo._import_5_2_PatientDemographics
