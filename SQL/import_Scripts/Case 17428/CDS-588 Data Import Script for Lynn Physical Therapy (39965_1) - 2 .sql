...
UPDATE dbo.Practice 
	SET WizardComplete = 1 , 
		Phone = '7815993365' ,
		Fax = '7815993995' , 
		BillingAddressLine1 = '26 state st' , 
		BillingAddressLine2 = '' , 
		BillingCity = 'Lynn' ,
		BillingState = 'ma' , 
		BillingZipCode = '01901' ,
		BillingContactFirstName = 'LYNN' ,
		BillingContactLastName = 'COLL.'
WHERE PracticeID = 5

PRINT ''
PRINT 'Updating Insurance Company...'
UPDATE dbo.InsuranceCompany 
	SET CreatedPracticeID = 2
...
FROM dbo.Doctor i WHERE i.PracticeID = 1 AND i.[External] = 0 AND i.ActiveDoctor = 1 
AND NOT EXISTS (SELECT * FROM dbo.Doctor d WHERE d.FirstName = i.FirstName AND d.LastName = i.LastName AND d.PracticeID = @PracticeID)
...
SELECT DISTINCT
          ...
          EIN ,
          BillTypeID 
FROM dbo.ServiceLocation WHERE PracticeID = 1
PRINT CAST(@@ROWCOUNT AS VARCHAR) + ' records inserted'
-- Insurance Company
...
--SELECT DISTINCT
--	ic.Name
--	,@PracticeID -- Created Practice ID (since its global it will be null)
--	,GETDATE()
--	,0
--	,GETDATE()
--	,0
--	,ic.Name -- Hope it's unique!
--	,@VendorImportID,
...
--	13
--FROM dbo._import_4_5_Carrier ic
--WHERE ic.id NOT IN (SELECT ISNULL(VendorID, '') FROM dbo.InsuranceCompany) 
...
--FROM dbo._import_4_5_Carrier impIns
--INNER JOIN dbo.InsuranceCompany ic ON
--	impIns.name = ic.VendorID	
--PRINT CAST(@@ROWCOUNT AS VARCHAR(10)) + ' records inserted'
...
SELECT    @PracticeID , -- PracticeID - int
          ...
          GETDATE() , -- ModifiedDate - datetime
          0 , -- ModifiedUserID - int
          ref.[ID] , -- VendorID - varchar(50)
          @VendorImportID , -- VendorImportID - int
          ref.npi ,
          1  -- External - bit
FROM dbo._import_4_5_referral ref
WHERE ref.id <> ''
PRINT CAST(@@ROWCOUNT AS VARCHAR(10)) + ' records inserted'
...
SELECT DISTINCT
	...
	,0
	,pat.cellphone
FROM dbo.[_import_4_5_Patient] pat
PRINT CAST(@@ROWCOUNT AS VARCHAR(10)) + ' records inserted'
...
FROM dbo._import_4_5_PatientNotes pjn
...
FROM dbo.[_import_4_5_Patient] p 
INNER JOIN dbo.[_import_4_5_Policy] i ON 
...
LEFT JOIN dbo.[_import_4_5_Patient] h ON
	p.id = h.id
PRINT CAST(@@ROWCOUNT AS VARCHAR(10)) + ' records inserted'
...
INSERT INTO dbo.ContractsAndFees_StandardFee
	...
	FROM dbo.[_import_4_5_FeeSchedule] impSFS
...
FROM dbo.[_import_4_5_Patient] pa
