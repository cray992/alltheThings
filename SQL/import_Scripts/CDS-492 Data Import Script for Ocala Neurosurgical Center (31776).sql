...
SELECT DISTINCT
          ...
          i.clmattn , -- AddressLine1 - varchar(256)
          i.clmaddr , -- AddressLine2 - varchar(256)
...
WHERE clmcono <> 'Insurance Plan # 5,1'
...
SELECT DISTINCT
          ...
          GETDATE() , -- ModifiedDate - datetime
          0  -- ModifiedUserID - int
FROM dbo.[_import_1_1_PatientFilePatfiledat] WHERE patemplname NOT IN ('','*****','.')
PRINT CAST(@@ROWCOUNT AS VARCHAR) + ' records inserted'
...
SELECT DISTINCT
          ...
          CASE WHEN i.patdob NOT IN ('','0') THEN CONVERT(DATETIME,i.patdob, 112) ELSE NULL END , -- DOB - datetime
          ...
          @VendorImportID , -- VendorImportID - int
          CASE i.patstatus WHEN 2 THEN 2 ELSE 1 END , -- CollectionCategoryID - int
...
WHERE NOT EXISTS (SELECT * FROM dbo.AppointmentReason ar WHERE ar.Name = i.aptreason)
PRINT CAST(@@ROWCOUNT AS VARCHAR) + ' records inserted'


PRINT ''
PRINT 'Updating Servivce Location VendorID...'
UPDATE dbo.ServiceLocation SET VendorID = 62 WHERE ServiceLocationID = 1
PRINT CAST(@@ROWCOUNT AS VARCHAR) + ' records inserted'
...
SELECT DISTINCT
          ...
          15  -- TimeZoneID - int
FROM dbo.[_import_1_1_ServiceLocationPsrfiledat] i
...
SELECT DISTINCT
		  a.AppointmentID , -- AppointmentID - int
		  1, -- AppointmentResourceTypeID - int
          CASE WHEN i.aptdr IN (6,8,10,20,21,22,30,32,70) THEN 2
		       WHEN i.aptdr IN (11,12,13,40,42,71) THEN 3
			   WHEN i.aptdr IN (60,62,73,80,82) THEN 4
			   WHEN i.aptdr IN (7,34,44,89) THEN 5
			   WHEN i.aptdr IN (1,74,76,90,94,95,100) THEN 6
			   WHEN i.aptdr IN (4,35,48,85,96) THEN 7
			   WHEN i.aptdr IN (9,38,45,88) THEN 8
			   WHEN i.aptdr IN (5,36,46,66,86) THEN 9
...
SELECT DISTINCT
          ...
          1  -- CustomCode - bit
FROM dbo.[_import_1_1_DeDupedServCode] i
...
SELECT DISTINCT
		  i.servmod , -- ProcedureModifierCode - varchar(16)
          i.servmod , -- ModifierName - varchar(250)
          GETDATE() , -- CreatedDate - datetime
          0 , -- CreatedUserID - int
          GETDATE() , -- ModifiedDate - datetime
          0  -- ModifiedUserID - int
FROM dbo.[_import_1_1_DeDupedServCodeAmounts] i
WHERE i.servmod <> '' AND NOT EXISTS (SELECT * FROM dbo.ProcedureModifier pm WHERE i.servmod = pm.ProcedureModifierCode)
PRINT CAST (@@ROWCOUNT AS VARCHAR) + ' records inserted'
...
PRINT 'Inserting into Standard Fee...'
      ...
      SELECT DISTINCT
                    ...
                    i.servprof2 , -- SetFee - money
                    0  -- AnesthesiaBaseUnits - int
      FROM dbo.[_import_1_1_DeDupedServCodeAmounts] AS i
...
SELECT DISTINCT
		  ...
		  CASE WHEN i.insstartdt <> '' THEN CONVERT(DATETIME,i.insstartdt,112) ELSE NULL END , -- PolicyStartDate - datetime
		  CASE WHEN i.insenddt <> '' THEN CONVERT(DATETIME,i.insenddt,112) ELSE NULL END , -- PolicyEndDate - datetime
          ...
          CASE WHEN ip.iprfname <> p.FirstName AND ip.iprlname <> p.LastName THEN 
			CASE WHEN ip.iprdob <> '' THEN CONVERT(DATETIME,ip.iprdob,112) END END , -- HolderDOB - datetime
		  CASE WHEN ip.iprfname <> p.FirstName AND ip.iprlname <> p.LastName THEN 
...
--SELECT DISTINCT 
--CAST(patfile.pataccount AS INT) AS [Medical Record Number] ,
--patfile.patfname AS [Pat FirstName] ,
--patfile.patmi AS [Pat MiddleName] ,
--patfile.patlname AS [Pat LastName] , 
--ipfile.precedence AS [Ins Pol Precedence], 
--ipfile.insstatus AS [Status],
--clfile.clmname AS [PlanName],
--ipfile.insgroup AS [GroupName] , 
--ipfile.insid AS [Policy] ,
--CASE WHEN ipcfile.iprfname <> patfile.patfname AND ipcfile.iprlname <> patfile.patlname THEN 'O' ELSE 'S' END AS [Pat Relation] , -- PatientRelationshipToInsured - varchar(1)
--CASE WHEN ipcfile.iprfname <> patfile.patfname AND ipcfile.iprlname <> patfile.patlname THEN ipcfile.iprfname END AS [Pol Holder F Name] , -- HolderFirstName - varchar(64)
--CASE WHEN ipcfile.iprfname <> patfile.patfname AND ipcfile.iprlname <> patfile.patlname THEN ipcfile.iprmi END AS [Pol Holder M Name], -- HolderMiddleName - varchar(64)
--CASE WHEN ipcfile.iprfname <> patfile.patfname AND ipcfile.iprlname <> patfile.patlname THEN ipcfile.iprlname END AS [Pol Holder L Name],
--CASE WHEN ipcfile.iprfname <> patfile.patfname AND ipcfile.iprlname <> patfile.patlname THEN 
--	CASE WHEN ipcfile.iprdob <> '' THEN CONVERT(DATETIME,ipcfile.iprdob,112) END END AS [Pol Holder DOB] , -- HolderDOB - datetime
--CASE WHEN ipcfile.iprfname <> patfile.patfname AND ipcfile.iprlname <> patfile.patlname THEN 
--	CASE ipcfile.iprsex WHEN 'M' THEN 'M' WHEN 'F' THEN 'F' ELSE 'U' END END AS [Pol Holder Gender], --HolderGender - char(1)
--CASE WHEN ipcfile.iprfname <> patfile.patfname AND ipcfile.iprlname <> patfile.patlname THEN ipcfile.ipraddr END AS [Pol Holder Add1], -- HolderAddressLine1 - varchar(256)
--CASE WHEN ipcfile.iprfname <> patfile.patfname AND ipcfile.iprlname <> patfile.patlname THEN '' END [Pol Holder Add2] , -- HolderAddressLine2 - varchar(256)
--CASE WHEN ipcfile.iprfname <> patfile.patfname AND ipcfile.iprlname <> patfile.patlname THEN ipcfile.iprcity END AS [Pol Holder City], -- HolderCity - varchar(128)
--CASE WHEN ipcfile.iprfname <> patfile.patfname AND ipcfile.iprlname <> patfile.patlname THEN LEFT(ipcfile.iprstate, 2) END AS [Pol Holder State], -- HolderState - varchar(2)
--CASE WHEN ipcfile.iprfname <> patfile.patfname AND ipcfile.iprlname <> patfile.patlname THEN 
--	CASE WHEN LEN(REPLACE(REPLACE(ipcfile.iprzip,' ',''),'-','')) IN (4,8) THEN '0' + REPLACE(REPLACE(ipcfile.iprzip,' ',''),'-','')
--	ELSE LEFT(REPLACE(REPLACE(ipcfile.iprzip,' ',''),'-',''), 9) END END AS [Pol Holder ZipCode] , -- HolderZipCode - varchar(9)
--CASE WHEN ipcfile.iprfname <> patfile.patfname AND ipcfile.iprlname <> patfile.patlname THEN ipcfile.iprphone END [Pol Holder Phone]
--FROM dbo.[_import_1_1_PatientFilePatfiledat] patfile
--INNER JOIN dbo.[_import_1_1_InsPolicyInsfiledat] ipfile ON
--patfile.pataccount = ipfile.insaccount
--INNER JOIN dbo.[_import_1_1_InsPlanClmfiledat] clfile ON
--clfile.clmcono = ipfile.insconum
--LEFT JOIN dbo.[_import_1_1_InsuredPartyIprfiledat] ipcfile ON
--ipfile.insipr = ipcfile.iprno
