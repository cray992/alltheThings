...
FROM dbo.Doctor d
INNER JOIN dbo.[_import_2_5_Doctor] i ON
...
FROM dbo.Patient p
INNER JOIN dbo.[_import_2_5_Patient] i ON
...
FROM dbo.[_import_2_5_Patient] p
...
INNER JOIN dbo.[_import_2_5_Appointment] a ON 
	p.PatientID = a.PatientID AND 
	a.PracticeID = @SourcePracticeID
LEFT JOIN dbo.Patient tp ON 
	tp.VendorID = p.PatientID AND 
	tp.PracticeID = @TargetPracticeID
WHERE p.CreatedDate > '2015-04-06 00:00:00.000' AND tp.PatientID IS NULL
...
FROM dbo.[_import_2_5_PatientCase] pc
	INNER JOIN dbo.Patient p ON 
...
FROM dbo.[_import_2_5_InsurancePolicy] ip
...
PRINT ''
PRINT 'Inserting Into Appointment 1 of 2...'
...
FROM dbo.[_import_2_5_Appointment] i
	INNER JOIN dbo.Patient p ON
...
PRINT ''
PRINT 'Inserting Into Appointment 2 of 2...'
INSERT INTO dbo.Appointment
        ( PatientID ,
          PracticeID ,
          ServiceLocationID ,
...
NOT EXISTS (SELECT * FROM dbo.Appointment a WHERE CAST (i.AppointmentID AS VARCHAR) = a.[Subject])
PRINT CAST(@@ROWCOUNT AS VARCHAR) + ' records inserted'
...
SELECT DISTINCT
          ...
          GETDATE() , -- ModifiedDate - datetime
          GETDATE()  -- CreatedDate - datetime
FROM dbo.[_import_2_5_PracticeResource] i
WHERE i.resourcename <> '' AND i.PracticeID = @SourcePracticeID AND NOT EXISTS (SELECT * FROM dbo.PracticeResource pr WHERE pr.ResourceName = i.resourcename AND pr.PracticeID = @TargetPracticeID) 
PRINT CAST(@@ROWCOUNT AS VARCHAR) + ' records inserted'
...
SELECT DISTINCT
	atar.AppointmentID ,
	ar.Name
FROM dbo.[_import_2_5_AppointmentReason] ar
INNER JOIN dbo.[_import_2_5_AppointmentToAppointmentReason] atar ON
...
SELECT DISTINCT
	atr.AppointmentID ,
	pr.ResourceName
FROM dbo.[_import_2_5_AppointmentToResource] atr
INNER JOIN dbo.[_import_2_5_PracticeResource] pr ON
...
SELECT DISTINCT
          ...
          GETDATE() , -- ModifiedDate - datetime
          @TargetPracticeID  -- PracticeID - int
FROM dbo.[_import_2_5_AppointmentToResource] i
	INNER JOIN dbo.Appointment a ON
...
PRINT ''
PRINT 'Inserting Into Other Appointment...'
INSERT INTO dbo.Appointment
        ( 
          PracticeID ,
          ServiceLocationID ,
...
SELECT 
          ...
          CAST(i.appointmentid AS VARCHAR) + ' | ' + ip.firstname + ' ' + ip.lastname + ' (' + CAST(ip.patientid AS VARCHAR) + ')'  , -- Subject - varchar(64)
          i.notes , -- Notes - text
          GETDATE() , -- CreatedDate - datetime
          0 , -- CreatedUserID - int
          GETDATE() , -- ModifiedDate - datetime
...
FROM #appttopracres atpr
	INNER JOIN dbo.PracticeResource pr ON	
		...
		pr.PracticeID = @TargetPracticeID
	INNER JOIN dbo.Appointment a ON
		LEFT(a.[Subject],CHARINDEX(' ', a.[Subject]) - 1) = atpr.appointmentid AND
		a.PracticeID = @TargetPracticeID
...
FROM dbo.[_import_2_5_AppointmentToResource] i
	INNER JOIN dbo.Appointment a ON
		LEFT(a.[Subject],CHARINDEX(' ', a.[Subject]) - 1) = i.appointmentid AND
		a.PracticeID = @TargetPracticeID
	LEFT JOIN dbo.Doctor d ON
		i.resourceid = d.VendorID AND
		d.PracticeID = @TargetPracticeID AND
