...
PRINT 'Inserting into Patient...'
      ...
      WHERE (impPat.[firstname] <> '' AND impPat.[lastname] <> '') 
		AND NOT EXISTS (SELECT * FROM dbo.Patient realP WHERE impPat.firstname = realP.FirstName AND impPat.lastname = realP.LastName AND CAST(CAST(impPat.dateofbirth AS DATE) AS DATETIME) = CAST(CAST(realP.DOB AS DATE) AS DATETIME))
PRINT CAST(@@ROWCOUNT AS VARCHAR) + ' records inserted'
END 
