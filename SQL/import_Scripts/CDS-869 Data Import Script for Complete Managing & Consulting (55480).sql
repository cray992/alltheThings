...
FROM dbo.[_import_1_2_rptPatientDemographicExport] 
...
FROM dbo.[_import_1_2_rptPatientDemographicExport] i 
WHERE NOT EXISTS 
...
SELECT DISTINCT
          ...
          CASE i.referringprovderfullname 
			WHEN 'Haridas M Dasani, MD' THEN (SELECT DoctorID FROM dbo.Doctor WHERE FirstName = 'HARIDAS' AND LastName = 'DASANI' AND [External] = 0 AND ActiveDoctor = 1 AND PracticeID = @PracticeID)
			WHEN 'HARIDAS M. DASANI, MD' THEN (SELECT DoctorID FROM dbo.Doctor WHERE FirstName = 'HARIDAS' AND LastName = 'DASANI' AND [External] = 0 AND ActiveDoctor = 1 AND PracticeID = @PracticeID)
          ...
          CASE WHEN ISDATE(i.patientdob) = 1 THEN i.patientdob ELSE '1901-01-01 12:00:00.000' END , -- DOB - datetime
          ...
          GETDATE() , -- ModifiedDate - datetime
          0 , -- ModifiedUserID - int
          CASE WHEN es.EmploymentStatusCode IS NULL THEN 'U' ELSE es.EmploymentStatusCode END , -- EmploymentStatus - char(1)
   ...
   --       CASE i.servicelocationname 
		 ...
		 -- ELSE NULL END , -- DefaultServiceLocationID - int
          ...
          CASE i.primarycarephysicianfullname
			WHEN 'Haridas M Dasani, MD' THEN (SELECT MAX(DoctorID) FROM dbo.Doctor WHERE FirstName = 'HARIDAS' AND LastName = 'DASANI' AND PracticeID = @PracticeID)  
			WHEN 'HARIDAS M. DASANI, MD' THEN (SELECT MAX(DoctorID) FROM dbo.Doctor WHERE FirstName = 'HARIDAS' AND LastName = 'DASANI' AND PracticeID = @PracticeID)  
		  ELSE NULL END, -- PrimaryCarePhysicianID - int
          i.patientid , -- VendorID - varchar(50)
          @VendorImportID , -- VendorImportID - int
          CASE WHEN cc.CollectionCategoryID IS NULL THEN @DefaultCollectionCategory ELSE cc.CollectionCategoryID END , -- CollectionCategoryID - int
          1 , -- Active - bit
...
WHERE p.PatientID IS NULL AND i.patientfirstname <> '' AND i.patientlastname <> ''
PRINT CAST(@@ROWCOUNT AS VARCHAR) + ' records inserted'
