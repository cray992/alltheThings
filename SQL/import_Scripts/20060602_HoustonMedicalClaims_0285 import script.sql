--06022006 kdev01_0285 Houston Medical Claims import script
-- PracticeID = 1
-- VendorImportID = ? -- 

--select * from doctor where practiceid = 1 -- HMC
BEGIN TRANSACTION

DECLARE @PracticeID int
DECLARE @VendorImportID int

INSERT INTO VendorImport (VendorName, Notes, VendorFormat) VALUES ('Barbara Marino MD', 'Houston Medical Claims - Patients, Doctors & Ins.', 'MediSoft')

SET @VendorImportID = @@IDENTITY
SET @PracticeID = 1 -- Barbara Marino, MD
...
FROM Doctor D INNER JOIN HMC_mwrph a
...
SELECT chartnumber, casenumber from HMC_mwcas
...
FROM HMC_MWPAT pat
INNER JOIN HMC_MWCAS cas ON cas.Guarantor = pat.ChartNumber
...
FROM Patient P INNER JOIN HMC_MWCAS DC
...
WHERE
	...
	AND p.VendorID NOT IN (SELECT DISTINCT ChartNumber FROM HMC_mwcas) 
...
FROM HMC_MWCAS DC INNER JOIN PatientCase PC
...
/*
select top 100 * from HMC_mwcas where chartnumber = '00000050'
...
select top 100 * from HMC_mwcas where chartnumber = '00000049'
...
--select * from hmc_mwpat where patienttype = 'Patient'
