...
FROM dbo._import_7_11_Case AS ins
...
FROM dbo.[_import_7_11_PatientDemographics] 
...
SELECT DISTINCT
	...
	@VendorImportID -- VendorImportID - int
FROM dbo.[_import_7_11_Case] impCase
...
SELECT  distinct  
	...
	@VendorImportID -- VendorImportID - int
FROM dbo.[_import_7_11_Case] AS impCase
INNER JOIN
	(
		SELECT impPolicy.acctid, impPolicy.acctplan, impPolicy.pataltlookup FROM _import_7_11_InsuredID impPolicy
		INNER JOIN
			(
				SELECT acctid, acctplan, MAX(AutoTempID) AS LatestPolicyRowID FROM dbo.[_import_7_11_InsuredID] GROUP BY acctid, acctplan
...
INNER JOIN dbo.[_import_7_11_GuarantorList] AS impGuarantor ON
...
FROM dbo.[_import_7_11_FeeSchedule] impSFS
