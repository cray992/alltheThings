USE superbill_28123_dev
--USE superbill_28123_prod
GO
...
FROM dbo.[_import_3_1_InsuranceCompanyPlanList] AS IICL
WHERE
	insurancecompanyname <> '' OR insuranceplanname <> ''
PRINT CAST (@@ROWCOUNT AS VARCHAR(10)) + ' records inserted '
--SELECT COUNT(*) FROM dbo.[_import_3_1_InsuranceCOMPANYPLANList] WHERE insurancecompanyname <> '' OR insuranceplanname <> ''
--1170
...
LEFT JOIN dbo.[_import_3_1_InsuranceCOMPANYPLANList] ICP ON
...
FROM dbo.[_import_3_1_ReferringProviders] irp 
...
PRINT ''
PRINT 'Inserting Into Referring Doctor from [dbo._import_3_1_PatientDemographics.PrimaryCarePhysician]...'
INSERT INTO dbo.Doctor
...
SELECT DISTINCT  
          ...
          @VendorImportID , -- VendorImportID - int
          1  -- External - bit
FROM dbo.[_import_3_1_PatientDemographics] irp
...
PRINT ''
PRINT 'Inserting Into Referring Doctor from [dbo._import_3_1_PatientDemographics.ReferringProviders]...'
INSERT INTO dbo.Doctor
        ( PracticeID , Prefix , FirstName , MiddleName , LastName , Suffix , ActiveDoctor , CreatedDate , CreatedUserID , ModifiedDate , ModifiedUserID , 
		  VendorID , VendorImportID , [External] 
        )
...
PRINT CAST (@@ROWCOUNT AS VARCHAR(10)) + ' records inserted '
--SELECT COUNT(*) FROM dbo.[_import_3_1_PatientDemographics] WHERE firstname <> '' AND lastname <> ''
--27361
...
PRINT CAST (@@ROWCOUNT AS VARCHAR(10)) + ' records inserted '
--SELECT COUNT(*) FROM dbo.[_import_3_1_PatientDemographics] WHERE patientnote <> ''
--8318
...
PRINT CAST (@@ROWCOUNT AS VARCHAR(10)) + ' records inserted '
--SELECT COUNT(*) FROM dbo.[_import_3_1_PatientDemographics] WHERE [patientalertmessage] <> ''
--18
...
FROM dbo.[_import_3_1_PatientDemographics] AS TP
...
FROM dbo.[_import_3_1_PatientDemographics] AS i
...
PRINT CAST (@@ROWCOUNT AS VARCHAR(10)) + ' records inserted '
--SELECT COUNT(*) FROM dbo.[_import_3_1_PatientDemographics] WHERE insurancecode1 <> ''
--24649
...
PRINT CAST (@@ROWCOUNT AS VARCHAR(10)) + ' records inserted '
--SELECT COUNT(*) FROM dbo.[_import_3_1_PatientDemographics] where insurancecode2 <> ''
--8807
...
PRINT CAST (@@ROWCOUNT AS VARCHAR(10)) + ' records inserted '
--SELECT COUNT(*) FROM dbo.[_import_3_1_PatientDemographics] WHERE insurancecode3 <> ''
--85
...
PRINT 'Inserting Into StandardFee...'
	...
	FROM dbo.[_import_3_1_StandardFeeSchedule] impSFS
...
PRINT CAST (@@ROWCOUNT AS VARCHAR(10)) + ' records inserted '
--SELECT COUNT(*) FROM dbo.[_import_3_1_StandardFeeSchedule] WHERE cpt <> ''
--286
...
PRINT 'Inserting Into ContractRateSchedule... '
	...
	SELECT DISTINCT
          ...
          15 , -- AnesthesiaTimeIncrement - int
          0  -- Capitated - bit
	FROM dbo.[_import_3_1_InsuranceSpecificFeeSchedule] impCRS
	INNER JOIN dbo.[InsuranceCompanyPLan] ICP ON
...
PRINT 'Inserting Into ContractRate...'
	...
	INNER JOIN dbo.[_import_3_1_InsuranceSpecificFeeSchedule] impIFS ON
...
PRINT CAST (@@ROWCOUNT AS VARCHAR(10)) + ' records inserted '
--SELECT COUNT(*) FROM dbo.[_import_3_1_InsuranceSpecificFeeSchedule] WHERE cpt <> ''
--8533
