...
FROM dbo._import_1_1_PatientDemographics AS PD
	...
	LEFT JOIN dbo.Patient AS p ON PD.firstname = p.FirstName AND PD.lastname = p.LastName AND DATEADD(hh,12,CAST(PD.dateofbirth AS DATETIME)) = p.DOB AND p.PracticeID = @PracticeID
WHERE PD.firstname <> '' AND PD.lastname <> '' AND p.PatientID IS NULL AND pd.active = 1
...
PRINT 'Inserting Into Patient Journal Note...'
	...
	SELECT    GETDATE() , -- CreatedDate - datetime
	          ...
	          0 , -- Hidden - bit
	          PJN.patientjournalnotemessage , -- NoteMessage - varchar(max)
	          0 , -- AccountStatus - bit
	          1 , -- NoteTypeCode - int
	          0  -- LastNote - bit
	FROM dbo._import_1_1_PatientDemographics PJN
	INNER JOIN dbo.Patient PAT ON
		PJN.ChartNumber = PAT.VendorID AND
		PAT.PracticeID = @PracticeID
	WHERE pjn.patientjournalnotemessage <> ''	
PRINT CAST(@@ROWCOUNT AS VARCHAR) + ' records inserted'
