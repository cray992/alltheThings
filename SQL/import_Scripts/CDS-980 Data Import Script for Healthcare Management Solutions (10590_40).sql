...
--			 i.ssn  -- SSN - varchar(9)
--FROM dbo.Patient p 
--INNER JOIN dbo._import_28_40_PatientDemographics i ON p.PatientID = i.id
...
CREATE TABLE #TempDoc (FirstName VARCHAR(64) , LastName VARCHAR(64))
INSERT INTO #TempDoc
        ( FirstName, LastName )
VALUES  ( 'Thalia', -- FirstName - varchar(64)
          'Farshchian'  -- LastName - varchar(64)
        )

INSERT INTO #TempDoc
        ( FirstName, LastName )
VALUES  ( 'MA', -- FirstName - varchar(64)
          'Schedule'  -- LastName - varchar(64)
        )

INSERT INTO #TempDoc
        ( FirstName, LastName )
VALUES  ( 'IP', -- FirstName - varchar(64)
          'Coach'  -- LastName - varchar(64)
        )

INSERT INTO dbo.Doctor
( 
	PracticeID ,  Prefix , FirstName , MiddleName , LastName , Suffix , ActiveDoctor , CreatedDate , CreatedUserID , ModifiedDate , 
	ModifiedUserID , UserID , VendorID , VendorImportID , [External] 
)
SELECT DISTINCT
	@PracticeID , -- PracticeID - int
	'' , -- Prefix - varchar(16)
	LEFT(D.firstname,64) , -- FirstName - varchar(64)
	'', -- MiddleName - varchar(64)
	LEFT(D.lastname,64) , -- LastName - varchar(64)
	'' , -- Suffix - varchar(16)
	1, -- ActiveDoctor - bit
	...
	0 , -- UserID - int
	d.FirstName+d.LastName , -- VendorID - varchar(50)
...
SELECT DISTINCT
	...
	@VendorImportID  -- VendorImportID - int
FROM dbo._import_27_40_InsuranceCOMPANYPLANList ICP 
...
/*
==========================================================================================================================================
CREATE INSURANCE COMPANIES FROM [_import_27_40_InsuranceCOMPANYPLANList]
==========================================================================================================================================
*/	
...
FROM dbo._import_27_40_InsuranceCOMPANYPLANList AS IICL
...
FROM dbo._import_27_40_PatientDemographics AS PD
...
FROM dbo._import_27_40_PatientDemographics pd
	JOIN dbo.Patient AS PAT ON VendorImportID = @VendorImportID AND VendorID = pd.chartnumber
	LEFT JOIN dbo.PatientCase pc ON pc.PatientID = PAT.PatientID AND pc.PracticeID = @PracticeID
...
(	SELECT DISTINCT 
	...
	FROM dbo._import_27_40_PatientDemographics
...
FROM dbo.Patient p
	INNER JOIN dbo._import_27_40_PatientDemographics i ON 
...
PRINT 'Inserting Into Appointment...'
	...
	FROM dbo._import_27_40_PatientAppointments PA
...
PRINT 'Inserting Into Appointmen to Resource...'	
	...
	SELECT DISTINCT  APP.AppointmentID , -- AppointmentID - int
	          ...
	          CASE WHEN DOC.DoctorID IS NULL THEN
				CASE pa.doctorlastname 
					WHEN 'Coach, _' THEN (SELECT DoctorID FROM dbo.Doctor WHERE LastName = 'Coach' AND ActiveDoctor = 1 AND [External] = 0 AND PracticeID = @PracticeID) 
					WHEN 'Farshchian, N.D.' THEN (SELECT DoctorID FROM dbo.Doctor WHERE LastName = 'Farshchian' AND ActiveDoctor = 1 AND [External] = 0 AND PracticeID = @PracticeID) 
					WHEN 'Schedule' THEN (SELECT DoctorID FROM dbo.Doctor WHERE LastName = 'Schedule' AND ActiveDoctor = 1 AND [External] = 0 AND PracticeID = @PracticeID) 
				END
...
PRINT 'Inserting Into Patient Journal Note...'
	...
	SELECT    CASE WHEN PJN.CreatedDate  = '' THEN GETDATE() ELSE PJN.CreatedDate END , -- CreatedDate - datetime
	          ...
	          1 , -- NoteTypeCode - int
	          0  -- LastNote - bit
	FROM dbo.[_import_27_40_PatientJournalNote] PJN
	INNER JOIN dbo.Patient PAT ON
...
DROP TABLE #tempPolicies , #TempDoc
