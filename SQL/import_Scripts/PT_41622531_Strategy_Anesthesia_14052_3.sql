...
SELECT DISTINCT
	...
	,13
FROM dbo.[_import_3_3_PatientsGuarantorsandInsur] ic
WHERE ic.[1payername] <> ''
...
SELECT DISTINCT
	...
	,13
FROM dbo.[_import_3_3_PatientsGuarantorsandInsur] ic
WHERE ic.[2payername] NOT IN (SELECT [1payername] FROM dbo.[_import_3_3_PatientsGuarantorsandInsur]) 
	AND ic.[2payername] <> ''
PRINT CAST(@@ROWCOUNT AS VARCHAR(10)) + ' records inserted'
...
SELECT
          ...
          @VendorImportID , -- VendorImportID - int
          1  -- Active - bit
FROM dbo.[_import_3_3_PatientsGuarantorsandInsur] pat
PRINT CAST(@@ROWCOUNT AS VARCHAR(10)) + ' records inserted'
...
FROM dbo.[_import_3_3_PatientsGuarantorsandInsur] ip
...
SELECT    pc.PatientCaseID , -- PatientCaseID - int
          ...
          '' , -- HolderSuffix - varchar(16)
		  CASE WHEN ip.[2patientrelationshiptoinsured] = 'Self' THEN ''
				WHEN ISDATE([2insureddob]) = 1 THEN [2insureddob] END  , -- HolderDOB - datetime
          GETDATE() , -- CreatedDate - datetime
          0 , -- CreatedUserID - int
          GETDATE() , -- ModifiedDate - datetime
          0 , -- ModifiedUserID - int
