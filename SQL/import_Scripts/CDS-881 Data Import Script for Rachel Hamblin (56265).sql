...
INNER JOIN dbo.[_import_1_2_PatientDemographics] AS m ON
   pat.VendorID = m.chartnumber AND 
   pat.VendorImportID = @VendorImportID 
WHERE m.BalanceForward <> '' AND REPLACE(m.BalanceFOrward,'$','') > '0' AND REPLACE(m.chartnumber,'$','') <> '' AND m.chartnumber <> ''
...
LEFT JOIN dbo.[_import_1_2_PatientDemographics] as pd ON 
