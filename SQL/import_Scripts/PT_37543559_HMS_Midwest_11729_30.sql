...
SET @PracticeID = 30
SET @VendorImportID = 36 -- Vendor import record created through import tool
...
SELECT DISTINCT
	...
	,GETDATE()
	,0
	,ic.[uid] + CAST(@VendorImportID AS VARCHAR) -- Hope it's unique!
	,@VendorImportID,
	...
	13
FROM dbo.[_import_35_30_Carrier] ic
...
SELECT DISTINCT 
	...
	,GETDATE()
	,0
	,impIns.[uid] + CAST(@VendorImportID AS VARCHAR)
	,@VendorImportID
	,ic.InsuranceCompanyID
FROM dbo.[_import_35_30_Carrier] impIns
INNER JOIN dbo.InsuranceCompany ic ON
	(impIns.[uid] + CAST(@VendorImportID AS VARCHAR)) = ic.VendorID
...
SELECT    @PracticeID , -- PracticeID - int
          '' , -- Prefix - varchar(16)
          LEFT(ref.FirstName, 64) , -- FirstName - varchar(64)
          LEFT(ref.Middle, 64) , -- MiddleName - varchar(64)   
          LEFT(ref.lastName, 64) , -- LastName - varchar(64)
          '' , -- Suffix - varchar(16)
          1 , -- ActiveDoctor - bit
          GETDATE() , -- CreatedDate - datetime
          0 , -- CreatedUserID - int
          GETDATE() , -- ModifiedDate - datetime
          0 , -- ModifiedUserID - int
          LEFT(ref.[UID], 50) , -- VendorID - varchar(50)
          @VendorImportID , -- VendorImportID - int
          LEFT(ref.npi, 10)  ,
          1  -- External - bit
FROM dbo._import_36_30_referral ref
...
SELECT DISTINCT
	...
	,1
FROM dbo.[_import_36_30_Patient] pat
...
FROM dbo._import_36_30_PatientNotes pjn
INNER JOIN dbo.Patient realP ON
...
LEFT JOIN dbo.[_import_36_30_Patient] pat ON 
	pol.[uid] = pat.primarypolicyid
lEFT JOIN dbo.InsuranceCompanyPlan icp ON 
	pol.carrierid + CAST(@VendorImportID AS VARCHAR) = icp.VendorID AND
...
PRINT 'Inserting records into ContractFeeSchedule (Standard)...'
	...
	SELECT
		...
		,0
		,0
	FROM dbo.[_import_36_30_FeeSchedule] impFS
...
FROM dbo.[_import_36_30_Patient] pa
