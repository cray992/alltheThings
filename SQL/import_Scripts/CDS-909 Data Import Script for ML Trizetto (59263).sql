...
FROM dbo.[_import_6_6_Carriers] i
...
--FROM dbo.[_import_6_6_Facilities] i
...
FROM dbo.[_import_6_6_ReferringProviders] i
WHERE i.display = 1
PRINT CAST(@@ROWCOUNT AS VARCHAR) + ' records inserted'
...
SELECT DISTINCT
          ...
          GETDATE() , -- ModifiedDate - datetime
          0  -- ModifiedUserID - int
FROM dbo.[_import_6_6_PatientInfo] i 
...
LEFT JOIN dbo.[_import_6_6_ResponsibleParties] rp ON
	i.responsiblepartyfid = rp.responsiblepartyuid
LEFT JOIN dbo.[_import_6_6_Providers] ip ON
...
SELECT DISTINCT
          ...
          1 , -- NoteTypeCode - int
          0  -- LastNote - bit
FROM dbo.[_import_6_6_Note] i
INNER JOIN dbo.Patient p ON
	i.patientfid = p.VendorID AND 
	p.VendorImportID = @VendorImportID
INNER JOIN dbo.[_import_6_6_NoteTypes] n ON 
...
FROM dbo.[_import_6_6_Coverages] i
...
FROM dbo.[_import_6_6_Appointments] i
INNER JOIN dbo.Patient p ON
...
INNER JOIN dbo.Patient p ON
	...
	p.VendorImportID = @VendorImportID
INNER JOIN dbo.[_import_6_6_Appointments] i ON
...
SELECT DISTINCT
          ...
          GETDATE()  -- ModifiedDate - datetime
FROM dbo.[_import_6_6_AppointmentTypes] i WHERE NOT EXISTS (SELECT * FROM dbo.AppointmentReason ar WHERE ar.name = i.name AND ar.PracticeID = @PracticeID) 
...
SELECT DISTINCT
	i.appointmentfid ,
	at.name
FROM dbo.[_import_6_6_LnkAppointmentsTypes] i
INNER JOIN dbo.[_import_6_6_AppointmentTypes] at ON
