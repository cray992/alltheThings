...
FROM dbo._import_4_10_Case AS ins
...
FROM dbo.[_import_4_10_PatientDemographics] 
...
SELECT DISTINCT
	...
	@VendorImportID -- VendorImportID - int
FROM dbo.[_import_4_10_Case] impCase
...
SELECT  distinct  
	...
	@VendorImportID -- VendorImportID - int
FROM dbo.[_import_4_10_Case] AS impCase
INNER JOIN
	(
		SELECT impPolicy.acctid, impPolicy.acctplan, impPolicy.pataltlookup FROM _import_4_10_InsuredID impPolicy
		INNER JOIN
			(
				SELECT acctid, acctplan, MAX(AutoTempID) AS LatestPolicyRowID FROM dbo.[_import_4_10_InsuredID] GROUP BY acctid, acctplan
...
INNER JOIN dbo.[_import_4_10_GuarantorList] AS impGuarantor ON
...
FROM dbo.[_import_4_10_FeeSchedule] impSFS
