...
SELECT DISTINCT
	...
	13
FROM dbo._import_4_4_Carrier ic
WHERE ic.name NOT IN (SELECT ISNULL(VendorID, '') FROM dbo.InsuranceCompany) 
...
FROM dbo._import_4_4_Carrier impIns
INNER JOIN dbo.InsuranceCompany ic ON
	impIns.name = ic.VendorID	
WHERE impIns.id NOT IN (SELECT ISNULL(VendorID, '') FROM dbo.InsuranceCompanyPlan) 
PRINT CAST(@@ROWCOUNT AS VARCHAR(10)) + ' records inserted'
...
SELECT    @PracticeID , -- PracticeID - int
          ...
          ref.npi ,
          1  -- External - bit
FROM dbo._import_4_4_referral ref
WHERE ref.id <> ''
PRINT CAST(@@ROWCOUNT AS VARCHAR(10)) + ' records inserted'
...
SELECT DISTINCT
	...
	,0
	,pat.cellphone
FROM dbo.[_import_4_4_Patient] pat
PRINT CAST(@@ROWCOUNT AS VARCHAR(10)) + ' records inserted'
...
FROM dbo._import_4_4_PatientNotes pjn
...
FROM dbo.[_import_4_4_Patient] p 
INNER JOIN dbo.[_import_4_4_Policy] i ON 
...
LEFT JOIN dbo.[_import_4_4_Patient] h ON
	p.id = h.id
PRINT CAST(@@ROWCOUNT AS VARCHAR(10)) + ' records inserted'
...
INSERT INTO dbo.ContractsAndFees_StandardFee
	...
	FROM dbo.[_import_4_4_FeeSchedule] impSFS
...
FROM dbo.[_import_4_4_Patient] pa
