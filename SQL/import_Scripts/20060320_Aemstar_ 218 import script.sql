--03202006 k0_0218 HCPlus - Aemstar import script
-- PracticeID = 28
-- VendorImportID = ?
...
--select * from doctor where practiceid = 28 -- Aemstar
BEGIN TRANSACTION

DECLARE @PracticeID int
DECLARE @VendorImportID int

INSERT INTO VendorImport (VendorName, Notes, VendorFormat) VALUES ('Aemstar EMS', 'HCPlus Import - Patients, Doctors & Insurance', 'MediSoft')

SET @VendorImportID = @@IDENTITY
SET @PracticeID = 28 -- Aemstar EMS
...
FROM Aemstar_MWPAT pat
INNER JOIN Aemstar_MWCAS cas ON cas.Guarantor = pat.ChartNumber
...
SELECT  @VendorImportID, pat.ChartNumber, @PracticeID, 		-- pg.PatientChartNumber, pg.GuarantorChartNumber, 
	...
	CASE WHEN pat.ZipCode IS NOT NULL AND LTRIM(RTRIM(pat.ZipCode)) <> '' THEN LEFT(LTRIM(RTRIM(REPLACE(ZipCode,'-',''))), 10) ELSE NULL END, 
...
/*WHERE ic.Name NOT IN (
	SELECT InsuranceCompanyName FROM #EIC eic
	JOIN Aemstar_mwins ic ON LTRIM(RTRIM(ic.Name)) = LTRIM(RTRIM(eic.InsuranceCompanyName))
	WHERE
	LTRIM(RTRIM(eic.City)) = LTRIM(RTRIM(ic.City))
	AND LTRIM(RTRIM(eic.State)) = LTRIM(RTRIM(ic.State))
	AND LEFT(eic.ZipCode, 5) = LEFT(ic.ZipCode, 5) 
	AND LTRIM(RTRIM(eic.InsuranceCompanyName)) = LTRIM(RTRIM(ic.Name))
	AND REPLACE(REPLACE(eic.AddressLine1, ' ', ''), '.', '') = REPLACE(REPLACE(ic.Street1, ' ', ''), '.', '')
)  */
...
FROM Patient P INNER JOIN Aemstar_MWCAS DC
...
FROM Aemstar_MWCAS DC INNER JOIN PatientCase PC
...
/*
select top 100 * from aemstar_mwcas where chartnumber = '00000050'
...
select top 100 * from aemstar_mwcas where chartnumber = '00000049'
...
delete from patientcase where vendorimportid is not null
