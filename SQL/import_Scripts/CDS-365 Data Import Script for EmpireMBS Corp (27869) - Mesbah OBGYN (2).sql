...
DECLARE @Practice2VendorImportID INT

SET @PracticeID2 = 2
SET @Practice2VendorImportID = 2

PRINT ''
PRINT ''
PRINT 'PracticeID = ' + CAST(@PracticeID2 AS VARCHAR(10))
PRINT 'VendorImportID = ' + CAST(@Practice2VendorImportID AS VARCHAR(10))

--DELETE FROM dbo.InsurancePolicy WHERE PracticeID = @PracticeID2 AND VendorImportID = @Practice2VendorImportID
--PRINT '    ' + CAST(@@ROWCOUNT AS VARCHAR(10)) + ' Insurance Policy records deleted'
--DELETE FROM dbo.InsuranceCompanyPlan WHERE VendorImportID = @Practice2VendorImportID
--PRINT '    ' + CAST(@@ROWCOUNT AS VARCHAR(10)) + ' Insurance Company Plan records deleted'
--DELETE FROM dbo.InsuranceCompany WHERE VendorImportID = @Practice2VendorImportID
--PRINT '    ' + CAST(@@ROWCOUNT AS VARCHAR(10)) + ' Insurance Company records deleted'
--DELETE FROM dbo.PatientCaseDate WHERE PatientCaseID IN (SELECT PatientCaseID FROM dbo.PatientCase WHERE PracticeID = @PracticeID2 AND VendorImportID = @Practice2VendorImportID)
--PRINT '    ' + CAST(@@ROWCOUNT AS VARCHAR(10)) + ' Patient Case Date records deleted'
--DELETE FROM dbo.PatientCase WHERE PracticeID = @PracticeID2 AND VendorImportID = @Practice2VendorImportID
--PRINT '    ' + CAST(@@ROWCOUNT AS VARCHAR(10)) + ' Patient Case records deleted'
--DELETE FROM dbo.PatientAlert WHERE PatientID IN (SELECT PatientID FROM dbo.Patient WHERE PracticeID = @PracticeID2 AND VendorImportID = @Practice2VendorImportID)
--PRINT '    ' + CAST(@@ROWCOUNT AS VARCHAR(10)) + ' Patient Alert records deleted'
--DELETE FROM dbo.Patient WHERE PracticeID = @PracticeID2 AND VendorImportID = @Practice2VendorImportID
--PRINT '    ' + CAST(@@ROWCOUNT AS VARCHAR(10)) + ' Patient records deleted'
--DELETE FROM dbo.Doctor WHERE PracticeID = @PracticeID2 AND VendorImportID = @Practice2VendorImportID
...
SELECT DISTINCT
		 ...
		 (CASE WHEN i.insurancetelephonerelations = '' THEN '' ELSE 'Phone Relations: ' + i.insurancetelephonerelations END) , -- Notes - text
          ...
          @PracticeID2 , -- CreatedPracticeID - int
          ...
          @Practice2VendorImportID , -- VendorImportID - int
          1 , -- NDCFormat - int
          1 , -- UseFacilityID - bit
          'U' , -- AnesthesiaType - varchar(1)
          18  -- InstitutionalBillingFormID - int
FROM dbo.[_import_2_2_InsuranceMaster] i
...
WHERE VendorImportID = @Practice2VendorImportID 
PRINT CAST(@@ROWCOUNT AS VARCHAR) + ' records inserted'
...
FROM dbo.[_import_2_2_ReferringMaster]
...
FROM dbo.[_import_2_2_MesbahOBGYN2] i
	LEFT JOIN dbo.Doctor rd ON
		i.patientreferringcode = rd.VendorID AND
		rd.VendorImportID = @Practice2VendorImportID
...
FROM dbo.[_import_2_2_MesbahOBGYN2] i
	INNER JOIN dbo.Patient p ON
		...
		p.VendorImportID = @Practice2VendorImportID
...
FROM dbo.[_import_2_2_PolicyMaster] i
	INNER JOIN dbo.PatientCase pc ON
		i.policyaccountnumber = pc.VendorID AND
		pc.VendorImportID = @Practice2VendorImportID
	INNER JOIN dbo.InsuranceCompanyPlan icp ON
		i.policyinsurancecode = icp.VendorID AND
		icp.CreatedPracticeID = @PracticeID2
...
UPDATE dbo.PatientCase 
      ...
      WHERE pc.VendorImportID = @Practice2VendorImportID AND
