...
CREATE TABLE #temppat (PatID INT)
INSERT INTO #temppat (PATID)
SELECT DISTINCT PatientID 
FROM dbo._import_1_1_Encounter 
WHERE LocationID = 2
...
INNER JOIN #temppat temppat ON 
	p.PatientID = temppat.PatID
...
SELECT DISTINCT
          ...
          CASE WHEN d.DoctorID IS NULL THEN (SELECT MIN(DoctorID) FROM dbo.Doctor WHERE [External] = 0 AND ActiveDoctor = 1 AND PracticeID = @TargetPracticeID)
...
DROP TABLE #tempdoc , #tempsl , #apptreason 
