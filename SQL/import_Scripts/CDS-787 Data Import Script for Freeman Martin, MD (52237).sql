USE superbill_52237_dev
--USE superbill_52237_prod
GO
...
INNER JOIN dbo._import_1_1_Appointment a ON atar.AppointmentID = a.AppointmentID AND a.PracticeID = @SourcePracticeID
INNER JOIN dbo._import_1_1_Patient p ON a.PatientID = p.PatientID AND a.PracticeID = @SourcePracticeID
WHERE NOT EXISTS (SELECT * FROM dbo.Patient pp WHERE pp.FirstName = p.firstname AND pp.LastName = p.lastname AND pp.DOB = p.dob AND pp.PracticeID = @TargetPracticeID) AND atar.[PracticeID] = @SourcePracticeID
...
INNER JOIN dbo.[_import_1_1_AppointmentToAppointmentReason] atar ON
	...
	atar.PracticeID = @SourcePracticeID
