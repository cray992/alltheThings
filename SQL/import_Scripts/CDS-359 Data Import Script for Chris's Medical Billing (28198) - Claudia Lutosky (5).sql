...
--	impPat.chartnumber = impCas.guarantor
--WHERE impCas.chartnumber <> '' AND pat.PracticeID = @PracticeID AND pat.VendorImportID = @VendorImportID AND 
--impCas.chartnumber = (SELECT TOP 1 impCas.chartnumber FROM dbo.[_import_24_5_CaseInformation] WHERE impCas.studentstatus <> '' AND impCas.employmentstatus = '' ORDER BY impCas.chartnumber DESC)
--PRINT CAST(@@ROWCOUNT AS VARCHAR) + ' records updated'
