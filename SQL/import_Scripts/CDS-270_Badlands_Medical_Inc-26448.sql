USE superbill_26448_dev
--USE superbill_26448_prod
GO
...
IF  EXISTS (SELECT * FROM sys.objects WHERE object_id = OBJECT_ID(N'[dbo].[_import_3_1_Insurances]') AND type in (N'U'))
BEGIN
...
IF  EXISTS (SELECT * FROM sys.objects WHERE object_id = OBJECT_ID(N'[dbo].[_import_3_1_ReferringPhys]') AND type in (N'U'))
BEGIN
...
PRINT 'Inserting into Referring Doctor...'
      ...
      SELECT DISTINCT
                    ...
                    1  -- External - bit
      FROM dbo._import_3_1_ReferringPhys AS impRph
      WHERE impRph.[firstname] <> '' AND impRph.[lastname] <> ''
PRINT CAST(@@ROWCOUNT AS VARCHAR) + ' records inserted'
END 

-- Patients (only if table exists)
IF  EXISTS (SELECT * FROM sys.objects WHERE object_id = OBJECT_ID(N'[dbo].[_import_3_1_PatientData]') AND type in (N'U'))
BEGIN
...
PRINT 'Inserting into Patient...'
      ...
      LEFT OUTER JOIN dbo._import_3_1_Providers AS impPro ON
      ...
      LEFT OUTER JOIN dbo._import_3_1_CaseInformation AS impCas ON
            impCas.[chartnumber] = impPat.[chartnumber]
      LEFT OUTER JOIN dbo._import_3_1_PatientData AS guar ON
...
IF  EXISTS (SELECT * FROM sys.objects WHERE object_id = OBJECT_ID(N'[dbo].[_import_3_1_CaseInformation]') AND type in (N'U'))
BEGIN
...
PRINT 'Inserting into Patient Case...'
      ...
      FROM dbo.Patient AS realP
		LEFT JOIN dbo.[_import_3_1_CaseInformation] AS impCas ON
...
PRINT 'Inserting into Primary Insurance Policy...'
      ...
      INNER JOIN 
            ...
            FROM dbo._import_3_1_PatientData) AS holder ON
...
IF  EXISTS (SELECT * FROM sys.objects WHERE object_id = OBJECT_ID(N'[dbo].[_import_3_1_StandardFeeSchedule]') AND type in (N'U'))
BEGIN
...
PRINT 'Inserting into Standard Fee...'
      ...
      FROM dbo._import_3_1_StandardFeeSchedule AS Impsfs
...
PRINT 'Inserting into Patient Case for Balance Forward...'
      ...
      INNER JOIN dbo._import_3_1_PatientData AS impPat ON
            realpat.VendorID = impPat.chartnumber       
      WHERE realpat.VendorImportID = @VendorImportID AND dbo.fn_RemoveNonNumericCharacters(impPat.patientremainderbalance) > '0.00'
PRINT CAST(@@ROWCOUNT AS VARCHAR) + ' records inserted'
...
PRINT 'Inserting into Encounter for Balance Forward...'
      ...
      FROM dbo._import_3_1_PatientData impPat
      ...
      INNER JOIN dbo._import_3_1_Providers AS impPro ON
               impPro.code = impPat.assignedprovider
      INNER JOIN dbo.Doctor doc ON
               ...
               doc.PracticeID = @PracticeID AND
               doc.[External] = 0
      WHERE dbo.fn_RemoveNonNumericCharacters(impPat.patientremainderbalance) > '0.00'
PRINT CAST(@@ROWCOUNT AS VARCHAR) + ' records inserted'
