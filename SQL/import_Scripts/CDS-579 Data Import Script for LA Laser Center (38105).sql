USE superbill_38105_dev
--USE superbill_38105_prod
GO


/*
DBCC CHECKIDENT('Patient', RESEED, 0)
DBCC CHECKIDENT('InsuranceCompany', RESEED, 0)
DBCC CHECKIDENT('InsuranceCompanyPlan', RESEED, 0)
DBCC CHECKIDENT('PatientCase', RESEED, 0)
DBCC CHECKIDENT('Appointment', RESEED, 0)
DBCC CHECKIDENT('AppontmenttoAppointmentReason', RESEED, 0)
DBCC CHECKIDENT('AppointmentToResource', RESEED, 0)
DBCC CHECKIDENT('AppointmentReason', RESEED, 0)
DBCC CHECKIDENT('Employers', RESEED, 0)
DBCC CHECKIDENT('InsurancePolicy', RESEED, 0)
DBCC CHECKIDENT('Doctor', RESEED, 0)
DBCC CHECKIDENT('ServiceLocation', RESEED, 0)
*/
...
/*
==========================================================================================================================================
CREATE INSURANCE COMPANIES FROM [_import_7_1_InsuranceCOMPANYPLANList]
==========================================================================================================================================
*/	
...
FROM dbo._import_7_1_InsuranceCOMPANYPLANList AS IICL
...
SELECT DISTINCT
	...
	@VendorImportID -- VendorImportID - int
FROM dbo._import_7_1_InsuranceCOMPANYPLANList ICP 
...
( 
	PracticeID , Name , AddressLine1 , AddressLine2 , City , State , ZipCode , CreatedDate , CreatedUserID , 
	ModifiedDate , ModifiedUserID , PlaceOfServiceCode , BillingName , Phone , PhoneExt , FaxPhone , FaxPhoneExt , 
	CLIANumber , RevenueCode , VendorImportID , VendorID ,	NPI , TimeZoneID , 
	PayToAddressLine1 , PayToAddressLine2 , PayToCity , PayToState , PayToCountry , PayToZipCode 
)
SELECT 
	@PracticeID , -- PracticeID - int
	CASE
		WHEN ISL.[ServiceLocationName] <> '' THEN LEFT(ISL.[ServiceLocationName],128)
		ELSE LEFT(ISL.BillingName,128) END , -- Name - varchar(128)
	LEFT(ISL.Address1,256) , -- AddressLine1 - varchar(256)
	LEFT(ISL.Address2,256) , -- AddressLine2 - varchar(256)
	LEFT(ISL.city,128) , -- City - varchar(128)
	UPPER(LEFT(ISL.[state],2)) , -- State - varchar(2)
	CASE 
		WHEN LEN(dbo.fn_RemoveNonNumericCharacters(ISL.zip)) IN (5,9) THEN dbo.fn_RemoveNonNumericCharacters(ISL.zip)
		WHEN LEN(dbo.fn_RemoveNonNumericCharacters(ISL.zip)) = 4 THEN '0' + dbo.fn_RemoveNonNumericCharacters(ISL.zip)
	...
	CASE 
		WHEN LEN(ISL.BillingName) > 1 THEN LEFT(ISL.BillingName,128)
		ELSE LEFT(ISL.[ServiceLocationName],128) END, -- BillingName - varchar(128)
	CASE
		WHEN LEN(dbo.fn_RemoveNonNumericCharacters(ISL.phone)) >= 10 THEN LEFT(dbo.fn_RemoveNonNumericCharacters(ISL.phone),10)
		ELSE '' END , -- Phone - varchar(10)
	CASE
		WHEN LEN(dbo.fn_RemoveNonNumericCharacters(ISL.phone)) > 10 THEN LEFT(SUBSTRING(dbo.fn_RemoveNonNumericCharacters(ISL.phone),11,LEN(dbo.fn_RemoveNonNumericCharacters(ISL.phone))),10)
		ELSE NULL END , -- PhoneExt - varchar(10)
	CASE
		WHEN LEN(dbo.fn_RemoveNonNumericCharacters(ISL.fax)) >= 10 THEN LEFT(dbo.fn_RemoveNonNumericCharacters(ISL.fax),10)
		ELSE '' END , -- FaxPhone - varchar(10)
	CASE
		WHEN LEN(dbo.fn_RemoveNonNumericCharacters(ISL.fax)) > 10 THEN LEFT(SUBSTRING(dbo.fn_RemoveNonNumericCharacters(ISL.fax),11,LEN(dbo.fn_RemoveNonNumericCharacters(ISL.fax))),10)
		ELSE NULL END , -- FaxPhoneExt - varchar(10)
	LEFT(ISL.clianumber,30) , -- CLIANumber - varchar(30)
	'0521' , -- RevenueCode - varchar(4)
	@VendorImportID, -- VendorImportID - int
	AutoTempID, -- VendorID - int
	CASE 
		WHEN LEN(dbo.fn_RemoveNonNumericCharacters(npi)) = 10 THEN dbo.fn_RemoveNonNumericCharacters(ISL.npi)
		ELSE NULL END , -- NPI - varchar(10)
	CASE WHEN ISL.timezone = '' THEN 5 ELSE CASE isl.timezone WHEN 'Pacific' THEN 5
															  WHEN 'Alaskan' THEN 4
															  WHEN 'Mountain' THEN 9
															  WHEN 'Central' THEN 11 
															  WHEN 'Eastern' THEN 15
															  ELSE ISL.timezone END END , -- TimeZoneID - int
...
FROM dbo._import_7_1_ServiceLocations AS ISL
	LEFT JOIN dbo.PlaceOfService AS POS ON POS.PlaceOfServiceCode = LEFT(ISL.placeofservice,2)
WHERE  ISL.billingname <> '' OR 	ISL.ServiceLocationName <> ''
PRINT CAST(@@ROWCOUNT AS VARCHAR) + ' records inserted'	
...
SELECT DISTINCT 
	...
	CASE 
		...
		ELSE NULL END -- NPI - varchar(10)
FROM dbo._import_7_1_ReferringDoctors AS RP 
...
SELECT DISTINCT 
	...
	GETDATE() , -- ModifiedDate - datetime
	0  -- ModifiedUserID - int
FROM dbo._import_7_1_PatientDemographics AS PD
...
FROM dbo._import_7_1_PatientDemographics pd
...
(	SELECT DISTINCT 
	...
	FROM dbo._import_7_1_PatientDemographics
...
DELETE FROM _import_7_1_PatientAppointments WHERE chartnumber = 'Reg Visit'
...
PRINT 'Inserting Into Appointment...'
	...
	FROM dbo._import_7_1_PatientAppointments PA
