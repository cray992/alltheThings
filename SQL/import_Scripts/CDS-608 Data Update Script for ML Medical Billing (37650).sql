...
FROM dbo.Patient p 
INNER JOIN dbo.[_import_11_6_PatientDemographics] i ON
...
FROM dbo._import_11_6_PatientDemographics AS PD
...
WHERE PD.firstname <> '' AND PD.lastname <> '' AND NOT EXISTS
(SELECT * FROM dbo.Patient p WHERE p.MedicalRecordNumber = pd.chartnumber AND p.PracticeID = @PracticeID)
...
FROM dbo._import_11_6_PatientDemographics pd
