--USE superbill_35236_dev
USE superbill_35236_prod
GO
...
CREATE TABLE #expat (sourcepatid VARCHAR(50) , targetpatid VARCHAR(50)) -- firstname VARCHAR(65), lastname VARCHAR(65), dob DATETIME)
INSERT INTO #expat ( sourcepatid , targetpatid ) -- firstname, lastname, dob )
SELECT DISTINCT
		  i.chartnumber ,-- sourcepatid - int
		  sp.id -- targetpatientid
FROM dbo.[_import_13_5_PatientDemographicsNew] i
INNER JOIN dbo.[_import_13_5_PatientDemographics] sp ON 
	i.firstname = sp.FirstName AND
    i.lastname = sp.LastName AND 
	DATEADD(hh,12,CAST(i.dateofbirth AS DATETIME)) = DATEADD(hh,12,CAST(sp.dateofbirth AS DATETIME))


PRINT ''
PRINT 'Updating Existing Patient Records with VendorID...'
UPDATE dbo.Patient 
	SET VendorID = expat.sourcepatid ,
		MedicalRecordNumber = expat.sourcepatid
FROM dbo.Patient p	
	INNER JOIN #expat expat ON 
		p.PatientID = expat.targetpatid AND
...
FROM dbo._import_13_5_InsuranceCOMPANYPLANList AS IICL
...
SELECT DISTINCT
	...
	@VendorImportID -- VendorImportID - int
FROM dbo._import_13_5_InsuranceCOMPANYPLANList ICP 
...
( 
	...
	MobilePhone , MobilePhoneExt , VendorID , VendorImportID , CollectionCategoryID , Active , SendEmailCorrespondence , 
...
FROM dbo._import_13_5_PatientDemographicsNew AS PD
	...
	LEFT JOIN dbo.Patient AS P ON p.VendorID = pd.chartnumber AND p.PracticeID = @PracticeID
...
FROM dbo._import_13_5_PatientDemographicsNew pd
...
SELECT DISTINCT
	...
	GETDATE() , -- ModifiedDate - datetime
	0  -- ModifiedUserID - int
FROM dbo._import_13_5_PatientDemographicsNew AS IPD
...
(	SELECT DISTINCT 
	...
	FROM dbo._import_13_5_PatientDemographicsNew
