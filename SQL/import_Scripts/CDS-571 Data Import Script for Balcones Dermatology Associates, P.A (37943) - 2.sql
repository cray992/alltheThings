USE superbill_37943_dev
--USE superbill_37943_prod
go
...
/*
==========================================================================================================================================
CREATE INSURANCE COMPANIES FROM [_import_5_1_InsuranceCOMPANYPLANList]
==========================================================================================================================================
*/	
...
WHERE ( insurancecompanyname <> '' OR planname <> '') AND
	NOT EXISTS (SELECT * FROM dbo.InsuranceCompany WHERE VendorID = LEFT(IICL.InsuranceCompanyName,50) AND
...
FROM dbo._import_5_1_ReferringDoctors AS RP 
...
SELECT DISTINCT
	...
	1, -- External - bit
	'' -- NPI - varchar(10)
FROM dbo._import_5_1_PatientDemographics AS PD
...
(
	EmployerName , AddressLine1 , AddressLine2 , City , State , Country ,  CreatedDate , CreatedUserID , ModifiedDate , ModifiedUserID 
)
SELECT DISTINCT 
...
WHERE PD.firstname <> '' AND PD.lastname <> '' AND NOT EXISTS (SELECT * FROM dbo.Patient p WHERE p.FirstName = pd.firstname AND p.LastName = pd.lastname AND p.DOB = DATEADD(hh,12,CAST(pd.dateofbirth AS DATETIME)))
PRINT CAST(@@ROWCOUNT AS VARCHAR) + ' records inserted'	
...
FROM dbo._import_5_1_PatientDemographics pd
...
(	SELECT DISTINCT 
	...
	FROM dbo._import_5_1_PatientDemographics
...
FROM #tempPolicies AS TP
	...
	JOIN dbo.InsuranceCompanyPlan ICP ON TP.InsuranceCode = ICP.VendorID AND ICP.VendorImportID IN (1,@VendorImportID)
PRINT CAST(@@ROWCOUNT AS VARCHAR) + ' records inserted'	
