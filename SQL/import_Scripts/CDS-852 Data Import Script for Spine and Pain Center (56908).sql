...
INNER JOIN dbo._import_4_1_BalanceForward AS m ON
   pat.VendorID = m.account AND 
   pat.VendorImportID = @VendorImportID 
WHERE m.accountbalance <> '' AND m.accountbalance > '0'
...
SELECT DISTINCT
          ...
          GETDATE() , -- ModifiedDate - datetime
          0 , -- ModifiedUserID - int
          pd.accountbalance , -- ServiceChargeAmount - money   
...
INNER JOIN dbo.ProcedureCodeDictionary AS pcd ON
   pcd.ProcedureCodeDictionaryID = (SELECT MIN(pcd2.ProcedureCodeDictionaryID) FROM dbo.ProcedureCodeDictionary pcd2 
								    WHERE pcd2.OfficialName = 'Balance Forward' AND 
										  pcd2.Active = 1)
INNER JOIN dbo.EncounterDiagnosis AS ed ON
   ed.vendorID = enc.VendorID AND 
   ed.VendorImportID = @VendorImportID   
INNER JOIN dbo._import_4_1_BalanceForward as pd ON 
    pd.account = enc.vendorID 
PRINT CAST(@@ROWCOUNT AS VARCHAR) + ' records inserted'
