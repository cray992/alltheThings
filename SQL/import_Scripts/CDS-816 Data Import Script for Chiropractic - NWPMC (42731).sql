...
-- Updating Provider Gretchen Graham with NPI from Source Doctor table
UPDATE dbo.Doctor SET NPI = '1336330208' WHERE DoctorID = 6

-- Updating Frank Park with correct LastName to match Target Doctor table
UPDATE dbo._import_1_1_Doctor SET LastName = 'Park' WHERE DoctorID = 1
...
FROM dbo.[_import_1_1_InsuranceCompanyPlan] i
	INNER JOIN dbo.InsuranceCompany ic ON 
		...
		ic.VendorImportID = @VendorImportID
WHERE i.InsuranceCompanyID IN (SELECT InsuranceCompanyPlanID FROM dbo._import_1_1_InsurancePolicy WHERE PracticeID = @SourcePracticeID)
PRINT CAST (@@ROWCOUNT AS VARCHAR(10)) + ' records inserted '
...
PRINT ''
PRINT 'Updating Patient VendorID...'
UPDATE dbo.Patient
...
INNER JOIN dbo._import_1_1_Patient i ON
	p.PatientID = (SELECT MAX(p2.patientid) FROM dbo.Patient p2 WHERE 
					p2.FirstName = i.FirstName AND 
					p2.LastName = p.LastName AND 
					p2.DOB = i.DOB AND
					p2.PracticeID = @TargetPracticeID AND
					p2.Active = 1)
WHERE i.Active = 1 AND i.PracticeID = @SourcePracticeID
PRINT CAST(@@ROWCOUNT AS VARCHAR) + ' records updated'
...
WHERE p.PracticeID = @SourcePracticeID AND NOT EXISTS (SELECT * FROM dbo.Patient pp WHERE pp.FirstName = p.firstname AND pp.LastName = p.lastname AND pp.DOB = p.dob AND pp.PracticeID = @TargetPracticeID) 
PRINT CAST(@@ROWCOUNT AS VARCHAR) + ' records inserted'
