...
select *
into contractFeeSchedule_20100110
from contractFeeSchedule

delete contractFeeSchedule
where vendorImportID=25
...
update dbo.zz_ContractFee_20100110_01
set Notes=nullif(Notes, '')
	,Modifier=nullif(Modifier,'')

select * from vendorImport
insert into vendorImport( VendorName, VendorFormat, DateCreated, Notes)
values( 'Custom', 'xls', getdate(), 'sf 99434, part3 ')

alter table dbo.zz_ContractFee_20100110_01 add rowid int identity(1,1) primary key clustered
alter table dbo.zz_ContractFee_20100110_02 add rowid int identity(1,1) primary key clustered
alter table dbo.zz_ContractFee_20100110_03 add rowid int identity(1,1) primary key clustered
...
select @contractID=18, @vendorImportID=25
...
select distinct 
	...
	,[Procedure Code]
	,@VendorImportID
from dbo.zz_ContractFee_20100110_01 i
...
select 
	...
	,rowID
	,Modifier
from zz_ContractFee_20100110_01 i
	left join dbo.ProcedureCodeDictionary pcd
		on pcd.ProcedureCode=i.[Procedure Code]
commit;
...
update dbo.zz_ContractFee_20100110_02
set Notes=nullif(Notes, '')
	,Modifier=nullif(Modifier,'')
...
declare @contractID int, @vendorImportID int
select @contractID=17, @vendorImportID=25
...
select distinct 
	...
	,[Procedure Code]
	,@VendorImportID
from dbo.zz_ContractFee_20100110_02 i
	left join dbo.ProcedureCodeDictionary pcd
		on pcd.ProcedureCode=i.[Procedure Code]
where pcd.ProcedureCodeDictionaryID is null
...
select 
	...
	,rowID
	,Modifier
from zz_ContractFee_20100110_02 i
	left join dbo.ProcedureCodeDictionary pcd
		on pcd.ProcedureCode=i.[Procedure Code]
-- rollback tran
commit;
...
declare @contractID int, @vendorImportID int
select @contractID=19, @vendorImportID=25
...
select distinct 
	...
	,[Procedure Code]
	,@VendorImportID
from dbo.zz_ContractFee_20100110_03 i
	left join dbo.ProcedureCodeDictionary pcd
		on pcd.ProcedureCode=i.[Procedure Code]
where pcd.ProcedureCodeDictionaryID is null
...
select 
	...
	,rowID
	,Modifier
from zz_ContractFee_20100110_03 i
	left join dbo.ProcedureCodeDictionary pcd
		on pcd.ProcedureCode=i.[Procedure Code]
