...
SELECT DISTINCT
			 ...
			 i.ssn  -- SSN - varchar(9)
FROM dbo.Patient p 
INNER JOIN dbo.[_import_2_1_PatientDemographicsSource] i ON p.PatientID = i.id
...
SELECT DISTINCT
			 ...
			 i.ssn  -- SSN - varchar(9)
FROM dbo.Patient p 
INNER JOIN dbo._import_2_1_PatientDemographicsTarget i ON p.PatientID = i.id
...
FROM dbo.[AppointmentReason] i
	INNER JOIN dbo.AppointmentToAppointmentReason atar ON 
		i.AppointmentReasonID = atar.AppointmentReasonID 
	INNER JOIN dbo.Appointment a ON 
		atar.AppointmentID = a.AppointmentID
	INNER JOIN dbo.Patient p ON 
		a.PatientID = p.VendorID AND
        p.VendorImportID = @VendorImportID
