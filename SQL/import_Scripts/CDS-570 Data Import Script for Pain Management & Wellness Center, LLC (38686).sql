...
INNER JOIN dbo.[_import_1_1_Patientsc902c1ba0873429ebf9] AS m ON
   pat.FirstName = m.patientfirstname AND 
   pat.LastName = m.patientlastname AND
   pat.DOB = DATEADD(hh,12,CAST(m.dob AS DATETIME)) 
WHERE m.patientbalance > '0'
...
SELECT DISTINCT
          ...
          0 , -- PaymentTypeID - int
          pc.PatientID , -- VendorID - varchar(50)                
          @VendorImportID , -- VendorImportID - int
...
SELECT DISTINCT
          ...
          GETDATE() , -- ModifiedDate - datetime
          0 , -- ModifiedUserID - int
          m.patientbalance , -- ServiceChargeAmount - money   
...
INNER JOIN dbo.Patient PAT ON
	ed.VendorID = PAT.PatientID AND
	ed.[PracticeID] = @PracticeID 
INNER JOIN dbo.[_import_1_1_Patientsc902c1ba0873429ebf9] AS m ON
	pat.FirstName = m.patientfirstname AND 
	pat.LastName = m.patientlastname AND
	pat.DOB = DATEADD(hh,12,CAST(m.dob AS DATETIME)) 
