...
--	p.id = i.patientid
--INNER JOIN dbo._import_1_1_UseThesePatientIDs sp ON 
--	i.PatientID = sp.PatID AND
--    p.id = sp.PatID
...
FROM dbo.[_import_1_1_CollectionCategory] i 
	INNER JOIN dbo._import_1_1_Patient p ON 
		i.CollectionCategoryID = p.CollectionCategoryID
	INNER JOIN dbo._import_1_1_UseThesePatientIDs sp ON 
		p.PatientID = sp.PatID
...
DROP TABLE #tempdoc , #tempemp , #tempsl , #apptreason 
