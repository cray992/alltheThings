...
FROM dbo._import_12_8_Case AS ins
...
SELECT    
	...
	LEFT(patsuffix, 16) , -- Suffix - varchar(16)
...
FROM dbo.[_import_12_8_PatientDemographics] 
...
SELECT DISTINCT
	...
	@VendorImportID -- VendorImportID - int
FROM dbo.[_import_12_8_Case] impCase
...
SELECT  distinct  
	...
	@VendorImportID -- VendorImportID - int
FROM dbo.[_import_12_8_Case] AS impCase
INNER JOIN
	(
		SELECT impPolicy.acctid, impPolicy.acctplan, impPolicy.pataltlookup FROM _import_12_8_InsuredID impPolicy
		INNER JOIN
			(
				SELECT acctid, acctplan, MAX(AutoTempID) AS LatestPolicyRowID FROM dbo.[_import_12_8_InsuredID] GROUP BY acctid, acctplan
...
INNER JOIN dbo.[_import_12_8_GuarantorList] AS impGuarantor ON
...
FROM dbo.[_import_12_8_FeeSchedule] impSFS
