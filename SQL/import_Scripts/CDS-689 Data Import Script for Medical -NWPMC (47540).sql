--USE superbill_47540_dev
USE superbill_47540_prod
GO
...
/*
CREATE TABLE #tempdocqa (firstname VARCHAR(65), lastname VARCHAR(65), NPI INT) INSERT INTO #tempdocqa (firstname, lastname, NPI )
SELECT DISTINCT d.firstname ,d.lastname, d.npi FROM dbo._import_1_1_Doctor d WHERE d.PracticeID = @SourcePracticeID

SELECT COUNT(*) AS [Existing Doctors To Be Updated] FROM dbo.Doctor d 
INNER JOIN #tempdocqa i ON d.FirstName = i.firstname AND d.LastName = i.lastname AND d.NPI = i.npi
WHERE d.PracticeID = @TargetPracticeID DROP TABLE #tempdocqa
...
INNER JOIN _import_1_1_InsuranceCompany ic ON icp.insurancecompanyid = ic.insurancecompanyid
INNER JOIN dbo.[_import_1_1_InsurancePolicy] ip ON icp.InsuranceCompanyPlanID = ip.InsuranceCompanyPlanID AND ip.PracticeID = @SourcePracticeID


SELECT COUNT(*) AS [Referring Provider Records To Be Inserted] FROM dbo._import_1_1_Doctor d 
WHERE NOT EXISTS(SELECT * FROM dbo.Doctor d2 WHERE d.FirstName = d2.FirstName AND d.LastName = d2.LastName AND d.NPI = d2.NPI AND d2.[External] = 1 AND d2.PracticeID = @SourcePracticeID)
...
--UPDATE _import_1_1_Doctor SET lastname = 'Parker' WHERE LastName = 'Park'
--UPDATE _import_1_1_Doctor SET npi = 1336330208 WHERE FirstName = 'GRETCHEN' AND LastName = 'GRAHAM'
--UPDATE _import_1_1_Doctor SET npi = 1063496875 WHERE FirstName = 'William' AND LastName = 'Baxter'

CREATE TABLE #tempdoc (doctorid INT, firstname VARCHAR(65), lastname VARCHAR(65), NPI INT)
INSERT INTO #tempdoc (doctorid, firstname, lastname, NPI )
SELECT DISTINCT
		  d.doctorid ,
...
WHERE d.PracticeID = @SourcePracticeID AND d.[External] = 0
...
FROM dbo.[_import_1_1_Appointment] i
	...
	LEFT JOIN dbo.[_import_1_1_AppointmentToResource] atr ON 
		i.AppointmentID = atr.AppointmentID AND 
		atr.PracticeID = @SourcePracticeID
	LEFT JOIN dbo.Doctor d ON 
		atr.ResourceID = d.VendorID AND 
		d.PracticeID = @TargetPracticeID
WHERE i.PracticeID = @SourcePracticeID  AND d.DoctorID IS NOT NULL
PRINT CAST(@@ROWCOUNT AS VARCHAR) + ' records inserted'
...
SELECT DISTINCT
          ...
          CASE WHEN d.DoctorID IS NULL THEN 1 ELSE d.DoctorID END , -- ResourceID - int
          GETDATE() , -- ModifiedDate - datetime
          @TargetPracticeID  -- PracticeID - int
