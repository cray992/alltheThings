USE superbill_31601_dev
--USE superbill_31601_prod
go
...
AND NOT EXISTS (SELECT * FROM dbo.Patient p WHERE p.PracticeID = @PracticeID AND
													p.FirstName = imppat.firstname AND
													p.LastName = imppat.lastname AND
													p.DOB = DATEADD(hh,12,CAST(imppat.dateofbirth AS DATETIME)))
PRINT CAST(@@ROWCOUNT AS VARCHAR) + ' records inserted'
...
INNER JOIN dbo.[_import_1_1_CaseInformation] impCas ON
	impCas.chartnumber = pat.VendorID
INNER JOIN dbo.[_import_1_1_PatientData] impPat ON
...
impCas.chartnumber = (SELECT TOP 1 impCas.chartnumber FROM dbo.[_import_1_1_CaseInformation] WHERE
...
INNER JOIN dbo.[_import_1_1_PatientData] impPat ON
	impPat.chartnumber = impCas.guarantor
WHERE impCas.chartnumber <> '' AND pat.PracticeID = @PracticeID AND pat.VendorImportID = @VendorImportID AND 
impCas.chartnumber = (SELECT TOP 1 impCas.chartnumber FROM dbo.[_import_1_1_CaseInformation] WHERE impCas.studentstatus <> '' AND impCas.employmentstatus = '' ORDER BY impCas.chartnumber DESC)
PRINT CAST(@@ROWCOUNT AS VARCHAR) + ' records updated'
