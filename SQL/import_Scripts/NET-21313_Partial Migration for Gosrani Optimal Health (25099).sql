...
--ALTER TABLE dbo._import_11_2_CaseInformation ADD planname VARCHAR(50)
...
--ALTER TABLE dbo._import_12_2_Appointments ADD plastname VARCHAR(30),pfirstname VARCHAR(30)
--ALTER TABLE dbo._import_12_2_Appointments ADD appointmentreason VARCHAR(30)
...
SELECT ic.insurancecompanyid,
       ...
       0                               -- IsEnrollable - bit
--select * 
FROM dbo._import_11_2_InsuranceCompany ic
...
--SELECT icp.planname,* 
FROM dbo._import_11_2_CaseInformation ip
    INNER JOIN _import_11_2_InsuranceCompanyPlan icp
...
SELECT icp.planname,          -- PlanName - varchar(128)
       ...
       @VendorImportID        -- VendorImportID - int
--select * 
FROM dbo._import_11_2_InsuranceCompanyPlan icp
...
SELECT DISTINCT
    ...
    null, --r.Relationship,             -- ResponsibleRelationshipToPatient - varchar(1)
    ...
    NULL, --i.primarycarephysicianid,   -- PrimaryProviderID - int
    NULL,  --i.defaultservicelocationid, -- DefaultServiceLocationID - int
    NULL,                       -- EmployerID - int
    ...
    i.active,                   -- Active - bit
    i.sendemailnotifications,   -- SendEmailCorrespondence - bit
...
FROM dbo._import_11_2_PatientDemographics i
...
FROM dbo._import_11_2_PatientDemographics i
    INNER JOIN Patient p
        ON p.FirstName = i.firstname
           AND p.LastName = i.lastname
    INNER JOIN dbo._import_11_2_CaseInformation ci
...
--select * 
FROM dbo._import_11_2_Providers pr;
PRINT CAST(@@ROWCOUNT AS VARCHAR) + ' records updated';
...
FROM dbo._import_11_2_PatientDemographics i
    INNER JOIN dbo.Patient p
        ON p.FirstName = i.firstname
           AND p.LastName = i.lastname
    --AND p.DOB=i.dateofbirth
	AND p.AddressLine1=i.addressline1
    INNER JOIN dbo._import_11_2_CaseInformation ci
        ON ci.patientid = i.id
    INNER JOIN dbo._import_11_2_InsuranceCompanyPlan icp
        ON icp.planname = ci.planname
    INNER JOIN dbo.InsuranceCompanyPlan pl
        ON pl.PlanName = icp.planname
           AND pl.CreatedPracticeID = 2
    INNER JOIN dbo.PatientCase pc
        ON pc.PatientID = p.PatientID
    --INNER JOIN dbo.Relationship r
    --    ON r.LongName = i.guarantorrelationshiptopatient;
WHERE ci.primaryinsurancepolicynumber <>''
PRINT CAST(@@ROWCOUNT AS VARCHAR) + ' records updated';
...
FROM dbo._import_11_2_PatientDemographics i
    ...
    INNER JOIN dbo._import_11_2_InsuranceCompanyPlan icp
        ON icp.planname = ci.planname
    INNER JOIN dbo.InsuranceCompanyPlan pl
        ON pl.PlanName = icp.planname-- WHERE p.PatientID=465
           AND pl.CreatedPracticeID = 2
    INNER JOIN dbo.PatientCase pc
        ON pc.PatientID = p.PatientID
    --INNER JOIN dbo.Relationship r
...
--SELECT * 
FROM _import_12_2_appointments i
    INNER JOIN dbo.Patient p
...
SELECT DISTINCT
    ...
    @VendorImportID
--SELECT * 
FROM dbo._import_12_2_appointments i
    INNER JOIN dbo.Patient p
    ...
    INNER JOIN dbo.Doctor d
        ON d.LastName = i.doclastname
           AND d.FirstName = i.docfirstname
           AND d.PracticeID = 2
...
SELECT DISTINCT
    @TargetPracticeID,         -- PracticeID - int
    LEFT(ip.appointmentreason1,128),                   -- Name - varchar(128)
    ...
    @VendorImportID
--SELECT * 
FROM dbo._import_12_2_Appointments ip
...
--SELECT *
FROM dbo._import_12_2_appointments imp
    INNER JOIN dbo.Patient p
    ...
    INNER JOIN dbo.AppointmentReason ar
        ON ar.Name = imp.appointmentreason1
WHERE p.PracticeID = @TargetPracticeID
...
--    @vendorimportid
--   --select *  
--FROM superbill_25099_prod.dbo.ContractsAndFees_StandardFeeSchedule fs
...
--)
--SELECT 
--    (SELECT sf.StandardFeeScheduleID FROM dbo.ContractsAndFees_StandardFeeSchedule a WHERE a.Name='Standard Fees'),    -- StandardFeeScheduleID - int
...
----select * 
--FROM superbill_25099_dev.dbo.ContractsAndFees_StandardFee sf 
...
--    (SELECT a.StandardFeeScheduleID FROM dbo.ContractsAndFees_StandardFeeSchedule a WHERE a.Name='Standard Fees'),  -- StandardFeeScheduleID - int
--    @VendorImportID
