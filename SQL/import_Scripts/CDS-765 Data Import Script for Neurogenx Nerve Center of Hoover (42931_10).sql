...
UPDATE dbo.[_import_3_10_Doctor] SET NPI = '1841221413' WHERE DoctorID = 1
...
SELECT DISTINCT d.firstname ,d.lastname, d.npi, d.[External] FROM dbo._import_3_10_Doctor d WHERE d.PracticeID = @SourcePracticeID
...
SELECT DISTINCT COUNT(DISTINCT i.insurancecompanyid) AS [Insurance Company Records To Be Inserted] FROM _import_3_10_InsuranceCompany i
INNER JOIN dbo.[_import_3_10_InsuranceCompanyPlan] icp ON i.InsuranceCompanyID = icp.InsuranceCompanyID
INNER JOIN dbo.[_import_3_10_InsurancePolicy] ip ON icp.InsuranceCompanyPlanID = ip.InsuranceCompanyPlanID AND ip.PracticeID = @SourcePracticeID

SELECT COUNT(DISTINCT ptic.pk_id) AS [Practice to Insurance Company Records To Be Inserted] FROM _import_3_10_PracticetoInsuranceCompany ptic 
INNER JOIN dbo._import_3_10_InsuranceCompany ic ON ptic.InsuranceCompanyId = ic.InsuranceCompanyID AND ptic.PracticeID = @SourcePracticeID
INNER JOIN dbo.[_import_3_10_InsuranceCompanyPlan] icp ON ic.InsuranceCompanyID = icp.InsuranceCompanyID
INNER JOIN dbo.[_import_3_10_InsurancePolicy] ip ON icp.InsuranceCompanyPlanID = ip.InsuranceCompanyPlanID AND ip.PracticeID = @SourcePracticeID

SELECT COUNT(DISTINCT icp.insurancecompanyplanid) AS [Insurance Company Plan Records To Be Inserted] FROM _import_3_10_InsuranceCompanyPlan icp
INNER JOIN dbo._import_3_10_InsuranceCompany ic ON icp.insurancecompanyid = ic.insurancecompanyid
INNER JOIN dbo.[_import_3_10_InsurancePolicy] ip ON icp.InsuranceCompanyPlanID = ip.InsuranceCompanyPlanID AND ip.PracticeID = @SourcePracticeID
...
SELECT COUNT(*) AS [Referring Provider Records To Be Inserted] FROM dbo._import_3_10_Doctor d 
...
SELECT COUNT(*) AS [Patients To Be Inserted] FROM dbo._import_3_10_Patient p WHERE p.PracticeID = @SourcePracticeID 
...
SELECT COUNT(*) AS [Patient Alert Records To Be Inserted] FROM dbo._import_3_10_PatientAlert pa 
INNER JOIN dbo._import_3_10_Patient p ON pa.PatientID = p.PatientID AND p.PracticeID = @SourcePracticeID
...
SELECT COUNT(*) AS [Patient Journal Records To Be Inserted] FROM dbo._import_3_10_PatientJournalNote pjn
INNER JOIN dbo._import_3_10_Patient p ON pjn.PatientID = p.PatientID AND p.PracticeID = @SourcePracticeID
WHERE NOT EXISTS (SELECT * FROM dbo.Patient pp WHERE pp.FirstName = p.firstname AND pp.LastName = p.lastname AND pp.DOB = p.dob AND pp.PracticeID = @TargetPracticeID) 

SELECT COUNT(*) AS [Patient Case Records To Be Inserted] FROM dbo._import_3_10_PatientCase pc 
INNER JOIN dbo._import_3_10_Patient p ON pc.PatientID = p.PatientID AND pc.PracticeID = @SourcePracticeID
WHERE NOT EXISTS (SELECT * FROM dbo.Patient pp WHERE pp.FirstName = p.firstname AND pp.LastName = p.lastname AND pp.DOB = p.dob AND pp.PracticeID = @TargetPracticeID) 

SELECT COUNT(*) AS [Patient Case Date Records To Be Inserted] FROM dbo._import_3_10_PatientCaseDate pcd 
INNER JOIN dbo._import_3_10_PatientCase pc ON pcd.PatientCaseID = pc.PatientCaseID AND pc.PracticeID = @SourcePracticeID
INNER JOIN dbo._import_3_10_Patient p ON pc.PatientID = p.PatientID AND p.PracticeID = @SourcePracticeID
WHERE NOT EXISTS (SELECT * FROM dbo.Patient pp WHERE pp.FirstName = p.firstname AND pp.LastName = p.lastname AND pp.DOB = p.dob AND pp.PracticeID = @TargetPracticeID) 

SELECT COUNT(*) AS [Insurance Policy Records To Be Inserted] FROM dbo._import_3_10_InsurancePolicy ip
INNER JOIN dbo._import_3_10_PatientCase pc ON ip.patientcaseid = pc.PatientCaseID AND pc.PracticeID = @SourcePracticeID
INNER JOIN dbo._import_3_10_Patient p ON pc.PatientID = p.PatientID AND p.PracticeID = @SourcePracticeID
WHERE NOT EXISTS (SELECT * FROM dbo.Patient pp WHERE pp.FirstName = p.firstname AND pp.LastName = p.lastname AND pp.DOB = p.dob AND pp.PracticeID = @TargetPracticeID) 

SELECT COUNT(*) AS [Insurance Policy Authorization Records To Be Inserted] FROM dbo._import_3_10_InsurancePolicyAuthorization ipa 
INNER JOIN dbo._import_3_10_InsurancePolicy ip ON ipa.InsurancePolicyID = ip.InsurancePolicyID AND ip.PracticeID = @SourcePracticeID
INNER JOIN dbo._import_3_10_PatientCase pc ON ip.PatientCaseID = pc.PatientCaseID AND pc.PracticeID = @SourcePracticeID
INNER JOIN dbo._import_3_10_Patient p ON pc.PatientID = p.PatientID AND p.PracticeID = @SourcePracticeID
WHERE NOT EXISTS (SELECT * FROM dbo.Patient pp WHERE pp.FirstName = p.firstname AND pp.LastName = p.lastname AND pp.DOB = p.dob AND pp.PracticeID = @TargetPracticeID) 

SELECT COUNT(*) AS [Appointment Records To Be Inserted] FROM dbo._import_3_10_Appointment a
INNER JOIN dbo._import_3_10_Patient p ON a.PatientID = p.PatientID AND a.PracticeID = @SourcePracticeID
WHERE NOT EXISTS (SELECT * FROM dbo.Patient pp WHERE pp.FirstName = p.firstname AND pp.LastName = p.lastname AND pp.DOB = p.dob AND pp.PracticeID = @TargetPracticeID) 

SELECT COUNT(*) AS [Appointment Reason Records To Be Inserted] FROM dbo._import_3_10_AppointmentReason ars
...
SELECT COUNT(*) AS [Practice Resource Records To Be Inserted] FROM dbo._import_3_10_PracticeResource prs 
...
SELECT COUNT(*) AS [Appointment to Appointment Reason Records To Be Inserted] FROM dbo._import_3_10_AppointmentToAppointmentReason atar 
INNER JOIN dbo._import_3_10_Appointment a ON atar.AppointmentID = a.AppointmentID
INNER JOIN dbo._import_3_10_Patient p ON a.PatientID = p.PatientID AND a.PracticeID = @SourcePracticeID
WHERE NOT EXISTS (SELECT * FROM dbo.Patient pp WHERE pp.FirstName = p.firstname AND pp.LastName = p.lastname AND pp.DOB = p.dob AND pp.PracticeID = @TargetPracticeID) 

SELECT COUNT(*) AS [Appointment to Resource - Doctor Resource Records To Be Inserted] FROM dbo._import_3_10_AppointmentToResource atr
INNER JOIN dbo._import_3_10_Appointment a ON atr.AppointmentID = a.AppointmentID AND a.PracticeID = @SourcePracticeID
INNER JOIN dbo._import_3_10_Patient p ON a.PatientID = p.PatientID AND a.PracticeID = @SourcePracticeID
WHERE NOT EXISTS (SELECT * FROM dbo.Patient pp WHERE pp.FirstName = p.firstname AND pp.LastName = p.lastname AND pp.DOB = p.dob AND pp.PracticeID = @TargetPracticeID) 
AND atr.AppointmentResourceTypeID = 1 AND atr.PracticeID = @SourcePracticeID

SELECT COUNT(*) AS [Appointment to Resource - Practice Resource Records To Be Inserted] FROM dbo._import_3_10_AppointmentToResource atr
INNER JOIN dbo._import_3_10_Appointment a ON atr.AppointmentID = a.AppointmentID AND a.PracticeID = @SourcePracticeID
INNER JOIN dbo._import_3_10_Patient p ON a.PatientID = p.PatientID AND a.PracticeID = @SourcePracticeID
WHERE NOT EXISTS (SELECT * FROM dbo.Patient pp WHERE pp.FirstName = p.firstname AND pp.LastName = p.lastname AND pp.DOB = p.dob AND pp.PracticeID = @TargetPracticeID) 
AND atr.AppointmentResourceTypeID = 2 AND atr.PracticeID = @SourcePracticeID

SELECT COUNT(*) AS [Appointment Recurrence Records To Be Inserted] FROM dbo._import_3_10_AppointmentRecurrence ar 
INNER JOIN dbo._import_3_10_Appointment a ON ar.AppointmentID = a.AppointmentID AND a.PracticeID = @SourcePracticeID
INNER JOIN dbo._import_3_10_Patient p ON a.PatientID = p.PatientID AND a.PracticeID = @SourcePracticeID
WHERE NOT EXISTS (SELECT * FROM dbo.Patient pp WHERE pp.FirstName = p.firstname AND pp.LastName = p.lastname AND pp.DOB = p.dob AND pp.PracticeID = @TargetPracticeID) 

SELECT COUNT(*) AS [Appointment Recurrence Exception Records To Be Inserted] FROM dbo._import_3_10_AppointmentRecurrenceException are 
INNER JOIN dbo._import_3_10_Appointment a ON are.AppointmentID = a.AppointmentID AND a.PracticeID = @SourcePracticeID
INNER JOIN dbo._import_3_10_Patient p ON a.PatientID = p.PatientID AND a.PracticeID = @SourcePracticeID
WHERE NOT EXISTS (SELECT * FROM dbo.Patient pp WHERE pp.FirstName = p.firstname AND pp.LastName = p.lastname AND pp.DOB = p.dob AND pp.PracticeID = @TargetPracticeID) 
...
SELECT DISTINCT
          ...
          d.npi ,  -- NPI - int
		  d.[External]
FROM dbo._import_3_10_Doctor d 
...
FROM dbo.[_import_3_10_Employers] e 
...
INNER JOIN dbo._import_3_10_ServiceLocation osl ON
...
FROM dbo.[_import_3_10_Patient] p
...
FROM dbo.[_import_3_10_Appointment] i
...
SELECT DISTINCT
          ...
          GETDATE() , -- ModifiedDate - datetime
          GETDATE()  -- CreatedDate - datetime
FROM dbo.[_import_3_10_PracticeResource] i
...
SELECT DISTINCT
          ...
          GETDATE()  -- ModifiedDate - datetime
FROM dbo.[_import_3_10_AppointmentReason] i
...
SELECT DISTINCT
	atar.AppointmentID ,
	ar.Name
FROM dbo.[_import_3_10_AppointmentReason] ar
INNER JOIN dbo.[_import_3_10_AppointmentToAppointmentReason] atar ON
	ar.AppointmentReasonID = atar.AppointmentReasonID AND
	ar.PracticeID = @SourcePracticeID
INNER JOIN dbo.[_import_3_10_Appointment] a ON 
...
SELECT DISTINCT
	atr.AppointmentID ,
	pr.ResourceName
FROM dbo.[_import_3_10_AppointmentToResource] atr
INNER JOIN dbo.[_import_3_10_PracticeResource] pr ON
...
SELECT DISTINCT
          ...
          CASE 
			WHEN d.DoctorID IS NULL THEN (SELECT DoctorID FROM dbo.Doctor WHERE FirstName = 'David' AND LastName = 'Dowell' AND PracticeID = @TargetPracticeID AND [External] = 0) 
		  ELSE d.DoctorID END, -- ResourceID - int
          GETDATE() , -- ModifiedDate - datetime
          @TargetPracticeID  -- PracticeID - int
FROM dbo.[_import_3_10_AppointmentToResource] i
	INNER JOIN dbo.Appointment a ON
