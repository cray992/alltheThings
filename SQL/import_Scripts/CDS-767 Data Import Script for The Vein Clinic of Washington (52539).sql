--USE superbill_52539_dev
USE superbill_52539_prod
GO
...
FROM dbo.[_import_1_1_Doctor] i WHERE i.DoctorID = 12
PRINT CAST(@@ROWCOUNT AS VARCHAR) + ' records updated'
*/

-- selecting patient ids where only assigned to Doctor Resource Samir Neimat DoctorID 12
CREATE TABLE #importpats (SourcePatientID INT)
INSERT INTO #importpats ( SourcePatientID )
SELECT DISTINCT	a.PatientID 
FROM dbo.[_import_1_1_Appointment] a
INNER JOIN dbo._import_1_1_AppointmentToResource atr ON a.AppointmentID = atr.AppointmentID AND atr.ResourceID = 12
INNER JOIN dbo._import_1_1_Patient p ON a.PatientID = p.PatientID AND p.PracticeID = @SourcePracticeID        
...
FROM dbo.[_import_1_1_InsuranceCompany] i
	...
	INNER JOIN dbo._import_1_1_PatientCase pc ON
		ip.PatientCaseID = pc.PatientCaseID AND 
        pc.PracticeID = @SourcePracticeID
	INNER JOIN #importpats ipats ON	
		pc.PatientID = ipats.SourcePatientID
...
FROM dbo.[_import_1_1_Employers] i
	...
	INNER JOIN #importpats ipats ON 
		p.PatientID = ipats.SourcePatientID
...
SELECT DISTINCT
          ...
          CASE WHEN d.DoctorID IS NULL THEN (SELECT DoctorID FROM dbo.Doctor WHERE FirstName = 'Samir' AND LastName = 'Neimat' AND PracticeID = @TargetPracticeID AND [External] = 0)
