...
SELECT  DISTINCT @VendorImportID, pat.ChartNumber, @PracticeID, 		-- pg.PatientChartNumber, pg.GuarantorChartNumber, 
	...
	CASE WHEN (pat.DateOfBirth IS NOT NULL AND LTRIM(RTRIM(pat.DateOfBirth)) <> '') THEN CAST(LTRIM(RTRIM(pat.DateOfBirth)) AS DATETIME) ELSE NULL END, 
...
-- and loop through the rows and insert each into patientcase wiht this patientid
	...
	SELECT CaseNumber, @VendorImportID, @PracticeID, @PatientID, ChartNumber, Description, Guarantor, -- replace all 'None' from groupnumber with ''
		Insured1, InsuredRelationship1, InsuranceCarrier1, 
		PolicyNumber1, 
		CASE WHEN UPPER(LTRIM(RTRIM(GroupNumber1)))= 'NONE' THEN '' ELSE GroupNumber1 END, 
		CASE WHEN LTRIM(RTRIM(Policy1StartDate)) = '' THEN NULL ELSE LTRIM(RTRIM(Policy1StartDate)) END, 
		CASE WHEN LTRIM(RTRIM(Policy1EndDate)) = '' THEN NULL ELSE LTRIM(RTRIM(Policy1EndDate)) END, 
		Insured2, InsuredRelationship2, InsuranceCarrier2, 
		PolicyNumber2, CASE WHEN UPPER(LTRIM(RTRIM(GroupNumber2)))= 'NONE' THEN '' ELSE GroupNumber2 END, 
		CASE WHEN LTRIM(RTRIM(Policy2StartDate)) = '' THEN NULL ELSE LTRIM(RTRIM(Policy2StartDate)) END, 
		CASE WHEN LTRIM(RTRIM(Policy2EndDate)) = '' THEN NULL ELSE LTRIM(RTRIM(Policy2EndDate)) END, 
		Insured3 , InsuredRelationship3, InsuranceCarrier3, 
		PolicyNumber3, CASE WHEN UPPER(LTRIM(RTRIM(GroupNumber3)))= 'NONE' THEN '' ELSE GroupNumber3 END, 
		CASE WHEN LTRIM(RTRIM(Policy3StartDate)) = '' THEN NULL ELSE LTRIM(RTRIM(Policy3StartDate)) END, 
		CASE WHEN LTRIM(RTRIM(Policy3EndDate)) = '' THEN NULL ELSE LTRIM(RTRIM(Policy3EndDate)) END,
		RelatedToEmployment, RelatedToAccident, DateOfInjuryIllness, DateFirstConsulted,
		DateUnableToWorkFrom, DateUnableToWorkTo, DateTotDisabilityFrom, DateTotDisabilityTo,
		AccidentState, DateSimilarSymptoms, ReferringProvider, Notes
	FROM HCPWS_MWCAS 
