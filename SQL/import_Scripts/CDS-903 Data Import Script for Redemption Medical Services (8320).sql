...
FROM dbo.[_import_4_5_Insurances]
...
SELECT DISTINCT
     ...
     @VendorImportID  -- VendorImportID - int
 FROM dbo.[_import_4_5_Insurances] AS impIns
...
SELECT DISTINCT
		...
		1  -- External - bit
	FROM dbo.[_import_4_5_ReferringPhys] AS impRph
...
FROM dbo.[_import_4_5_CaseInformation] AS ci
	LEFT JOIN dbo.[_import_4_5_CaseInformation] AS ci2 ON
		ci.chartnumber = ci2.chartnumber AND CAST(ci.casenumber AS INT) < CAST(ci2.casenumber AS INT)
WHERE ci2.chartnumber IS NULL 

PRINT ''
PRINT 'Updating Existing Patients - VendorID'
UPDATE p 
	SET VendorID = i.chartnumber , 
		VendorImportID = @VendorImportID
FROM dbo.Patient p
INNER JOIN dbo._import_4_5_PatientData i ON 
...
FROM dbo.[_import_4_5_PatientData] AS impPat
	LEFT JOIN dbo.[_import_4_5_Providers] AS impPro ON
	...
	LEFT JOIN dbo.[_import_4_5_PatientData] AS gua ON
		tci.[guarantor] = gua.[chartnumber]
	LEFT JOIN dbo.Patient p ON 
		imppat.chartnumber = p.VendorID AND 
...
FROM dbo.[_import_4_5_CaseInformation] AS impCas
	...
	LEFT JOIN dbo.Doctor AS realdoc ON
		...
		realdoc.[External] = 1
	LEFT JOIN dbo.InsuranceCompanyPlan AS realicp1 ON
		realicp1.[VendorID] = impCas.[insurancecarrier1] AND
		realicp1.[VendorImportID] = @VendorImportID AND
		realicp1.[CreatedPracticeID] = @PracticeID
	LEFT JOIN dbo.InsuranceCompanyPlan AS realicp2 ON
		realicp2.[VendorID] = impCas.[insurancecarrier1] AND
		realicp2.[VendorImportID] = @VendorImportID AND
		realicp2.[CreatedPracticeID] = @PracticeID
	LEFT JOIN dbo.InsuranceCompanyPlan AS realicp3 ON
		realicp3.[VendorID] = impCas.[insurancecarrier1] AND
		realicp3.[VendorImportID] = @VendorImportID AND
		realicp3.[CreatedPracticeID] = @PracticeID
WHERE impCas.chartnumber <> '' AND (
									realicp1.InsuranceCompanyPlanID IS NOT NULL OR 
									realicp2.InsuranceCompanyPlanID IS NOT NULL OR
									realicp3.InsuranceCompanyID IS NOT NULL
								   )
PRINT CAST(@@ROWCOUNT AS VARCHAR) + ' records inserted'
...
FROM dbo.[_import_4_5_CaseInformation] AS impCas
	...
	INNER JOIN 
		...
		FROM dbo.[_import_4_5_PatientData]) holder ON
...
FROM dbo.[_import_4_5_CaseInformation] AS impCas
	...
	INNER JOIN 
		(SELECT DISTINCT [chartnumber] , [firstname] , [middleinitial] , [lastname] , [street1] , [street2] , [city] ,
		 [state] , [zipcode] , [phone1] , [socialsecuritynumber] , [sex] , [dateofbirth] 
		 FROM dbo.[_import_4_5_PatientData]) AS holder ON
