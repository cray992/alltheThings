...
IF  EXISTS (SELECT * FROM sys.objects WHERE object_id = OBJECT_ID(N'[dbo].[_import_2_1_Insurances]') AND type in (N'U'))
BEGIN
...
PRINT 'Inserting into Insurance Company...'
      ...
      FROM dbo._import_2_1_Insurances
...
PRINT 'Inserting into Insurance Company Plan...'
      ...
      SELECT DISTINCT
                    ...
                    @VendorImportID  -- VendorImportID - int
      FROM dbo._import_2_1_Insurances AS impIns
...
IF  EXISTS (SELECT * FROM sys.objects WHERE object_id = OBJECT_ID(N'[dbo].[_import_2_1_ReferringPhys]') AND type in (N'U'))
BEGIN
...
PRINT 'Inserting into Referring Doctor...'
      ...
      SELECT DISTINCT
                    ...
                    1  -- External - bit
      FROM dbo._import_2_1_ReferringPhys AS impRph
      WHERE impRph.[firstname] <> '' AND impRph.[lastname] <> ''
PRINT CAST(@@ROWCOUNT AS VARCHAR) + ' records inserted'
END 

-- Patients (only if table exists)
IF  EXISTS (SELECT * FROM sys.objects WHERE object_id = OBJECT_ID(N'[dbo].[_import_2_1_PatientData]') AND type in (N'U'))
BEGIN
...
PRINT 'Inserting into Patient...'
      ...
      SELECT DISTINCT
                    ...
                    CASE impCas.[maritalstatus] WHEN 'Divorced' THEN 'D'
                    ...
                    CASE WHEN impCas.[Guarantor] <> impCas.[chartnumber] THEN 1 ELSE 0 END , -- ResponsibleDifferentThanPatient - bit
                    '' , -- ResponsiblePrefix - varchar(16)
                    CASE WHEN impCas.[Guarantor] <> impCas.[chartnumber] THEN guar.[firstname] END , -- ResponsibleFirstName - varchar(64)
                    CASE WHEN impCas.[Guarantor] <> impCas.[chartnumber] THEN guar.[middleinitial] END , -- ResponsibleMiddleName - varchar(64)
                    CASE WHEN impCas.[Guarantor] <> impCas.[chartnumber] THEN guar.[lastname] END , -- ResponsibleLastName - varchar(64)
                    '' , -- ResponsibleSuffix - varchar(16)
                    CASE WHEN impCas.[Guarantor] <> impCas.[chartnumber] THEN 'O' ELSE 'S' END , -- ResponsibleRelationshipToPatient - varchar(1)
                    CASE WHEN impCas.[Guarantor] <> impCas.[chartnumber] THEN guar.[street1] END , -- ResponsibleAddressLine1 - varchar(256)
                    CASE WHEN impCas.[Guarantor] <> impCas.[chartnumber] THEN guar.[street2] END , -- ResponsibleAddressLine2 - varchar(256)
                    CASE WHEN impCas.[Guarantor] <> impCas.[chartnumber] THEN guar.[city] END , -- ResponsibleCity - varchar(128)
                    CASE WHEN impCas.[Guarantor] <> impCas.[chartnumber] THEN LEFT(guar.[state] , 2) END , -- ResponsibleState - varchar(2)
                    CASE      WHEN impCas.[Guarantor] <> impCas.[chartnumber] THEN (CASE WHEN LEN(dbo.fn_RemoveNonNumericCharacters(guar.[zipcode])) IN (5,9) THEN dbo.fn_RemoveNonNumericCharacters(guar.[zipcode])
                              WHEN LEN(dbo.fn_RemoveNonNumericCharacters(guar.[zipcode])) IN (4,8) THEN '0' + dbo.fn_RemoveNonNumericCharacters(guar.[zipcode])
      ...
      FROM dbo._import_2_1_PatientData AS impPat
      LEFT OUTER JOIN dbo._import_2_1_Providers AS impPro ON
      ...
      LEFT OUTER JOIN dbo._import_2_1_CaseInformation AS impCas ON
            impCas.[chartnumber] = impPat.[chartnumber]
      LEFT OUTER JOIN dbo._import_2_1_PatientData AS guar ON
            guar.[chartnumber] = impCas.[guarantor]
      WHERE impPat.[firstname] <> '' AND impPat.[lastname] <> ''
PRINT CAST(@@ROWCOUNT AS VARCHAR) + ' records inserted'
END 

-- Patient Cases, Case Dates and Policies (onlyl if table exists)
IF  EXISTS (SELECT * FROM sys.objects WHERE object_id = OBJECT_ID(N'[dbo].[_import_2_1_CaseInformation]') AND type in (N'U'))
BEGIN
...
PRINT 'Inserting into Patient Case...'
      ...
      SELECT DISTINCT
              realP.[PatientID] , -- PatientID - int
                    ...
                    0 , -- ModifiedUserID - int
                    @PracticeID , -- PracticeID - int
                    realP.VendorID , -- VendorID - varchar(50)
                    @VendorImportID , -- VendorImportID - int
     ...
     FROM dbo.Patient AS realP
	  LEFT JOIN dbo.[_import_4_1_CaseInformation] AS impCas ON
			impCas.chartnumber = realP.VendorID
...
PRINT 'Inserting into PatientCaseDate - First Consulted...'
      ...
      SELECT DISTINCT
                    ...
                    GETDATE() , -- ModifiedDate - datetime
                    0  -- ModifiedUserID - int
      FROM dbo._import_2_1_CaseInformation AS impCas
      INNER JOIN dbo.PatientCase AS realpc ON
            realpc.[VendorID] = impCas.[chartnumber] AND
...
PRINT 'Inserting into Primary Insurance Policy...'
      ...
      SELECT DISTINCT
                    ...
                    CASE WHEN impCas.[insuredrelationship1] <> 'Self' AND impCas.[insured1] <> impCas.[chartnumber] THEN holder.[firstname] END , -- HolderFirstName - varchar(64)
                    CASE WHEN impCas.[insuredrelationship1] <> 'Self' AND impCas.[insured1] <> impCas.[chartnumber] THEN holder.[middleinitial] END , -- HolderMiddleName - varchar(64)
                    CASE WHEN impCas.[insuredrelationship1] <> 'Self' AND impCas.[insured1] <> impCas.[chartnumber] THEN holder.[lastname] END , -- HolderLastName - varchar(64)
                    ...
                    CASE WHEN impCas.[insuredrelationship1] <> 'Self' AND impCas.[insured1] <> impCas.[chartnumber] THEN holder.[socialsecuritynumber] END , -- HolderSSN - char(11)
                    GETDATE() , -- CreatedDate - datetime
                    0 , -- CreatedUserID - int
                    GETDATE() , -- ModifiedDate - datetime
                    0 , -- ModifiedUserID - int
                    ...
                    CASE WHEN impCas.[insuredrelationship1] <> 'Self' AND impCas.[insured1] <> impCas.[chartnumber] THEN holder.[street1] END , -- HolderAddressLine1 - varchar(256)
                    CASE WHEN impCas.[insuredrelationship1] <> 'Self' AND impCas.[insured1] <> impCas.[chartnumber] THEN holder.[street2] END , -- HolderAddressLine2 - varchar(256)
                    CASE WHEN impCas.[insuredrelationship1] <> 'Self' AND impCas.[insured1] <> impCas.[chartnumber] THEN holder.[city] END , -- HolderCity - varchar(128)
                    CASE WHEN impCas.[insuredrelationship1] <> 'Self' AND impCas.[insured1] <> impCas.[chartnumber] THEN LEFT(holder.[state] , 2) END , -- HolderState - varchar(2)
                    CASE WHEN impCas.[insuredrelationship1] <> 'Self' AND impCas.[insured1] <> impCas.[chartnumber] THEN '' END , -- HolderCountry - varchar(32)
                    CASE WHEN impCas.[insuredrelationship1] <> 'Self' AND impCas.[insured1] <> impCas.[chartnumber] THEN
                    ...
                    CASE WHEN impCas.[insuredrelationship1] <> 'Self' AND impCas.[insured1] <> impCas.[chartnumber] THEN LEFT(dbo.fn_RemoveNonNumericCharacters(holder.[phone1]) , 10) END , -- HolderPhone - varchar(10)
                    CASE WHEN impCas.[insuredrelationship1] <> 'Self' AND impCas.[insured1] <> impCas.[chartnumber] THEN impCas.[policynumber1] END , -- DependentPolicyNumber - varchar(32)
                    impCas.[notes] , -- Notes - text
                    impCas.[copaymentamount] , -- Copay - money
                    impCas.[annualdeductible] , -- Deductible - money
                    CASE WHEN impCas.[insuredrelationship1] <> 'Self' AND impCas.[insured1] <> impCas.[chartnumber] THEN impCas.[policynumber1] END , -- PatientInsuranceNumber - varchar(32)
                    1 , -- Active - bit
                    @PracticeID , -- PracticeID - int
                    impCas.[chartnumber] , -- VendorID - varchar(50)
                    @VendorImportID , -- VendorImportID - int
                    CASE      WHEN impCas.[groupname1] = '' THEN ''
                              ELSE LEFT(impCas.[groupname1] , 14) END  -- GroupName - varchar(14)
      FROM dbo._import_2_1_CaseInformation AS impCas
      INNER JOIN dbo.PatientCase AS realpc ON
            realpc.[VendorID] = impCas.[chartnumber] AND
...
IF  EXISTS (SELECT * FROM sys.objects WHERE object_id = OBJECT_ID(N'[dbo].[_import_2_1_StandardFeeSchedule]') AND type in (N'U'))
BEGIN
...
PRINT 'Inserting into Standard Fee...'
      ...
      FROM dbo._import_2_1_StandardFeeSchedule AS Impsfs
...
IF  EXISTS (SELECT * FROM sys.objects WHERE object_id = OBJECT_ID(N'[dbo].[_import_2_1_Appointments]') AND type in (N'U'))
BEGIN
      --Appointment
      ...
      SELECT DISTINCT
                    ...
                    DATEADD(hh, CAST(impApp.timezone AS INT), impApp.[startdate]) , -- StartDate - datetime
                    DATEADD(hh, CAST(impApp.timezone AS INT), impApp.[enddate]) , -- EndDate - datetime
                    ...
                    (impApp.[starttm] + CAST(impApp.[TimeZone] AS INT)*100) , -- StartTm - smallint
                    (impApp.[endtm] + CAST(impApp.[TimeZone] AS INT)*100)  -- EndTm - smallint
      ...
      INNER JOIN dbo.Appointment AS realapp ON
            ...
            realapp.[StartDate] = impApp.[startdate]
...
PRINT 'Inserting into Patient Case for Balance Forward...'
      ...
      INNER JOIN dbo._import_2_1_PatientData AS impPat ON
...
PRINT 'Inserting into Encounter for Balance Forward...'
      ...
      FROM dbo._import_2_1_PatientData impPat
