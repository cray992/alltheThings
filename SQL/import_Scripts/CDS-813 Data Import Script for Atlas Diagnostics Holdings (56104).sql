...
FROM dbo.[_import_1_1_ServiceLocation] i
	LEFT JOIN dbo.ServiceLocation sl ON 
		i.Name = sl.Name AND 
		i.AddressLine1 = sl.AddressLine1
WHERE i.PracticeID = @SourcePracticeID AND sl.ServiceLocationID IS NULL
...
SELECT DISTINCT
          ...
          i.DefaultUnits , -- DefaultUnits - decimal
...
FROM dbo._import_1_1_ProcedureCodeDictionary i
...
FROM dbo.[_import_1_1_AppointmentReason] i
	INNER JOIN dbo.ProcedureCodeDictionary pcd ON
		LEFT(i.name, CHARINDEX(' ', i.name)) = pcd.ProcedureCode
