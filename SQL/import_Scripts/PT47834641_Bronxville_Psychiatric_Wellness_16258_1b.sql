...
IF  EXISTS (SELECT * FROM sys.objects WHERE object_id = OBJECT_ID(N'[dbo].[_import_8_1_Insurances]') AND type in (N'U'))
BEGIN
	PRINT ''
	...
	SELECT DISTINCT
		...
		13
	FROM dbo.[_import_8_1_Insurances]
	WHERE
	...
	FROM dbo.[_import_8_1_Insurances] impIns
...
IF  EXISTS (SELECT * FROM sys.objects WHERE object_id = OBJECT_ID(N'[dbo].[_import_8_1_ReferringPhys]') AND type in (N'U'))
BEGIN
...
-- Doctor Referring
	...
	SELECT DISTINCT
		...
		,1
	FROM dbo.[_import_8_1_ReferringPhys]
	WHERE lastname <> ''
	PRINT CAST(@@rowcount AS VARCHAR(10)) + ' records inserted'
...
IF  EXISTS (SELECT * FROM sys.objects WHERE object_id = OBJECT_ID(N'[dbo].[_import_8_1_PatientData]') AND type in (N'U'))
BEGIN

	DECLARE @PatientMaritalStatus TABLE (ChartNumber VARCHAR(100), MaritalStatus VARCHAR(20))
	IF EXISTS (SELECT * FROM sys.objects WHERE OBJECT_ID = OBJECT_ID(N'[dbo].[_import_8_1_CaseInformation]') AND type IN (N'U'))
	BEGIN
		...
		INSERT INTO @PatientMaritalStatus (ChartNumber, MaritalStatus) SELECT ChartNumber, MAX(MaritalStatus) FROM dbo.[_import_8_1_CaseInformation] WHERE MaritalStatus <> '' GROUP BY ChartNumber
	END
	...
	FROM dbo.[_import_8_1_PatientData] impP
...
BEGIN
    ...
    FROM dbo.[_import_8_1_CaseInformation] impCase
...
BEGIN
	...
	SELECT
		...
		,@VendorImportID
	FROM 
		dbo.[_import_8_1_CaseInformation] impCase
	INNER JOIN dbo.Patient realP ON 
	...
	FROM dbo.[_import_8_1_CaseInformation] impPC
	...
	INNER JOIN (
		SELECT DISTINCT [ChartNumber], [LastName], [FirstName], [MiddleInitial], [Street1], [Street2], [City], [State], [ZipCode], [Phone1], [Sex], [DateofBirth] FROM dbo.[_import_8_1_PatientData]
...
IF  EXISTS (SELECT * FROM sys.objects WHERE object_id = OBJECT_ID(N'[dbo].[_import_8_1_StandardFeeSchedule]') AND type in (N'U'))
BEGIN
	PRINT ''
	...
	SELECT  
          ...
          impSFS.[AmountA] , 
          0  
	FROM dbo.[_import_8_1_StandardFeeSchedule] impSFS
...
IF  EXISTS (SELECT * FROM sys.objects WHERE object_id = OBJECT_ID(N'[dbo].[_import_8_1_Appointments]') AND type in (N'U'))
BEGIN
	PRINT ''
				...
				DATEADD(hh, CAST(
				  ...
				  CASE WHEN charindex( 'PM' , app.[starttime], 0) > 0 then  
					  ...
					  ELSE
						...
						END 
				   END   -- EndTm - smallint
		FROM dbo.[_import_8_1_Appointments] app 
	...
	PRINT 'Inserting into AppointmentToResource...'
		...
		SELECT 
			...
			GETDATE(),
			@PracticeID
		FROM dbo.[_import_8_1_Appointments] impAppt
...
SELECT * FROM dbo.[_import_8_1_PatientData]
*/