...
SELECT DISTINCT
          ...
          1 , -- External - bit
          npi  -- NPI - varchar(10)
FROM dbo.[_import_2_14_REfProviders]
...
SELECT DISTINCT
          ...
          CASE impPat.defaultservicelocationid WHEN 83 THEN 79
											   WHEN 84 THEN 80
											   WHEN 85 THEN 81
											   WHEN 86 THEN 82
											   WHEN 77 THEN 78
...
FROM dbo.[_import_2_14_PatientDemographics] AS impPat
...
LEFT JOIN dbo.[_import_2_14_Employers] AS impEmp ON
	impEmp.employerid = impPat.employerid
LEFT JOIN dbo.Employers AS emp ON
	emp.EmployerName = impEmp.employername
LEFT JOIN dbo.[_import_2_14_ApptProvider] AS impAppP ON
...
SELECT DISTINCT
          ...
          1 , -- NoteTypeCode - int
          0  -- LastNote - bit
FROM dbo.[_import_2_14_NotesAlerts] AS impNA
...
FROM dbo.[_import_2_14_CaseInformation] AS impCas
...
SELECT DISTINCT
          ...
          @PracticeID , -- PracticeID - int
          78 , -- ServiceLocationID - int
          impApp.startdate , -- StartDate - datetime
          impApp.enddate , -- EndDate - datetime
          'P' , -- AppointmentType - varchar(1)
		  impApp.appointmentid , -- Subject
          impApp.notes , -- Notes - text
          GETDATE() , -- CreatedDate - datetime
          0 , -- CreatedUserID - int
          GETDATE() , -- ModifiedDate - datetime
          0 , -- ModifiedUserID - int
          1 , -- AppointmentResourceTypeID - int
          CASE WHEN impApp.appointmentconfirmationstatuscode <> '' THEN impApp.appointmentconfirmationstatuscode ELSE 'S' END , -- AppointmentConfirmationStatusCode - char(1)
          realpc.PatientCaseID , -- PatientCaseID - int
          dk.DKPracticeID , -- StartDKPracticeID - int
          dk.DKPracticeID , -- EndDKPracticeID - int
          impApp.starttm , -- StartTm - smallint
          impApp.endtm  -- EndTm - smallint
FROM dbo.[_import_2_14_Appointment] AS impApp
...
INNER JOIN dbo.Appointment app ON 
	...
	app.Subject = atar.AppointmentID
...
WHERE app.AppointmentType = 'P' AND app.CreatedDate BETWEEN DATEADD(mi,-2,GETDATE()) AND DATEADD(mi,2,GETDATE())
PRINT CAST(@@ROWCOUNT AS VARCHAR) + ' records inserted'
