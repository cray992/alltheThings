USE superbill_21956_dev
--USE superbill_21956_prod
GO
...
FROM dbo.[_import_3_68_InsuranceCOMPANYPLANList] AS IICL
WHERE ( insurancecompanyname <> '' OR planname <> '') AND
	NOT EXISTS (SELECT * FROM dbo.InsuranceCompany WHERE LEFT(iicl.insurancecompanyname, 50) = VendorID AND VendorImportID = 3)
...
SELECT DISTINCT
	...
	@VendorImportID -- VendorImportID - int
FROM dbo.[_import_3_68_InsuranceCOMPANYPLANList] ICP 
...
(	SELECT DISTINCT 
	...
	FROM dbo.[_import_3_68_PatientDemographics]
...
FROM #tempPolicies AS TP
	JOIN dbo.Patient AS PAT ON VendorImportID = 3 AND VendorID = PatientVendorID
	JOIN dbo.PatientCase AS PC ON PAT.PatientID = pc.PatientID AND PC.VendorImportID = 3 AND pc.ModifiedUserID = 0
...
SET 
	...
	NAME = 'Default Case'
WHERE 
	3 = VendorImportID AND 
	@PracticeID = PracticeID AND
	PatientCaseID IN (SELECT PatientCaseID FROM dbo.InsurancePolicy WHERE PracticeID = @PracticeID AND ModifiedUserID = 0) AND
	PayerScenarioID <> 5
