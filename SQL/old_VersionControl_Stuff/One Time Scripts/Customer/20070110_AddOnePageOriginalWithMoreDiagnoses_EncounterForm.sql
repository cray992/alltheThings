---------------------------------------------------------------------------------------------------------------------------------------------------------
-- Case 17916: Implement a one page customized encounter form for GR Medical Management(Customer 108).
---------------------------------------------------------------------------------------------------------------------------------------------------------

IF NOT EXISTS(SELECT EncounterFormTypeID FROM EncounterFormType WHERE EncounterFormTypeID = 57)
BEGIN
	...
	VALUES (57, '1-Page, Original with More Diagnoses', 'Encounter form that prints on a single page', 56, 1, 82, 1, 1, 1, 1, 1, 1)
END

IF NOT EXISTS(SELECT PrintingFormDetailsID FROM PrintingFormDetails WHERE PrintingFormDetailsID = 82)
BEGIN
	INSERT INTO PrintingFormDetails (PrintingFormDetailsID, PrintingFormID, SVGDefinitionID, Description, SVGTransform)
	VALUES (82, 9, 69, '1-Page, Original with More Diagnoses', 1)
END
	...
	<xsl:template match="/formData/page">
		...
		<svg xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" formId="EncounterForm" pageId="EncounterForm.1" width="8.5in" height="11in" topOffset="0.0in" leftOffset="0.0in" dpi="200">
			...
			<g id="InsuranceCoverage">
				...
				<text x="3.052in" y="2.22in" width="2.5in" height="0.1in" valueSource="EncounterForm.1.SecondaryIns1" />
			...
			<g id="EncounterInformation">
				...
				<text x="5.565in" y="2.22in" width="2.5in" height="0.1in" valueSource="EncounterForm.1.Provider1" />
				<text x="5.565in" y="2.52in" width="2.5in" height="0.1in" valueSource="EncounterForm.1.RefProvider1" />
			</g>
			<g id="ProcedureDetails">
				<xsl:call-template name="CreateProcedureColumnLayout">
					<xsl:with-param name="maximumRowsInColumn" select="32"/>
			...
			<g id="DiagnosisDetails">
				<xsl:call-template name="CreateDiagnosisColumnLayout">
					...
					<xsl:with-param name="maximumRowsInLastColumn" select="20"/>
					<xsl:with-param name="maximumColumns" select="3"/>
			...
			<g id="PreviousAccountBalance">
				<text x="0.537in" y="10.18in" width="1.22in" height="0.1in" valueSource="EncounterForm.1.LastInsPay1" />
				<text x="0.537in" y="10.48in" width="1.22in" height="0.1in" valueSource="EncounterForm.1.LastPatientPay1" />
				<text x="1.77in" y="10.18in" width="1.25in" height="0.1in" valueSource="EncounterForm.1.InsBalance1" />
				<text x="1.77in" y="10.48in" width="1.25in" height="0.1in" valueSource="EncounterForm.1.PatientBalance1" />
			</g>
		</svg>
	</xsl:template>
</xsl:stylesheet>'
WHERE PrintingFormDetailsID = 82