--------------------------------------------------------------------------------------------------------------------------------------------------
-- Case 13441: Implement a one page customized encounter form for Tomball Foot And Ankle Center, a practice of PRS Medical Managers(Customer 278).
--------------------------------------------------------------------------------------------------------------------------------------------------

IF NOT EXISTS(SELECT EncounterFormTypeID FROM EncounterFormType WHERE EncounterFormTypeID = 41)
BEGIN
	INSERT INTO EncounterFormType (EncounterFormTypeID, [Name], Description, SortOrder, NumberOfPages, PageOneDetailsID, ShowProcedures, ShowDiagnoses)
	VALUES (41, 'One Page No Grid Lines Version 2', 'Encounter form that prints on a single page', 41, 1, 57, 1, 1)
END

IF NOT EXISTS(SELECT PrintingFormDetailsID FROM PrintingFormDetails WHERE PrintingFormDetailsID = 57)
BEGIN
	INSERT INTO PrintingFormDetails
		(PrintingFormDetailsID, PrintingFormID, SVGDefinitionID, Description, SVGTransform)
	VALUES
		(57, 9, 44, 'One Page No Grid Lines Version 2', 1)
END
...
<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns="http://www.w3.org/2000/svg">
	...
	<xsl:template name="CreateProceduresColumnLayout">
		...
		<xsl:variable name="yOffset" select="0.13325"/>
		<xsl:variable name="codeXOffset" select="1.48"/>
		<xsl:variable name="descriptionXOffset" select="0.01"/>
		...
		<xsl:if test="$totalElements >= $currentElement and $maximumColumns >= $currentColumn">
			...
			<xsl:choose>
				...
				<xsl:when test="$currentCategory != $CurrentCategoryIndex">
					...
					<text x="{$xLeftOffset + $xOffset * ($currentColumn - 1) + $descriptionXOffset}in" y="{$yTopOffset + $descriptionYOffset + $yOffset * ($currentRow + 1)}in" width="{$descriptionWidth + $codeWidth + 0.07}in" height="0.1in" valueSource="EncounterForm.1.ProcedureCategoryName{$CurrentCategoryIndex}" class="bold" />
					<!-- Creates the detail lines -->
					<text x="{$xLeftOffset + $xOffset * ($currentColumn - 1) + $descriptionXOffset}in" y="{$yTopOffset + $codeYOffset + $yOffset * ($currentRow + 2)}in" width="{$descriptionWidth}in" height="0.10in" font-size="6pt" valueSource="EncounterForm.1.ProcedureName{$currentElement}" />
					<text x="{$xLeftOffset + $xOffset * ($currentColumn - 1) + $codeXOffset}in" y="{$yTopOffset + $codeYOffset + $yOffset * ($currentRow + 2)}in" width="{$codeWidth}in" height="0.10in" font-size="6pt" valueSource="EncounterForm.1.ProcedureCode{$currentElement}" />
				...
				<xsl:otherwise>
					...
					<!-- Creates the detail lines -->
					<text x="{$xLeftOffset + $xOffset * ($currentColumn - 1) + $descriptionXOffset}in" y="{$yTopOffset + $codeYOffset + $yOffset * ($currentRow + 1)}in" width="{$descriptionWidth}in" height="0.10in" font-size="6pt" valueSource="EncounterForm.1.ProcedureName{$currentElement}" />
					<text x="{$xLeftOffset + $xOffset * ($currentColumn - 1) + $codeXOffset}in" y="{$yTopOffset + $codeYOffset + $yOffset * ($currentRow + 1)}in" width="{$codeWidth}in" height="0.10in" font-size="6pt" valueSource="EncounterForm.1.ProcedureCode{$currentElement}" />
	...
	<xsl:template name="CreateDiagnosesColumnLayout">
		...
		<xsl:variable name="xLeftOffset" select="0.50"/>
		<xsl:variable name="xOffset" select="1.875"/>
		<xsl:variable name="yTopOffset" select="5.31"/>
		<xsl:variable name="yOffset" select="0.13325"/>
		<xsl:variable name="codeXOffset" select="1.48"/>
		<xsl:variable name="descriptionXOffset" select="0.01"/>
		<xsl:variable name="codeYOffset" select="0.04"/>
		...
		<xsl:if test="$totalElements >= $currentElement and $maximumColumns >= $currentColumn">
			...
			<xsl:choose>
				...
				<xsl:when test="$currentCategory != $CurrentCategoryIndex">
					...
					<!-- Creates the actual category line -->
					<text x="{$xLeftOffset + $xOffset * ($currentColumn - 1) + $descriptionXOffset}in" y="{$yTopOffset + $descriptionYOffset + $yOffset * ($currentRow + 1)}in" width="{$descriptionWidth + $codeWidth + 0.07}in" height="0.10in" valueSource="EncounterForm.1.DiagnosisCategoryName{$CurrentCategoryIndex}" class="bold" />
					<!-- Creates the detail lines -->
					<text x="{$xLeftOffset + $xOffset * ($currentColumn - 1) + $descriptionXOffset}in" y="{$yTopOffset + $codeYOffset + $yOffset * ($currentRow + 2)}in" width="{$descriptionWidth}in" height="0.10in" font-size="6pt" valueSource="EncounterForm.1.DiagnosisName{$currentElement}"/>
					<text x="{$xLeftOffset + $xOffset * ($currentColumn - 1) + $codeXOffset}in" y="{$yTopOffset + $codeYOffset + $yOffset * ($currentRow + 2)}in" width="{$codeWidth}in" height="0.10in" font-size="6pt" valueSource="EncounterForm.1.DiagnosisCode{$currentElement}"/>
					<!-- Calls the same template recursively (increments 2 to the current row for the category line) -->
					<xsl:call-template name="CreateDiagnosesColumnLayout">
						<xsl:with-param name="maximumRowsInColumn" select="$maximumRowsInColumn"/>
						<xsl:with-param name="maximumColumns" select="$maximumColumns"/>
				...
				<xsl:otherwise>
					<!-- The category is the same and there is enough room on this column so print out the detail -->
					<!-- Creates the detail lines -->
					<text x="{$xLeftOffset + $xOffset * ($currentColumn - 1) + $descriptionXOffset}in" y="{$yTopOffset + $codeYOffset + $yOffset * ($currentRow + 1)}in" width="{$descriptionWidth}in" height="0.10in" font-size="6pt" valueSource="EncounterForm.1.DiagnosisName{$currentElement}"/>
					<text x="{$xLeftOffset + $xOffset * ($currentColumn - 1) + $codeXOffset}in" y="{$yTopOffset + $codeYOffset + $yOffset * ($currentRow + 1)}in" width="{$codeWidth}in" height="0.10in" font-size="6pt" valueSource="EncounterForm.1.DiagnosisCode{$currentElement}"/>
					<!-- Calls the same template recursively -->
					<xsl:call-template name="CreateDiagnosesColumnLayout">
						<xsl:with-param name="maximumRowsInColumn" select="$maximumRowsInColumn"/>
						<xsl:with-param name="maximumColumns" select="$maximumColumns"/>
	...
	<xsl:template match="/formData/page">
		...
		<svg xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" formId="EncounterForm" pageId="EncounterForm.1" width="8.5in" height="11in" topOffset="0.0in" leftOffset="0.0in" dpi="300">
			...
			<g id="DiagnosisDetails">
				<text x="0.50in" y="5.51in" font-size="6pt">__________________________________________________________________________________________DIAGNOSIS____________________________________________________________________________________________</text>
				<xsl:call-template name="CreateDiagnosesColumnLayout">
					<xsl:with-param name="maximumRowsInColumn" select="31"/>
					<xsl:with-param name="maximumColumns" select="4"/>
...
WHERE PrintingFormDetailsID = 57
