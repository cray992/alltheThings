...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" version="1.0">
	...
	<xsl:template match="/formData/page">
        ...
        <xsl:if test="data[@id=''CMS1500.1.PlaceOfService1''] != 12">
          ...
          <xsl:choose>
            <xsl:when test="data[@id=''CMS1500.1.PayerInsuranceProgramCode1''] = ''MB'' and (data[@id=''CMS1500.1.PayerState1''] = ''NY'' or data[@id=''CMS1500.1.PayerState1''] = ''NJ'')"></xsl:when>
            <xsl:otherwise>
  ...
  <xsl:template match="/formData/page">
    <formData formId="CMS1500">
      <page pageId="CMS1500.1">
        ...
        <xsl:variable name="mod90-total1">
          <xsl:choose>
            <xsl:when test="data[@id=''CMS1500.1.ProcedureModifier11'']=''90''">
              <xsl:value-of select="data[@id=''CMS1500.1.ChargeAmount1'']" />
            </xsl:when>
            <xsl:otherwise>0</xsl:otherwise>
          </xsl:choose>
        </xsl:variable>
        <xsl:variable name="mod90-total2">
          <xsl:choose>
            <xsl:when test="data[@id=''CMS1500.1.ProcedureModifier12'']=''90''">
              <xsl:value-of select="data[@id=''CMS1500.1.ChargeAmount2'']" />
            </xsl:when>
            <xsl:otherwise>0</xsl:otherwise>
          </xsl:choose>
        </xsl:variable>
        <xsl:variable name="mod90-total3">
          <xsl:choose>
            <xsl:when test="data[@id=''CMS1500.1.ProcedureModifier13'']=''90''">
              <xsl:value-of select="data[@id=''CMS1500.1.ChargeAmount3'']" />
            </xsl:when>
            <xsl:otherwise>0</xsl:otherwise>
          </xsl:choose>
        </xsl:variable>
        <xsl:variable name="mod90-total4">
          <xsl:choose>
            <xsl:when test="data[@id=''CMS1500.1.ProcedureModifier14'']=''90''">
              <xsl:value-of select="data[@id=''CMS1500.1.ChargeAmount4'']" />
            </xsl:when>
            <xsl:otherwise>0</xsl:otherwise>
          </xsl:choose>
        </xsl:variable>
        <xsl:variable name="mod90-total5">
          <xsl:choose>
            <xsl:when test="data[@id=''CMS1500.1.ProcedureModifier15'']=''90''">
              <xsl:value-of select="data[@id=''CMS1500.1.ChargeAmount5'']" />
            </xsl:when>
            <xsl:otherwise>0</xsl:otherwise>
          </xsl:choose>
        </xsl:variable>
        <xsl:variable name="mod90-total6">
          <xsl:choose>
            <xsl:when test="data[@id=''CMS1500.1.ProcedureModifier16'']=''90''">
              <xsl:value-of select="data[@id=''CMS1500.1.ChargeAmount6'']" />
            </xsl:when>
            <xsl:otherwise>0</xsl:otherwise>
          </xsl:choose>
        </xsl:variable>
        <!-- Create the variable for the totaling Modifier 90 charges -->
        <xsl:variable name="mod90-total" select="$mod90-total1+$mod90-total2+$mod90-total3+$mod90-total4+$mod90-total5+$mod90-total6" />
        <data id="CMS1500.1.dExtra1">
          <xsl:value-of select="data[@id=''CMS1500.1.ProcedureModifier21'']" />
          <xsl:value-of select="data[@id=''CMS1500.1.ProcedureModifier31'']" />
          <xsl:value-of select="data[@id=''CMS1500.1.ProcedureModifier41'']" />
        ...
        <!-- Handle Outside Lab based on the totals for Modifier 90 -->
        <data id="CMS1500.1.2_0Dollars">
          <xsl:variable name="charges-dollars" select="substring-before(format-number($mod90-total, ''#0.00'', ''default-format''), ''.'')" />
          <xsl:value-of select="substring(''      '', 1, 6 - string-length($charges-dollars))" />
          <xsl:value-of select="$charges-dollars" />
        </data>
        <data id="CMS1500.1.2_0Cents">
          <xsl:variable name="charges-cents" select="substring-after(format-number($mod90-total, ''#0.00'', ''default-format''), ''.'')" />
          <xsl:text>.</xsl:text>
          <xsl:value-of select="$charges-cents" />
        </data>
        <xsl:if test="$mod90-total=0">
          <data id="CMS1500.1.2_0Yes"> </data>
          <data id="CMS1500.1.2_0No">X</data>
        </xsl:if>
        <xsl:if test="$mod90-total>0">
          <data id="CMS1500.1.2_0Yes">X</data>
