--------------------------------------------------------------------------------------------------------------------------------------
-- Case 13425: Implement a one page customized encounter form for Dr. Roger McNeill, a provider of PRS Medical Managers(Customer 278).
--------------------------------------------------------------------------------------------------------------------------------------

IF NOT EXISTS(SELECT EncounterFormTypeID FROM EncounterFormType WHERE EncounterFormTypeID = 40)
BEGIN
	...
	VALUES (40, 'One Page DMN', 'Encounter form that prints on a single page', 40, 1, 56, 1, 1, 1, 1, 1, 1)
END

IF NOT EXISTS(SELECT PrintingFormDetailsID FROM PrintingFormDetails WHERE PrintingFormDetailsID = 56)
BEGIN
	INSERT INTO PrintingFormDetails
		(PrintingFormDetailsID, PrintingFormID, SVGDefinitionID, Description, SVGTransform)
	VALUES
		(56, 9, 43, 'One Page DMN', 1)
END
