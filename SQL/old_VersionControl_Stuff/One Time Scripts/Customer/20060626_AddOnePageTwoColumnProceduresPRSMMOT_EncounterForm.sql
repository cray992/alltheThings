----------------------------------------------------------------------------------------------------------------------------
-- Case 12045: Implement a one page, with two Procedures columns, customized encounter form for PRSMMOT practice TIOPASM. 
----------------------------------------------------------------------------------------------------------------------------

IF NOT EXISTS(SELECT EncounterFormTypeID FROM EncounterFormType WHERE EncounterFormTypeID = 23)
BEGIN
	...
	VALUES (23, '1-Page, 2-Column Procedures', 'Encounter form that prints on a single page', 23, 1, 41, 1, 1, 1)
END
ELSE
BEGIN
	-- Fixes any existing records that might be messed up
	UPDATE EncounterFormType SET PageOneDetailsID=41, ShowPatientBalance=1 WHERE EncounterFormTypeID=23
END

IF NOT EXISTS(SELECT PrintingFormDetailsID FROM PrintingFormDetails WHERE PrintingFormDetailsID = 41)
BEGIN
	INSERT INTO PrintingFormDetails
		(PrintingFormDetailsID, PrintingFormID, SVGDefinitionID, Description, SVGTransform)
	VALUES
		(41, 9, 26, '1-Page, 2-Column Procedures', 1)
END
...
<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns="http://www.w3.org/2000/svg">
	...
	<xsl:template name="CreateProcedureColumnLayout">
		...
		<xsl:variable name="yTopOffset" select="2.11"/>
		...
		<xsl:if test="$totalElements >= $currentElement and $maximumColumns >= $currentColumn">
			...
			<xsl:choose>
				...
				<xsl:when test="$currentCategory != $CurrentCategoryIndex">
					...
					<text x="{$xLeftOffset + $xOffset * ($currentColumn - 1) + $descriptionXOffset}in" y="{$yTopOffset + $yOffset * ($currentRow - 1)}in" width="3.44in" height="0.1in" valueSource="EncounterForm.1.ProcedureCategoryName{$CurrentCategoryIndex}" style="fill:black" font-family="Arial" font-weight="bold" font-size="9.5pt" />
					<!-- Creates the detail lines -->
					<text x="{$xLeftOffset + $xOffset * ($currentColumn - 1) + $codeXOffset}in" y="{$yTopOffset + $yOffset * $currentRow}in" width=".33in" height="0.1in" valueSource="EncounterForm.1.ProcedureCode{$currentElement}" font-family="Arial Narrow" font-size="7pt"/>
					<text x="{$xLeftOffset + $xOffset * ($currentColumn - 1) + $descriptionXOffset}in" y="{$yTopOffset + $yOffset * $currentRow}in" width="3.44in" height="0.1in" valueSource="EncounterForm.1.ProcedureName{$currentElement}" font-family="Arial Narrow" font-size="8.5pt" />
				...
				<xsl:otherwise>
					...
					<!-- Creates the detail lines -->
					<text x="{$xLeftOffset + $xOffset * ($currentColumn - 1) + $codeXOffset}in" y="{$yTopOffset + $yOffset * ($currentRow - 1)}in" width=".33in" height="0.1in" valueSource="EncounterForm.1.ProcedureCode{$currentElement}" font-family="Arial Narrow" font-size="7pt"/>
					<text x="{$xLeftOffset + $xOffset * ($currentColumn - 1) + $descriptionXOffset}in" y="{$yTopOffset + $yOffset * ($currentRow - 1)}in" width="3.44in" height="0.1in" valueSource="EncounterForm.1.ProcedureName{$currentElement}" font-family="Arial Narrow" font-size="8.5pt"/>
	...
	<xsl:template name="CreateDiagnosisColumnLayout">
		...
		<xsl:variable name="yTopOffset" select="7.5175"/>
		<xsl:variable name="yOffset" select=".140625"/>
		...
		<xsl:if test="$totalElements >= $currentElement and $maximumColumns >= $currentColumn">
			...
			<xsl:choose>
				<xsl:when test="(($currentColumn != $maximumColumns) and        (($currentCategory != $CurrentCategoryIndex and $currentRow + 1 > $maximumRowsInColumn) or        ($currentCategory = $CurrentCategoryIndex and $currentRow > $maximumRowsInColumn)) or        ($currentColumn = $maximumColumns) and         (($currentCategory != $CurrentCategoryIndex and $currentRow + 1 > $maximumRowsInLastColumn) or        ($currentCategory = $CurrentCategoryIndex and $currentRow > $maximumRowsInLastColumn)))">
					<!-- There isn''t enough room to print on this column -->
					<!-- Calls the same template recursively (sets current row to 1, increments the current column, and resets the current category so it will print) -->
				...
				<xsl:when test="$currentCategory != $CurrentCategoryIndex">
					<!-- The category is not the same but there is enough room on this column so print out the category -->
					<!-- Creates the actual category line -->
					<text x="{$xLeftOffset + $xOffset * ($currentColumn - 1) + $descriptionXOffset}in"         y="{$yTopOffset + $yOffset * ($currentRow - 1)}in"         width="1.55in" height="0.1in"         valueSource="EncounterForm.1.DiagnosisCategoryName{$CurrentCategoryIndex}" style="fill:black"         font-family="Arial" font-weight="bold" font-size="8pt"/>
					<!-- Creates the detail lines -->
	...
	<xsl:template name="DrawHorLines">
		...
		<xsl:if test="$maximumRows >= $currentRow">
			<line x1="0.5in" y1="{$yOffset + .140625 * ($currentRow)}in"       x2="8in" y2="{$yOffset + .140625 * ($currentRow)}in"       style="fill:none" stroke="black" stroke-width="0.5pt" stroke-dasharray="2, 2"/>
	...
	<xsl:template name="DrawVertLines">
		...
		<xsl:param name="LineOffset"/>
		<xsl:param name="yOffset"/>
		<xsl:param name="height"/>
		<xsl:param name="maximumCols"/>
		<xsl:param name="currentCol" select="0"/>
		<xsl:if test="$maximumCols > $currentCol">
			<line x1="{$xOffset+($currentCol)*$LineOffset}in" y1="{$yOffset}in"       x2="{$xOffset+($currentCol)*$LineOffset}in" y2="{$yOffset +$height}in"       style="fill:none" stroke="black" stroke-width="0.5pt" stroke-dasharray="2, 2"/>
			<xsl:call-template name="DrawVertLines">
				<xsl:with-param name="xOffset" select="$xOffset"/>
				<xsl:with-param name="LineOffset" select="$LineOffset"/>
				<xsl:with-param name="yOffset" select="$yOffset"/>
	...
	<xsl:template match="/formData/page">
		...
		<svg xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" formId="EncounterForm" pageId="EncounterForm.1" width="8.5in" height="11in" topOffset="0.0in" leftOffset="0.0in" dpi="200">
			<defs>
				<style type="text/css">       g       {       font-family: Arial,Arial Narrow,Helvetica;       font-size: 9pt;       font-style: Normal;       font-weight: Normal;       alignment-baseline: text-before-edge;       }         g#Title       {       font-family: Arial,Arial Narrow,Helvetica;       font-size: 16pt;       font-style: Normal;       font-weight: Normal;       alignment-baseline: text-before-edge;       }         text       {       baseline-shift: -100%;       }         text.ralignedtext       {       text-anchor: end;       }         text.centeredtext       {       text-anchor: middle;       }      </style>
			</defs>
			...
			<g id="ProcedureDetails">
				...
				<xsl:call-template name="CreateProcedureColumnLayout">
					<xsl:with-param name="maximumRowsInColumn" select="37"/>
			...
			<xsl:call-template name="DrawHorLines">
				...
				<xsl:with-param name="maximumRows" select="37"/>
			</xsl:call-template>
			<xsl:call-template name="DrawVertLines">
				<xsl:with-param name="xOffset" select="0.5"/>
				<xsl:with-param name="LineOffset" select="3.75"/>
				<xsl:with-param name="yOffset" select="2.125"/>
				<xsl:with-param name="height" select="5.1875"/>
				<xsl:with-param name="maximumCols" select="3"/>
			</xsl:call-template>
			<xsl:call-template name="DrawVertLines">
				...
				<xsl:with-param name="yOffset" select="2.125"/>
				<xsl:with-param name="height" select="5.1875"/>
				<xsl:with-param name="maximumCols" select="2"/>
			</xsl:call-template>
			<g id="DiagnosisDetails">
				<rect x="0.50in" y="7.3925in" width="7.5in" height="0.125in" fill="rgb(190, 190, 190)" stroke="black" stroke-width="0.5pt"/>
				<text x="0.55in" y="7.3925in" font-family="Arial" font-weight="bold" font-size="7pt">DIAGNOSES</text>
				<xsl:call-template name="CreateDiagnosisColumnLayout">
			...
			<xsl:call-template name="DrawHorLines">
				<xsl:with-param name="yOffset" select="7.5175"/>
				<xsl:with-param name="maximumRows" select="17"/>
			</xsl:call-template>
			<xsl:call-template name="DrawVertLines">
				<xsl:with-param name="xOffset" select="0.5"/>
				<xsl:with-param name="LineOffset" select="1.875"/>
				<xsl:with-param name="yOffset" select="7.53"/>
				<xsl:with-param name="height" select="2.375"/>
			...
			<g id="ReturnScheduleBox">
				<rect x="0.50in" y="9.975in" width="1.84375in" height="0.125in" fill="rgb(190, 190, 190)" stroke="black" stroke-width="0.5pt"/>
				<rect x="0.50in" y="9.975in" width="1.84375in" height="0.500in" fill="none" stroke="black"/>
				<text x="0.55in" y="9.98in" font-family="Arial" font-weight="bold" font-size="7pt">RETURN SCHEDULE</text>
				<text x="0.66in" y="10.15in" width="1.751in" height="0.500in" font-family="Arial" font-weight="bold" font-size="7pt">1      2      3      4      5      6      7</text>
				<text x="0.66in" y="10.30in" width="1.751in" height="0.500in" font-family="Arial" font-weight="bold" font-size="7pt">D       W       M       Y    PRN_____</text>
			</g>
			<g id="AccountStatusBox">
				<rect x="2.40625in" y="9.975in" width="1.8125in" height="0.125in" fill="rgb(190, 190, 190)" stroke="black" stroke-width="0.5pt"/>
				<rect x="2.40625in" y="9.975in" width="1.8125in" height="0.500in" fill="none" stroke="black"/>
				<text x="2.45625in" y="9.98in" font-family="Arial" font-weight="bold" font-size="7pt">ACCOUNT STATUS</text>
				<text x="2.56625in" y="10.135in" width="1.751in" height="0.500in" font-family="Arial Narrow" font-size="7pt"></text>
			</g>
			<g id="AccountActivityBox">
				<rect x="4.28125in" y="9.975in" width="1.8125in" height="0.125in" fill="rgb(190, 190, 190)" stroke="black" stroke-width="0.5pt"/>
				<rect x="4.28125in" y="9.975in" width="1.8125in" height="0.500in" fill="none" stroke="black"/>
				<text x="4.33125in" y="9.98in" font-family="Arial" font-weight="bold" font-size="7pt">ACCOUNT ACTIVITY</text>
				<text x="5.18125in" y="10.15in" font-family="Arial Narrow" font-weight="bold" font-size="7pt" class="ralignedtext">PRIOR BALANCE:</text>
				<text x="5.18125in" y="10.30in" font-family="Arial Narrow" font-weight="bold" font-size="7pt" class="ralignedtext">TODAY''S CHARGES:</text>
				<text x="5.21125in" y="10.13in" width="0.75in" height="0.1in"  font-family="Arial Narrow" font-weight="bold" font-size="8pt" valueSource="EncounterForm.1.PatientBalance1"/>
			</g>
			<g id="PaymentOnAccountBox">
				<rect x="6.15625in" y="9.975in" width="1.84375in" height="0.125in" fill="rgb(190, 190, 190)" stroke="black" stroke-width="0.5pt"/>
				<rect x="6.15625in" y="9.975in" width="1.84375in" height="0.500in" fill="none" stroke="black"/>
				<text x="6.20625in" y="9.98in" font-family="Arial" font-weight="bold" font-size="7pt">PAYMENT ON ACCOUNT</text>
				<circle cx="6.24625in" cy="10.22in" r="0.03in" fill="none" stroke="black" />
				<circle cx="6.24625in" cy="10.37in" r="0.03in" fill="none" stroke="black" />
				<circle cx="6.67625in" cy="10.22in" r="0.03in" fill="none" stroke="black" />
				<text x="6.30625in" y="10.15in" font-family="Arial Narrow" font-weight="bold" font-size="7pt" >CHECK</text>
				<text x="6.30625in" y="10.30in" font-family="Arial Narrow" font-weight="bold" font-size="7pt" >CREDIT CARD</text>
				<text x="6.73625in" y="10.15in" font-family="Arial Narrow" font-weight="bold" font-size="7pt" >CASH</text>
				<line x1="7.03125in" y1="10.10in" x2="7.03125in" y2="10.475in" stroke="black" stroke-width="0.5pt"/>
				<text x="7.50625in" y="10.15in" font-family="Arial Narrow" font-weight="bold" font-size="7pt" class="ralignedtext">DUE:</text>
				<text x="7.50625in" y="10.30in" font-family="Arial Narrow" font-weight="bold" font-size="7pt" class="ralignedtext">PAYMENT:</text>
			</g>
		</svg>
	</xsl:template>
</xsl:stylesheet>'
WHERE PrintingFormDetailsID = 41
