-----------------------------------------------------------------------------------------------------------------------------------------
-- Case 13071: Implement a one page customized encounter form for Dr. Linda Wang, a provider of California Medical Billing(Customer 327). 
-----------------------------------------------------------------------------------------------------------------------------------------

IF NOT EXISTS(SELECT EncounterFormTypeID FROM EncounterFormType WHERE EncounterFormTypeID = 32)
BEGIN
	...
	VALUES (32, 'One Page Three Column', 'Encounter form that prints on a single page', 32, 1, 48, 1, 1)
END

IF NOT EXISTS(SELECT PrintingFormDetailsID FROM PrintingFormDetails WHERE PrintingFormDetailsID = 48)
BEGIN
	INSERT INTO PrintingFormDetails
		(PrintingFormDetailsID, PrintingFormID, SVGDefinitionID, Description, SVGTransform)
	VALUES
		(48, 9, 35, 'One Page Three Column', 1)
END
...
<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns="http://www.w3.org/2000/svg">
	...
	<xsl:template name="CreateDiagnosesGridLines">
		...
		<xsl:param name="currentRow"/>
		<xsl:variable name="xLeftOffset" select="0.50"/>
		<xsl:variable name="yTopOffset" select="4.58"/>
		<xsl:variable name="yOffset" select="0.13"/>
		<xsl:variable name="lineWidth" select="7.50"/>
		<xsl:variable name="headerYOffset" select="0.01"/>
		<xsl:variable name="codeXOffset" select="0.02"/>
	...
	<xsl:template name="CreateDiagnosesColumnLayout">
		...
		<xsl:variable name="xLeftOffset" select="0.50"/>
		<xsl:variable name="xOffset" select="2.50"/>
		<xsl:variable name="yTopOffset" select="4.15"/>
		<xsl:variable name="yOffset" select="0.13"/>
		<xsl:variable name="codeXOffset" select="2.03"/>
		<xsl:variable name="descriptionXOffset" select="0.27"/>
		<xsl:variable name="codeYOffset" select="0.04"/>
	...
	<xsl:template match="/formData/page">
		...
		<svg xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" formId="EncounterForm" pageId="EncounterForm.1" width="8.5in" height="11in" topOffset="0.0in" leftOffset="0.0in" dpi="300">
			<defs>
				<style type="text/css">       g       {       font-family: Arial Narrow,Arial,Helvetica;       font-size: 7pt;       font-style: Normal;       font-weight: Normal;       alignment-baseline: text-before-edge;       }         text.bold       {       font-family: Arial,Arial Narrow,Helvetica;       font-size: 8pt;       font-style: Normal;       font-weight: bold;       alignment-baseline: text-before-edge;       }         g#Title       {       font-family: Arial,Arial Narrow,Helvetica;       font-size: 16pt;       font-style: Normal;       font-weight: Normal;       alignment-baseline: text-before-edge;       }         text       {       baseline-shift: -100%;       }         g#DiagnosisGrid line       {       stroke: black;       }         g#ProceduresGrid line       {       stroke: black;       }         text.centeredtext       {       font-size: 7pt;       text-anchor: middle;       }         text.ralignedtext       {       text-anchor: end;       }         text.header       {       font-weight: bold;       font-size: 12pt;       text-anchor: middle;       }        </style>
			</defs>
			<g id="Title">
				<text x="0.49in" y="0.43in" width="5.0in" height="0.10in" valueSource="EncounterForm.1.TemplateName1" />
			</g>
			<g id="PracticeInformation">
				<text x="0.49in" y="0.71in" width="5.0in" height="0.10in" font-size="9pt" valueSource="EncounterForm.1.PracticeName1" />
				<text x="0.49in" y="0.86in" width="5.0in" height="0.10in" font-size="9pt" valueSource="EncounterForm.1.PracticeAddress1" />
				<text x="0.49in" y="1.01in" width="5.0in" height="0.10in" font-size="9pt" valueSource="EncounterForm.1.PracticePhoneFax1" />
				<text x="0.49in" y="1.16in" width="5.0in" height="0.10in" font-size="9pt" valueSource="EncounterForm.1.PracticeTaxID1" />
			</g>
			...
			<g id="ProceduresGrid">
				<xsl:call-template name="CreateProceduresGridLines">
					<xsl:with-param name="maximumRows" select="9"/>
					<xsl:with-param name="currentRow" select="1"/>
				</xsl:call-template>
			</g>
			<g id="ProceduresDetails">
				<xsl:call-template name="CreateProceduresColumnLayout">
					<xsl:with-param name="maximumRowsInColumn" select="9"/>
			...
			<g id="DiagnosisGrid">
				<xsl:call-template name="CreateDiagnosesGridLines">
					<xsl:with-param name="maximumRows" select="47"/>
					<xsl:with-param name="currentRow" select="1"/>
				</xsl:call-template>
			</g>
			<g id="DiagnosisDetails">
				<xsl:call-template name="CreateDiagnosesColumnLayout">
					<xsl:with-param name="maximumRowsInColumn" select="47"/>
					<xsl:with-param name="maximumColumns" select="3"/>
...
WHERE PrintingFormDetailsID = 48
