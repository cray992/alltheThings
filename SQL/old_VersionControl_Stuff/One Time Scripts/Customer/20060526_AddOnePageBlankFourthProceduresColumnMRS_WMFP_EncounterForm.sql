----------------------------------------------------------------------------------------------------------------------------
-- Case 10966: Implement a one page, with a blank fourth Procedures column, customized encounter form for MRS practice WMFP. 
----------------------------------------------------------------------------------------------------------------------------

IF NOT EXISTS(SELECT EncounterFormTypeID FROM EncounterFormType WHERE EncounterFormTypeID = 22)
BEGIN
	...
	VALUES (22, 'One Page Blank Fourth Columns', 'Encounter form that prints on a single page', 22, 1, 40, 1, 1, 1)
END

IF NOT EXISTS(SELECT PrintingFormDetailsID FROM PrintingFormDetails WHERE PrintingFormDetailsID = 40)
BEGIN
	INSERT INTO PrintingFormDetails
		(PrintingFormDetailsID, PrintingFormID, SVGDefinitionID, Description, SVGTransform)
	VALUES
		(40, 9, 25, 'One Page Blank Fourth Columns', 1)
END
...
<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns="http://www.w3.org/2000/svg">
	...
	<xsl:template name="CreateProcedureColumnLayout">
		...
		<xsl:if test="$currentElement > $totalElements">
			<xsl:choose>
				<xsl:when test="$currentColumn = $maximumColumns">
					<rect x="6.125in" y="{7.1875 - ($maximumRowsInColumn + 1 - $currentRow) * 0.140625}in" width="1.875in" height="{($maximumRowsInColumn + 1 - $currentRow) * 0.140625}in" fill="rgb(255, 255, 255)" stroke="black" stroke-width="0.5pt" />
				</xsl:when>
				<xsl:when test="$maximumColumns > $currentColumn">
					<rect x="6.125in" y="2.125in" width="1.875in" height="5.0625in" fill="rgb(255, 255, 255)" stroke="black" stroke-width="0.5pt" />
				</xsl:when>
			</xsl:choose>
		</xsl:if>
	</xsl:template>
	<xsl:template name="CreateDiagnosisColumnLayout">
		...
		<xsl:variable name="xLeftOffset" select="0.495"/>
		<xsl:variable name="xOffset" select="1.875"/>
		<xsl:variable name="yTopOffset" select="7.376875"/>
		<xsl:variable name="yOffset" select=".140625"/>
		<xsl:variable name="codeXOffset" select=".03"/>
		...
		<xsl:if test="$currentElement > $totalElements">
			<xsl:choose>
				<xsl:when test="$currentColumn = $maximumColumns">
					<rect x="6.125in" y="{9.345625 - ($maximumRowsInLastColumn + 1 - $currentRow) * 0.140625}in" width="1.875in" height="{($maximumRowsInLastColumn + 1 - $currentRow) * 0.140625}in" fill="rgb(255, 255, 255)" stroke="black" stroke-width="0.5pt" />
				</xsl:when>
				<xsl:when test="$maximumColumns > $currentColumn">
					<rect x="6.125in" y="7.376875in" width="1.875in" height="1.96875in" fill="rgb(255, 255, 255)" stroke="black" stroke-width="0.5pt" />
				</xsl:when>
			</xsl:choose>
		</xsl:if>
	</xsl:template>
	...
	<xsl:template name="DrawVertLines">
		...
		<xsl:if test="$maximumCols > $currentCol">
			<line x1="{$xOffset+($currentCol)*1.875}in" y1="{$yOffset}in"       x2="{$xOffset+($currentCol)*1.875}in" y2="{$yOffset +$height}in"       style="fill:none" stroke="black" stroke-width="0.5pt" stroke-dasharray="2, 2"/>
		...
		<svg xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" formId="EncounterForm" pageId="EncounterForm.1" width="8.5in" height="11in" topOffset="0.0in" leftOffset="0.0in" dpi="200">
			...
			<g id="ProcedureDetails">
				...
				<xsl:call-template name="DrawHorLines">
					...
					<xsl:with-param name="maximumRows" select="36"/>
				</xsl:call-template>
				<xsl:call-template name="DrawVertLines">
					<xsl:with-param name="xOffset" select="0.5"/>
					<xsl:with-param name="yOffset" select="2.125"/>
					<xsl:with-param name="height" select="5.046875"/>
				...
				<xsl:call-template name="CreateProcedureColumnLayout">
					<xsl:with-param name="maximumRowsInColumn" select="36"/>
			...
			<g id="DiagnosisDetails">
				<rect x="0.50in" y="7.251875in" width="7.5in" height="0.125in" fill="rgb(190, 190, 190)" stroke="black" stroke-width="0.5pt"/>
				<!-- <rect x="0.50in" y="2.000in" width="7.50in" height="1.01in" fill="none" stroke="black"/> -->
				<text x="0.55in" y="7.251875in" font-family="Arial" font-weight="bold" font-size="7pt">DIAGNOSES</text>
				<!-- Draw the lines for Diagnosis Codes-->
				<xsl:call-template name="DrawHorLines">
					<xsl:with-param name="yOffset" select="7.376875"/>
					<xsl:with-param name="maximumRows" select="18"/>
				</xsl:call-template>
				<xsl:call-template name="DrawVertLines">
					<xsl:with-param name="xOffset" select="0.5"/>
					<xsl:with-param name="yOffset" select="7.389375"/>
					<xsl:with-param name="height" select="2.515625"/>
					<xsl:with-param name="maximumCols" select="5"/>
				</xsl:call-template>
				<xsl:call-template name="DrawVertLines">
					<xsl:with-param name="xOffset" select="0.8125"/>
				...
				<xsl:call-template name="CreateDiagnosisColumnLayout">
					<xsl:with-param name="maximumRowsInColumn" select="18"/>
					<xsl:with-param name="maximumRowsInLastColumn" select="14"/>
					<xsl:with-param name="maximumColumns" select="4"/>
			...
			<g id="PaymentOnAccountBox">
				...
				<text x="7.50625in" y="10.3in" font-family="Arial Narrow" font-weight="bold" font-size="7pt" class="ralignedtext">PAYMENT:</text>
			</g>
			<g id="Signature">
				<rect x="6.125in" y="9.345625in" width="1.875in" height="0.5625in" fill="rgb(255, 255, 255)" stroke="black" stroke-width="0.5pt" />
				<text x="6.185in" y="9.656875in" font-family="Arial Narrow" font-size="5pt">X</text>
				<line x1="6.155in" y1="9.766875in" x2="7.97in" y2="9.766875in" stroke="black"></line>
				<text x="6.185in" y="9.796875in" font-family="Arial Narrow" font-size="5pt">PHYSICIAN''S SIGNATURE</text>
			</g>
		</svg>
	</xsl:template>
</xsl:stylesheet>'
WHERE PrintingFormDetailsID = 40