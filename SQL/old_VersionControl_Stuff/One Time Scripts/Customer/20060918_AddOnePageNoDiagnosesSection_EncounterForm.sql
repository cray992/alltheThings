--------------------------------------------------------------------------------------------------------------------------------------------------------
-- Case 14005: Implement a one page customized encounter form for Dr. Marilyn Rolluts, a provider of SSK, LLC Medical Billing Specialists(Customer 720).
--------------------------------------------------------------------------------------------------------------------------------------------------------

IF NOT EXISTS(SELECT EncounterFormTypeID FROM EncounterFormType WHERE EncounterFormTypeID = 51)
BEGIN
	INSERT INTO EncounterFormType (EncounterFormTypeID, [Name], Description, SortOrder, NumberOfPages, PageOneDetailsID, ShowProcedures, ShowTertiaryInsurance)
	VALUES (51, 'One Page No Diagnoses Section', 'Encounter form that prints on a single page', 51, 1, 67, 1, 1)
END

IF NOT EXISTS(SELECT PrintingFormDetailsID FROM PrintingFormDetails WHERE PrintingFormDetailsID = 67)
BEGIN
	INSERT INTO PrintingFormDetails
		(PrintingFormDetailsID, PrintingFormID, SVGDefinitionID, Description, SVGTransform)
	VALUES
		(67, 9, 54, 'One Page No Diagnoses Section', 1)
END
...
<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns="http://www.w3.org/2000/svg">
	...
	<xsl:template name="CreateProceduresColumnLayout">
		...
		<xsl:variable name="yOffset" select="0.15625"/>
		<xsl:variable name="codeXOffset" select="1.98"/>
