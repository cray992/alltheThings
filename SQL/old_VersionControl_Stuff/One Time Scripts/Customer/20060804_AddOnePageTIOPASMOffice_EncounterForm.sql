------------------------------------------------------------------------------------------------------------------------------------------------------------
-- Case 13051: Implement a one page customized encounter form for TX Institute Of Pulmonary And Sleep Med, a practice of PRS Medical Managers(Customer 278).
------------------------------------------------------------------------------------------------------------------------------------------------------------

IF NOT EXISTS(SELECT EncounterFormTypeID FROM EncounterFormType WHERE EncounterFormTypeID = 37)
BEGIN
	...
	VALUES (37, 'One Page No Grid Lines', 'Encounter form that prints on a single page', 37, 1, 53, 1, 1, 1, 1, 1, 1)
END
ELSE
BEGIN
	-- Fixes any existing records that might be messed up
	UPDATE EncounterFormType SET PageOneDetailsID=53, ShowLastInsurancePaymentDate=1, ShowInsuranceBalance=1, ShowlastPatientPaymentDate=1, ShowPatientBalance=1 WHERE EncounterFormTypeID=37
END

IF NOT EXISTS(SELECT PrintingFormDetailsID FROM PrintingFormDetails WHERE PrintingFormDetailsID = 53)
BEGIN
	INSERT INTO PrintingFormDetails
		(PrintingFormDetailsID, PrintingFormID, SVGDefinitionID, Description, SVGTransform)
	VALUES
		(53, 9, 40, 'One Page No Grid Lines', 1)
END
