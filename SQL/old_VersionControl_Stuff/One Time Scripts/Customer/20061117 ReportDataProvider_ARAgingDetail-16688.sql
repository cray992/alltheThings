...
*/
		...
		WHERE @RespType = PayerTypeCode AND
			@RespID = PayerID AND
	...
	LEFT JOIN PatientCase pc ON pc.PracticeID = e.PracticeID AND pc.PatientCaseID = e.PatientCaseID
	WHERE 
		( @ProviderID = -1 OR e.DoctorID = @ProviderID ) AND
		( @ServiceLocationID = -1 OR e.LocationID = @ServiceLocationID ) AND 
		( @DepartmentID = -1 OR d.DepartmentID = @DepartmentID ) AND
		( @PayerScenarioID = -1 OR pc.PayerScenarioID = @PayerScenarioID ) AND
		( @PatientID = -1 OR p.PatientID = @PatientID ) AND 
		( @ContractID = -1 OR ASN.InsuranceCompanyPlanID IN (select cp.PlanID From ContractToInsurancePlan cp LEFT JOIN contract con ON  con.PracticeID = @PracticeID AND con.ContractID = cp.ContractID where con.ContractID = @ContractID ))
