---------------------------------------------------------------------------------------------------------
-- Case 15047 - Create the C4 printing form, printing form details, and billing form if it doesn't exist
---------------------------------------------------------------------------------------------------------

if not exists(select PrintingFormID from PrintingForm where PrintingFormID = 21)
begin
	...
	values		(21,
				'C4',
				'C-4',
				'BillDataProvider_GetC4DocumentData',
				1)
end

if not exists(select PrintingFormDetailsID from PrintingFormDetails where PrintingFormDetailsID = 78)
begin
...
<svg xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" printerAffinity="hcfa" formId="C4" pageId="C4.1" width="8.5in" height="11in" topOffset="0.0in" leftOffset="0.0in" dpi="200">
	<defs>
		<style type="text/css">        <![CDATA[        g {     font-family: Courier New;     font-size: 9pt;     font-style: Normal;     font-weight: bold;     alignment-baseline: text-before-edge;    }        text    {     baseline-shift: -100%;    }        text.money    {     text-anchor: end;    }      text.smaller    {     font-size: 9pt;    }              ]]>      </style>
	</defs>
...
if not exists(select BillingFormID from BillingForm where BillingFormID = 15)
begin
			...
			<page pageId="C4.1">
				...
				<xsl:if test="data[@id=''C4.1.ServiceUnitCount1''] &gt; 0">
					...
					<data id="C4.1.13G_ServiceUnitCount1">
						<xsl:value-of select="data[@id=''C4.1.ServiceUnitCount1'']"/>
					</data>
				</xsl:if>
				...
				<xsl:if test="data[@id=''C4.1.ServiceUnitCount2''] &gt; 0">
					...
					<data id="C4.1.13G_ServiceUnitCount2">
						<xsl:value-of select="data[@id=''C4.1.ServiceUnitCount2'']"/>
					</data>
				</xsl:if>
				...
				<xsl:if test="data[@id=''C4.1.ServiceUnitCount3''] &gt; 0">
					...
					<data id="C4.1.13G_ServiceUnitCount3">
						<xsl:value-of select="data[@id=''C4.1.ServiceUnitCount3'']"/>
					</data>
				</xsl:if>
				...
				<xsl:if test="data[@id=''C4.1.ServiceUnitCount4''] &gt; 0">
					...
					<data id="C4.1.13G_ServiceUnitCount4">
						<xsl:value-of select="data[@id=''C4.1.ServiceUnitCount4'']"/>
					</data>
				</xsl:if>
				...
				<xsl:if test="data[@id=''C4.1.ServiceUnitCount5''] &gt; 0">
					...
					<data id="C4.1.13G_ServiceUnitCount5">
						<xsl:value-of select="data[@id=''C4.1.ServiceUnitCount5'']"/>
					</data>
				</xsl:if>
				...
				<xsl:if test="data[@id=''C4.1.ServiceUnitCount6''] &gt; 0">
					...
					<data id="C4.1.13G_ServiceUnitCount6">
						<xsl:value-of select="data[@id=''C4.1.ServiceUnitCount6'']"/>
					</data>
				</xsl:if>
