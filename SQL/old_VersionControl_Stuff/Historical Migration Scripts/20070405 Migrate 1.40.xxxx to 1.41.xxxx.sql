...
------------------------------------------------------------------------------
-----------------------------------------------------------------------------
-- This is dbo.BillDataProvider_GetHCFAFormXml in order to be deployed with PATCH release 4/09/2007
...
--			WHERE PatientCaseID=@PatientCaseID AND InsuranceCompanyPlanID=@RecordID
			...
			SELECT @RecipientInsurancePolicyID=InsurancePolicyID from ClaimAccounting_Assignments where ClaimID=@ClaimID and LastAssignment=1
	...
	SELECT	TOP 1
		...
		CASE
			WHEN IsNull(PTIC.ExcludePatientPayment, 0) = 0 THEN dbo.BusinessRule_HCFABillTotalPaidAmount(DH.Document_HCFAID)
			WHEN IsNull(PTIC.ExcludePatientPayment, 0) = 1 THEN dbo.BusinessRule_HCFABillTotalPaidAmount(DH.Document_HCFAID) - dbo.BusinessRule_HCFABillTotalPatientPaidAmount(DH.Document_HCFAID)
		END AS TotalPaidAmount,

		CASE
			WHEN IsNull(PTIC.ExcludePatientPayment, 0) = 0 THEN dbo.BusinessRule_HCFABillTotalBalanceAmount(DH.Document_HCFAID)
			WHEN IsNull(PTIC.ExcludePatientPayment, 0) = 1 THEN dbo.BusinessRule_HCFABillTotalBalanceAmount(DH.Document_HCFAID) + dbo.BusinessRule_HCFABillTotalPatientPaidAmount(DH.Document_HCFAID)
		...
		DD.DiagnosisCode4,
		CASE
			WHEN PC.PregnancyRelatedFlag = 0 AND LIStartDate IS NOT NULL THEN dbo.fn_FormatDate(DATEADD(hour, 5, PCDTDateOfInjury.LIStartDate))
			WHEN PC.PregnancyRelatedFlag <> 0 AND LMPStartDate IS NOT NULL THEN dbo.fn_FormatDate(DATEADD(hour, 5, PCDTDateOfInjury.LMPStartDate))
			ELSE dbo.fn_FormatDate(DATEADD(hour, 5, PCDTDateOfInjury.ITRStartDate /*LIStartDate*/))
		END AS CurrentIllnessDate,
		...
		IP.PatientRelationshipToInsured AS PatientRelationshipToSubscriber,
		...
		CASE WHEN IC.ReferringProviderNumberTypeID IS NULL THEN 0 ELSE 1 END AS ShowReferringProviderInfo,
		...
		CASE
			...
			WHEN (		
					...
					WHERE PN.DoctorID = RP.DoctorID
						AND PN.ProviderNumberTypeID = IC.ReferringProviderNumberTypeID
						AND PN.AttachConditionsTypeID IN (1, 2)
						AND (PN.InsuranceCompanyPlanID IS NULL OR PN.InsuranceCompanyPlanID = ICP.InsuranceCompanyPlanID)
						AND (PN.LocationID IS NULL OR PN.LocationID = E.LocationID)
