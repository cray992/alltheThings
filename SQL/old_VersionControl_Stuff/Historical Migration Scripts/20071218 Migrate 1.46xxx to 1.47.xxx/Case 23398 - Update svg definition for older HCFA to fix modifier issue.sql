...
<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns="http://www.w3.org/2000/svg">
	...
	<xsl:template name="GenerateProcedureSVG">
		...
		<g id="box24_{$currentProcedure}">
			...
			<text x="3.46in" y="{$yProcedureLocation}in" width="0.3in" height="0.1in" valueSource="CMS1500.1.dModifier1{$currentProcedure}" />
	...
	<!--        Generates NDC procedure SVG for one procedure.       This prints over one row doubling up the NDC text in the same row.                    Parameters:          currentProcedure - index of the current procedure being processed          yProcedureLocation - y location for this procedure              -->
	<xsl:template name="GenerateNDCProcedureSVGOneLine">
		...
		<g id="box24_{$currentProcedure}_NDC_1row">
			...
			<text x="3.46in" y="{$yProcedureLocation + $NDCFirstLineOffset}in" width="0.3in" height="0.1in" valueSource="CMS1500.1.dModifier1{$currentProcedure}" />
	...
	<!--        Generates NDC procedure SVG for one procedure.       This prints over two rows so it requires two procedure locations                    Parameters:          currentProcedure - index of the current procedure being processed          yProcedureLocation1 - y location for the first procedure line          yProcedureLocation2 - y location for the second procedure line              -->
	<xsl:template name="GenerateNDCProcedureSVGTwoLine">
		...
		<g id="box24_{$currentProcedure}_NDC_2row">
			...
			<text x="3.46in" y="{$yProcedureLocation1}in" width="0.3in" height="0.1in" valueSource="CMS1500.1.dModifier1{$currentProcedure}" />
	...
	<!--            Takes in the number procedures to process.          Loops through each procedure to print.          If this is a regular procedure print in one row, if it is an NDC procedure print in two rows.          Keep track of how many rows have been printed.                    Parameters:          totalProcedures - total number of procedures to display          currentProcedure - index of the current procedure being processed          currentProcedureLine - current procedure line to print out              -->
				...
				<xsl:when test="$NDCFormat = 1">
					...
					<xsl:choose>
						<xsl:when test="$currentProcedureLine = 1">
							<!-- Note that procedure 2 y location is passed in even though this is supposed to be the first line.            This is due to the fact that we can''t print a one line NDC in procedure line 1. -->
