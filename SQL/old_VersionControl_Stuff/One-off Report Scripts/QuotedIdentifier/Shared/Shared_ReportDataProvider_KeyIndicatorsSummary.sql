...
if exists (select * from dbo.sysobjects where id = object_id(N'[dbo].[Shared_ReportDataProvider_KeyIndicatorsSummary]') and OBJECTPROPERTY(id, N'IsProcedure') = 1)
drop procedure [dbo].[Shared_ReportDataProvider_KeyIndicatorsSummary]
GO
...
BEGIN
	...
	SET @sql = @sql + ',@ReportType = ' + CAST( ISNULL(@ReportType, 1) as varchar)
