...
if exists (select * from dbo.sysobjects where id = object_id(N'[dbo].[DashboardDataProvider_UpdateKeyIndicatorsVolatileJob]') 
	and OBJECTPROPERTY(id, N'IsProcedure') = 1)
drop procedure [dbo].[DashboardDataProvider_UpdateKeyIndicatorsVolatileJob]
GO
...
---------------- Sets Claims amount for Charges, Adj, Applied Receipts -----------------
	...
	FROM ClaimAccounting ca (NOLOCK) 
			LEFT OUTER JOIN PaymentClaimTransaction PCT  (NOLOCK) ON  ca.PracticeID = pct.PracticeID AND CA.ClaimTransactionID=PCT.ClaimTransactionID -- AND CA.ClaimTransactionTypeCode = 'PAY'
			LEFT JOIN Payment P  (NOLOCK) ON  p.PracticeID = pct.PracticeID AND PCT.PaymentID=P.PaymentID 
			CROSS JOIN @Calendar c
	WHERE ISNULL(p.PostingDate, CA.PostingDate) <= c.EndDate
...
------------- Sets Refund Totals --------------------------------
	...
	FROM Refund r  (NOLOCK) CROSS JOIN @Calendar c
	WHERE PostingDate <= c.EndDate
	GROUP BY PracticeID, PeriodType, Period
...
------------------ Sets Receipts --------------------	
	...
	FROM Payment P (NOLOCK) CROSS JOIN @Calendar c
...
------------------ Days in AR ---------------------------------
	...
	FROM ClaimAccounting CA  (NOLOCK) 
		INNER JOIN @ClaimAmounts R ON CA.PracticeID = R.PracticeID AND CA.ClaimID=R.ClaimID AND R.InPeriod = 1 AND R.PostingDate IS NOT NULL AND R.Receipts <> 0
...
------------------ Preperation for Other Days -------
	...
	WHERE CA.PostingDate<=EndDate AND Status=0 AND ClaimTransactionTypeCode IN ('CST')
