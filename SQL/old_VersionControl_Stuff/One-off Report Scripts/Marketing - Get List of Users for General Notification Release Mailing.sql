...
FROM
	Users U
	INNER JOIN UserNotification UN ON UN.UserID = U.UserID
	LEFT OUTER JOIN @excludeEmails EE ON U.EmailAddress LIKE EE.EmailAddress
WHERE
	UN.NotificationOptionTypeID = 3
	AND Checked = 1
	AND AccountLocked = 0
	AND EE.EmailAddress IS NULL
ORDER BY
	REVERSE(SUBSTRING(REVERSE(U.EmailAddress),0,CHARINDEX('@',REVERSE(U.EmailAddress))))
