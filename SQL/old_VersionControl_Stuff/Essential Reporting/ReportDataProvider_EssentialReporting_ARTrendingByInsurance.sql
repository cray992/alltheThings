...
INSERT INTO #ASNMax(ClaimID, ClaimTransactionID)
	...
	WHERE PracticeID=@PracticeID AND caa.PostingDate<=@EndDate AND caa.PostingDate>=@BeginArDate
...
INSERT INTO #ASN(ClaimID, InsurancePolicyID, InsuranceCompanyPlanID, InsuranceCompanyID)
	...
	WHERE CAA.PracticeID=@PracticeID AND CAA.PostingDate<=@EndDate AND caa.PostingDate>=@BeginArDate
	-----------------------------------------------------------------------
