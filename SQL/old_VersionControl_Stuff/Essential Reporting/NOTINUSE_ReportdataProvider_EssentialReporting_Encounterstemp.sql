...
--CREATE PROCEDURE [dbo].[ReportdataProvider_Beta_EssentialBilling_Encounters] 
--	@BeginDate DATETIME,
--	@EndDate DATETIME,
...
----DEBUG----
 DECLARE
	@BeginDate DATETIME,
	@EndDate DATETIME,
	@BucketType VARCHAR(10)= 'yy', --dd-day, ww-Week, mm-Month, yy-year
	@PracticeID INT,
	@NumberOfIns INT

SELECT  @BeginDate = '1/1/2012' ,
        @EndDate = '7/20/2015' ,
		@PracticeID = 45,
		@NumberOfIns = 6
...
BEGIN
		...
		SELECT  @CurrentDate, 
				CASE @BucketType WHEN 'dd' THEN --This returns the 11:59 of current day
								 ...
								 WHEN 'ww' THEN -- This returns saturday night of the current week
											CASE WHEN DATEADD(s, -1, DATEADD(d, 1, DATEADD(d, (7 - C.day_of_Week ), @CurrentDate))) > @EndOfDayEndDate 
												THEN @EndOfDayEndDate ELSE DATEADD(s, -1, DATEADD(d, 1, DATEADD(d, (7 - C.day_of_Week ), @CurrentDate))) END
		...
		SET @CurrentDate = (SELECT CASE @BucketType WHEN 'dd' THEN DATEADD(d, 1, @CurrentDate) 
													WHEN 'ww' THEN DATEADD(d, 1, DATEADD(d, (7 - C.day_of_Week ), @CurrentDate))
