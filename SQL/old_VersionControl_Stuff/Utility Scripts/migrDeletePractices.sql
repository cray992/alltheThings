...
--	PRINT 'elapse time: ' + CAST( DATEDIFF(s, @time, GETDATE()) AS VARCHAR ) + ' Deleted '+ CAST(ISNULL(@rows,0) AS varchar) + ' rows FROM table ' + @table_name 
	...
	-- Contracts and Fees
	-- Standard Rates
	SET @time = GETDATE() SET @table_name ='ContractsAndFees_StandardFeeScheduleLink'
	DELETE FROM ContractsAndFees_StandardFeeScheduleLink
	WHERE StandardFeeScheduleID IN (SELECT StandardFeeScheduleID FROM dbo.ContractsAndFees_StandardFeeSchedule WHERE PracticeID NOT IN (select practiceID from @practice ))
	SET @rows = @@ROWCOUNT
	PRINT 'elapse time: ' + CAST( DATEDIFF(s, @time, GETDATE()) AS VARCHAR ) + ' Deleted '+ CAST(ISNULL(@rows,0) AS varchar) + ' rows FROM table ' + @table_name 

	SET @time = GETDATE() SET @table_name ='ContractsAndFees_StandardFee'
	DELETE FROM ContractsAndFees_StandardFee
	WHERE StandardFeeScheduleID IN (SELECT StandardFeeScheduleID FROM dbo.ContractsAndFees_StandardFeeSchedule WHERE PracticeID NOT IN (select practiceID from @practice ))
	SET @rows = @@ROWCOUNT
	PRINT 'elapse time: ' + CAST( DATEDIFF(s, @time, GETDATE()) AS VARCHAR ) + ' Deleted '+ CAST(ISNULL(@rows,0) AS varchar) + ' rows FROM table ' + @table_name 

	SET @time = GETDATE() SET @table_name ='ContractsAndFees_StandardFeeSchedule'
	DELETE FROM ContractsAndFees_StandardFeeSchedule
	WHERE PracticeID NOT IN (select practiceID from @practice )
	SET @rows = @@ROWCOUNT
	PRINT 'elapse time: ' + CAST( DATEDIFF(s, @time, GETDATE()) AS VARCHAR ) + ' Deleted '+ CAST(ISNULL(@rows,0) AS varchar) + ' rows FROM table ' + @table_name 

	-- Contract Rates
	SET @time = GETDATE() SET @table_name ='ContractsAndFees_ContractRateScheduleLink'
	DELETE FROM ContractsAndFees_ContractRateScheduleLink
	WHERE ContractRateScheduleID IN (SELECT ContractRateScheduleID FROM dbo.ContractsAndFees_ContractRateSchedule WHERE PracticeID NOT IN (select practiceID from @practice ))
	SET @rows = @@ROWCOUNT
	PRINT 'elapse time: ' + CAST( DATEDIFF(s, @time, GETDATE()) AS VARCHAR ) + ' Deleted '+ CAST(ISNULL(@rows,0) AS varchar) + ' rows FROM table ' + @table_name 

	SET @time = GETDATE() SET @table_name ='ContractsAndFees_ContractRate'
	DELETE FROM ContractsAndFees_ContractRate
	WHERE ContractRateScheduleID IN (SELECT ContractRateScheduleID FROM dbo.ContractsAndFees_ContractRateSchedule WHERE PracticeID NOT IN (select practiceID from @practice ))
	SET @rows = @@ROWCOUNT
	PRINT 'elapse time: ' + CAST( DATEDIFF(s, @time, GETDATE()) AS VARCHAR ) + ' Deleted '+ CAST(ISNULL(@rows,0) AS varchar) + ' rows FROM table ' + @table_name 

	SET @time = GETDATE() SET @table_name ='ContractsAndFees_ContractRateSchedule'
